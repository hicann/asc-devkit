...
Ascend C API的操作数通常为[GlobalTensor](基础API/数据结构/GlobalTensor/GlobalTensor.md)和[LocalTensor](基础API/数据结构/LocalTensor/LocalTensor.md)，Tensor数据的存储位置均使用逻辑位置（[TPosition](基础API/辅助数据结构/TPosition.md)）来表达，从而隐藏了硬件架构的差异。TPosition类型包括：VECIN、VECOUT、VECCALC、A1、A2、B1、B2、CO1、CO2等，其与物理内存的映射关系如下表所示。
