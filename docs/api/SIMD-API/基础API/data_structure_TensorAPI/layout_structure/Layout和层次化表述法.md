...
通常Shape或Stride中的元素是一个单独的整数，但遇到一些复杂的内存排布情况时，普通二维表述可能无法准确表达，因此这里采用[层次化表述法](https://dl.acm.org/doi/abs/10.1145/3582016.3582018)。在这种方式中，元素也可以是一个元组。元组的第一个元素用于描述不同行层次中的矩阵行数，第二个元素用于描述不同层次中的矩阵列数。
