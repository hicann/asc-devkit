...
- 编写和调用VectorFunction（向量函数）。这些函数使用__simd_vf__标记，通过[asc\_vf\_call](./VF调用/asc_vf_call.md)被调用；
- 定义矢量数据寄存器[RegTensor](./寄存器数据类型/RegTensor.md)、掩码寄存器[MaskReg](./寄存器数据类型/MaskReg.md)；
- 编写for循环映射到硬件循环（[HardwareLoop](./关键特性说明/关键特性说明.md#硬件循环hardwareloop)）；[UpdateMask](./寄存器数据类型/MaskReg.md#mask设置方式)函数用于更新参与计算的mask，每次循环都会消耗一个VL长度的元素。
- 循环内调用Reg数据搬入接口连续对齐搬入（[LoadAlign](./Reg数据搬入/LoadAlign_continuous.md)）从UB中搬入单个VL长度数据；
- 循环内调用Reg计算接口Add完成单次Repeat计算；
- 循环内调用Reg数据搬出接口连续对齐搬出（[StoreAlign](./Reg数据搬出/StoreAlign_continuous.md)）往UB中搬出计算后的数据；
...
>[!NOTE]说明 
>- GM与UB间的数据搬运需通过[GM与UB数据搬运](../Memory矢量计算/数据搬运/数据搬运.md)完成；
>- Vector Function的流水类型为PIPE_V，Vector Function内部的同步依赖通过[LocalMemBar](./同步控制/LocalMemBar.md)接口插入。
