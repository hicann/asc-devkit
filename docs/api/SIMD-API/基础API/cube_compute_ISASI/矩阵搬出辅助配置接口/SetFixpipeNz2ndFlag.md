...
DataCopy数据搬运（[L0C Buffer-\>GM](../矩阵计算的搬出/DataCopy_L0CToGM.md)）过程中进行随路格式转换（NZ格式转换为ND格式）时，通过调用该接口设置格式转换的相关配置。
...
| 参数名称 | 输入/输出 | 含义 |
| ---------- | ---------- | ---------- |
| ndNum | 输入 | nd矩阵的数量，取值范围∈[1, 65535]。（[具体含义图示参见NZ2ND介绍特性章节](../矩阵搬出关键特性说明/NZ2ND.md)） |
