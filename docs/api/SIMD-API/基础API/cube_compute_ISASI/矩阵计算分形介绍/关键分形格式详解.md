...
- **格式转换过程：**（K, N）大小的矩阵被分为K1 \* N1个分形，按照row major（行主序）排布，形状如Z字形；每个分形内部有K0 \* N0个元素，按照column major（列主序）排布，形状如N字形，所以这种数据格式称为Zn格式。其中，（N0，K0）表示一个分形的大小，分形shape为16 x \(32B / sizeof\(Datatype\)\)，大小为512字节。
...
- **格式转换过程：** 通常使用[随路转换-ND2NZ搬运（DataCopy）](../矩阵计算的搬入/DataCopy_GMToL1_ND2NZ.md)指令来进行NHWC格式到NC1HWC0格式的转换。
