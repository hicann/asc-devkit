...
用于设置[LoadData（卷积数据搬运）](../矩阵计算的搬入/Load3D.md)接口的repeat参数。设置repeat参数后，可以通过调用一次Load3D接口完成多个迭代的数据搬运。
...
| 参数名称 | 输入/输出 | 含义 |
| --- | --- | --- |
| repeatParams | 输入 | 设置[LoadData（卷积数据搬运）](../矩阵计算的搬入/Load3D.md)接口的repeat参数，类型为LoadDataRepeatParam。<br>具体定义请参考：\$\{INSTALL_DIR\}/include/ascendc/basic_api/interface/kernel_struct_mm.h，\$\{INSTALL_DIR\}请替换为CANN软件安装后文件存储路径。<br>参数说明请参考[表2](#table15780447181917)。 |
