...
-   在SIMT函数定义时可使用[\_\_launch\_bounds\_\_\(\)](扩展语法/核函数配置-147.md)配置最大线程数，用于编译期底层对寄存器资源进行预分配。
...
2.  动态内存：位于静态内存之后，通过[<<<...\>\>\>](扩展语法/核函数配置-147.md#调用)中参数dyn\_ub\_size指定大小的动态内存空间，可通过以下方式申请使用：
