...
- 相关术语统一写作“核函数（Kernel）”；代码标识符、API名称和链接目标保持不变
- 每篇文档首次提及该存储单元时写作“Unified Buffer（UB）”，后续统一使用“UB”；API名称、文档名称、导航或特性清单、紧凑表项或表头、数据通路标签可按上下文使用“UB”，且不计入术语首次出现；代码标识符和链接目标保持不变
...
### Host侧与核函数（Kernel）侧明确分离

API文档中Host侧和核函数（Kernel）侧必须明确标注和分离：

- 每个API页面标注适用侧：Host侧/核函数（Kernel）侧/Host+核函数（Kernel）
- Host侧API（Tiling计算、workspace分配）与核函数（Kernel）侧API（计算接口、数据搬运）分节展示
- 编程指南中有Host/核函数（Kernel）分工全景图
...
| **Host/核函数（Kernel）分离** | 找到对应侧 | 侧别清晰 | 端到端链路 | 标签+分节+链路图 |
