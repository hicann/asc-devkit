...
下面示例使用`lanemask_lt`统计当前Lane之前的线程数，并据此判断当前线程位于Warp的上半区还是下半区，再通过`asc_shfl`分别广播上下半Warp中指定Lane的数据。示例中使用[asc_shfl](../Warp-Shfl类函数/asc_shfl.md)，需另外包含"simt_api/device_warp_functions.h"头文件，其中`srcLane`的取值范围为[0, 15]。
