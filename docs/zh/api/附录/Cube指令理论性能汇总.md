...
| --- | --- | --- | --- | --- | --- | --- |
| [Mmad](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算/Mmad.md) | int4b_t | int4b_t | 16 | 16 | 64 | 64 |
| Mmad | int8_t | int8_t | 16 | 16 | 32 | 32 |
| Mmad | half | half | 16 | 16 | 16 | 16 |
| Mmad | bfloat16_t | bfloat16_t | 16 | 16 | 16 | 16 |
| Mmad | float | float | 16 | 16 | 4 | 8 |
| Mmad（开启[HF32](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算关键特性说明/HF32.md)） | float | float | 16 | 16 | 8 | 8 |
| [MmadWithSparse](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算/MmadWithSparse.md) | int8_t | int8_t | 16 | 16 | 32 | 32 |
<!-- end id5 -->
...
| 接口 | 左矩阵A | 右矩阵B | cube<sub>m</sub> | cube<sub>n</sub> | cube<sub>k</sub> | k<sub>0</sub> |
| --- | --- | --- | --- | --- | --- | --- |
| [Mmad](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算/Mmad.md) | int8_t | int8_t | 16 | 16 | 32 | 32 |
...
| Mmad（开启[HF32](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算关键特性说明/HF32.md)） | float | float | 16 | 16 | 8 | 8 |
| [MmadMx](../SIMD-API/基础API/cube_compute_ISASI/Mmad计算/MmadMx.md) | fp4x2_e1m2_t | fp4x2_e1m2_t | 16 | 16 | 64 | 64 |
...
| --- | --- |
| [LoadData（2D矩阵搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_2D.md)（L1 Buffer->L0A Buffer） | 256 |
| LoadData（2D矩阵搬运）（L1 Buffer->L0B Buffer） | 128 |
| [LoadDataWithTranspose](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadDataWithTranspose.md)（L1 Buffer->L0A Buffer） | 256 |
| LoadDataWithTranspose（L1 Buffer->L0B Buffer） | 128 |
| [LoadDataWithSparse](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadDataWithSparse.md)（L1 Buffer->L0B Buffer） | 128 |
| [DataCopy（L1 Buffer->BiasTable Buffer）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/DataCopy_L1ToBiasTable.md) | 32 |
| [DataCopy（L1 Buffer->Fixpipe Buffer）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/DataCopy_L1ToFixpipe.md) | 32 |

[LoadData（卷积数据搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_3D.md)指令的理论性能与参数相关，相关参数说明见[LoadData3DParamsV2结构体内参数说明](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_3D.md#zh-cn_topic_0000002512171652_table193501032193419)，理论性能说明见下表：
...
| 接口 | 搬运带宽（单位：Byte/cycle） |
| --- | --- |
| [LoadData（2D矩阵搬运V2）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_2D_V2.md)（L1 Buffer->L0A Buffer） | 256 |
| LoadData（2D矩阵搬运V2）（L1 Buffer->L0B Buffer） | 256 |
| [LoadData（MX矩阵搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_2D_MX.md)（L1 Buffer->L0A Buffer） | 256 |
...
针对Ascend 950PR/Ascend 950DT，[LoadData（卷积数据搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_3D.md)和[LoadDataWithStride](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadDataWithStride.md)指令的理论性能与参数相关，相关参数说明见[LoadData3DParamsV2结构体内参数说明](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_3D.md#zh-cn_topic_0000002512171652_table193501032193419)。此外，LoadDataWithStride接口相比于LoadData（卷积数据搬运）接口，内部不包含针对其他芯片版本的兼容性实现，减少了兼容造成的额外开销，性能表现有所优化。两个接口的理论性能说明见下表：
...
矩阵计算搬出类指令及其随路功能的详细介绍可参考[DataCopy（L0C到GM数据搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬出/DataCopy_L0CToGM.md)、[Fixpipe（L0C到GM数据搬运）](../SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬出/Fixpipe_L0CToGM.md)以及相同目录下的其他通路对应资料。以下为理论性能说明列表：
...
| L0C Buffer->Unified Buffer（开启[双目标模式](../SIMD-API/基础API/cube_compute_ISASI/矩阵搬出关键特性说明/L0C-Buffer到UB双目标模式.md)） | float | NoQuant | NZ2NZ | float | 32 + 32 | 128 + 128 |
...
注：srcNzC0Stride为启用NZ2DN功能需配置的参数之一，参数详细介绍请参考[NZ2DN关键特性说明](../SIMD-API/基础API/cube_compute_ISASI/矩阵搬出关键特性说明/NZ2DN.md)。
<!-- end id10 -->
<!-- end id1 -->
