...
-   Tiling下沉场景，日志不会输出到plog中，而是需要落盘并进行解析。算子运行之前需要开启Dump功能，使得日志Dump功能生效。如何开启Dump功能依赖于具体的网络运行方式。以TorchAir图模式为例，需要配置enable\_dump、dump\_path、dump\_mode等Dump参数。详细说明可参考[算子data dump功能](https://www.hiascend.com/document/detail/zh/Pytorch/2610/devguide/TorchAir/docs/zh/ascend_ir/features/advanced/data_dump.md)。示例如下：
