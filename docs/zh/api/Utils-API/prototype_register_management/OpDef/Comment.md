...
基于OpDef算子原型定义，自定义算子工程可以实现如下自动化能力：自动生成图模式场景使用的算子原型头文件，开发者可以使用生成的算子原型进行构图、图编译、图执行等操作。
...
具体参考[算子工程编译时出现文件名过长报错](../../../../guide/programming_guide/appendix/faq/filename_too_long_error.md)。
