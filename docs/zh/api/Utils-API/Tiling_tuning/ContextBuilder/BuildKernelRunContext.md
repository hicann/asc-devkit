...
构造KernelRunContext并返回[KernelRunContextHolder](KernelRunContextHolder_struct_definition.md)的智能指针，该对象可通过GetContext接口获取KernelContext类型的对象。
...
KernelRunContextHolder的共享指针，可通过`GetContext<gert::KernelContext>()`函数获取KernelContext对象。
