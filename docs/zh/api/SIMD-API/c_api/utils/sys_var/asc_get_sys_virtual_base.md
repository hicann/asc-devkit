...
读取系统虚拟基地址特殊寄存器的值并按`int64_t`返回，该寄存器为只读特殊寄存器，存储当前核上下文的系统虚拟基地址，由系统在核函数（Kernel）启动前按核配置完成。系统虚拟基地址用于在核函数（Kernel）内将相对偏移量转换为系统虚拟地址，常用于地址推导、跨核地址对齐等需要系统虚拟基地址参与的场景。
