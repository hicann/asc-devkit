...
`int64_t`类型，表示`value`二进制表示中值为0的位数。取值范围为[0, 64]。
...
__global__ __vector__ void asc_zero_bits_cnt_kernel(__gm__ int64_t* output)
{
    asc_init();
    output[0] = asc_zero_bits_cnt(15);
    asc_sync();
}
} // namespace
...
int main()
{
    ...
    asc_zero_bits_cnt_kernel<<<1, 0>>>(output_device);
    ...
    std::cout << (passed ? "[Success] asc_zero_bits_cnt passed." : "[Failed] asc_zero_bits_cnt failed.") << std::endl;
