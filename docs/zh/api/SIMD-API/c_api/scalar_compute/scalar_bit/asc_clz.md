...
计算一个`uint64_t`类型整数在二进制表示下的前导零个数，即从二进制最高位开始，到第一个出现二进制1为止，中间连续的0的数量。
...
| 参数名 | 输入/输出 | 描述 |
| ------------ | ------------ | ----------- |
| value_in | 输入 | 待统计的64位无符号整数。取值范围为[0, 2^64-1]。 |

## 返回值说明

返回`value_in`的前导零个数，取值范围为[0, 64]。当`value_in`为0时，返回64。
...
__global__ __vector__ void asc_clz_kernel(__gm__ int64_t* output)
{
    asc_init();
    output[0] = asc_clz(1);
    asc_sync();
...
int main()
{
    ...
    asc_clz_kernel<<<1, 0>>>(output_device);
    ...
    std::cout << (passed ? "[Success] asc_clz passed." : "[Failed] asc_clz failed.") << std::endl;
