...
- 转换示例：从ND（GM）到Nz（L1 Buffer）的数据格式转换见下图3，图中展示了一个具体的M=40，K=56的矩阵（数据类型为half），在从Global Memory（GM）搬运至L1 Buffer并转换为Nz格式时的内存排布变化：
    - 原始数据（ND格式）：
...
<!-- npu="950" id5 -->
- 转换示例：针对Ascend 950PR/Ascend 950DT，可以使用[矩阵搬入数据搬运 DN2Nz接口](../../cube_datamove/asc_copy_gm2l1_dn2nz.md)将其转换为Nz格式以适配Cube单元。
<!-- end id5 -->
