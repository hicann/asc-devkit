...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| config | 输入 | NDDMA高维切分搬运的填充元素数配置，类型为`asc_ndim_pad_count_config&`。详细字段说明请参见[asc_ndim_pad_count_config](../defs/union/asc_ndim_pad_count_config.md)。 |
...
- 调用`asc_ndim_copy_gm2ub`前，必须先调用本接口完成`loop1`至`loop4`各维的填充元素数配置。
- 各维的填充元素数配置会持续生效，直至重新配置。若后续搬运任务使用不同的填充元素数，应在调用`asc_ndim_copy_gm2ub`前调用本接口重新配置。
- 使用常数填充模式时，还需通过[asc_set_ndim_pad_value](asc_set_ndim_pad_value.md)设置与搬运数据类型一致的填充值。
...
__global__ __vector__ void asc_set_ndim_pad_count_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    asc_set_ndim_pad_count_kernel<<<1, 0>>>(output_device, input_device);
    ...
    std::cout << (passed ? "[Success] asc_set_ndim_pad_count passed." : "[Failed] asc_set_ndim_pad_count failed.") << std::endl;
