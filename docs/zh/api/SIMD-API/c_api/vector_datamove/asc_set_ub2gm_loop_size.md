...
本接口用于设置[asc_copy_ub2gm_align](./asc_copy_ub2gm_align/asc_copy_ub2gm_align.md)使用循环搬运模式时的两层循环迭代次数。`loop1_size`用于设置内层循环`loop1`的迭代次数，`loop2_size`用于设置外层循环`loop2`的迭代次数。

使用循环搬运模式时，还需通过[asc_set_ub2gm_loop1_stride](./asc_set_ub2gm_loop1_stride.md)和[asc_set_ub2gm_loop2_stride](./asc_set_ub2gm_loop2_stride.md)分别设置内层循环和外层循环中，相邻两次迭代源操作数数据块起始地址之间的偏移量，以及目的操作数数据块起始地址之间的偏移量。
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| loop1_size | 输入 | 内层循环`loop1`的迭代次数。取值范围为[0, $2^{21}−1$]。 |
| loop2_size | 输入 | 外层循环`loop2`的迭代次数。取值范围为[0, $2^{21}−1$]。 |
...
- 调用`asc_copy_ub2gm_align`前，必须通过本接口、`asc_set_ub2gm_loop1_stride`和`asc_set_ub2gm_loop2_stride`完成循环次数及两层循环步长的配置。
- 循环次数配置会持续生效，直至重新配置。若后续搬运任务使用不同的循环次数，应在调用`asc_copy_ub2gm_align`前调用本接口重新配置。
- `loop1_size`或`loop2_size`设置为0时，对应循环不执行，后续的`asc_copy_ub2gm_align`调用不搬运数据。
- 完成当前循环搬运任务后，需再次调用本接口将`loop1_size`和`loop2_size`均设置为1，复位循环配置，否则可能影响下一次搬运。
