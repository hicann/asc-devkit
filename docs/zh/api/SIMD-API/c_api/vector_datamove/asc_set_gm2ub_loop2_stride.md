...
本接口用于设置[asc_copy_gm2ub_align](./asc_copy_gm2ub_align/asc_copy_gm2ub_align.md)使用循环搬运模式时，外层循环中相邻两次迭代源操作数数据块起始地址之间的偏移量，以及目的操作数数据块起始地址之间的偏移量。

使用循环搬运模式时，还需通过[asc_set_gm2ub_loop_size](./asc_set_gm2ub_loop_size.md)设置循环次数，并通过[asc_set_gm2ub_loop1_stride](./asc_set_gm2ub_loop1_stride.md)设置内层循环`loop1`的步长。
...
```c
__aicore__ inline void asc_set_gm2ub_loop2_stride(uint64_t loop2_src_stride,
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| loop2_src_stride | 输入 | 外层循环中相邻迭代源操作数的数据块间隔，单位为字节，取值范围为[0, $2^{40}−1$]。 |
| loop2_dst_stride | 输入 | 外层循环中相邻迭代目的操作数的数据块间隔，单位为字节，取值范围为[0, $2^{24}−1$]，且必须32字节对齐。 |
...
- 本接口在非AIV上调用直接返回。
- 调用`asc_copy_gm2ub_align`前，必须通过本接口、`asc_set_gm2ub_loop_size`和`asc_set_gm2ub_loop1_stride`完成循环次数及两层循环步长的配置。
- `loop2`步长配置会持续生效，直至重新配置。若后续搬运任务使用不同的`loop2`步长，应在调用`asc_copy_gm2ub_align`前调用本接口重新配置。
- `loop2_src_stride`必须32字节对齐。
- 完成当前循环搬运任务后，需调用`asc_set_gm2ub_loop_size(1, 1)`将两层循环次数复位为1，否则可能影响下一次搬运。
...
__global__ __vector__ void asc_set_gm2ub_loop2_stride_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
    __ubuf__ uint8_t local[OUTPUT_BYTES];
    asc_set_gm2ub_loop_size(2, 2);
    asc_set_gm2ub_loop1_stride(64, 32);
    asc_set_gm2ub_loop2_stride(128, 64);
    asc_copy_gm2ub_align(local, input, 1, 32, 0, 0, false,
        asc_load_l2_cache_mode::NORMAL_FIRST_VICTIM, 0, 0);
...
int main()
{
    ...
    asc_set_gm2ub_loop2_stride_kernel<<<1, 0>>>(output_device, input_device);
    ...
    std::cout << (passed ? "[Success] asc_set_gm2ub_loop2_stride passed." : "[Failed] asc_set_gm2ub_loop2_stride failed.") << std::endl;
