...
根据mask对源操作数src执行左移，左移的位数由输入参数value决定，将结果写入目的操作数dst。根据源操作数的数据类型，左移操作分为以下两种情况：
...
- 对于逻辑位移（无符号数据类型），如果位移量（value）大于数据类型位宽，则输出为0。
- 对于算术位移（有符号数据类型），如果位移量（value）大于数据类型位宽，则输出为0。
- value不支持设置为负，负数行为未定义。
