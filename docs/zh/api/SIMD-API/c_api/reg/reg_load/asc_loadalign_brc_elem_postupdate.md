...
对齐数据搬运接口，从Unified Buffer（UB）连续对齐搬入目的操作数，实现BRC搬入模式并启用Post Update：搬运一个b8/b16/b32类型的数据并Broadcast到所有元素位置，接口调用后自动更新源操作数地址。
...
vector_half dst;
__ubuf__ half* src;
asc_loadalign_brc_elem_postupdate(dst, src, 0);
```
