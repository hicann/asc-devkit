...
从Unified Buffer（UB）中32字节对齐的起始地址读取一个`DataBlock`（32字节），并将该`DataBlock`广播到整个矢量数据寄存器。搬运过程中数据格式和内容保持不变。接口采用Post Update模式，搬运完成后自动更新源地址指针。
...
```c
__simd_callee__ inline void asc_loadalign_brc_datablock_postupdate(vector_<dtype>& dst,
                                                                   __ubuf__ <dtype>*& src,
                                                                   int32_t offset)
```
...
```c
// 示例：int8_t类型。
__simd_callee__ inline void asc_loadalign_brc_datablock_postupdate(vector_int8_t& dst,
                                                                   __ubuf__ int8_t*& src,
                                                                   int32_t offset)
```
...
__simd_vf__ inline void asc_loadalign_brc_datablock_postupdate_vf(__ubuf__ uint8_t* output,
                                                                  ...
                                                                  uint16_t one_repeat_size, uint16_t repeat_time)
{
    ...
    for (uint16_t i = 0; i < repeat_time; ++i) {
        asc_loadalign_brc_datablock_postupdate(dst_reg, src, 32);
        asc_storealign(output + i * one_repeat_size, dst_reg, mask);
    }
}

__global__ __vector__ void asc_loadalign_brc_datablock_postupdate_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
    ...
    asc_vf_call<asc_loadalign_brc_datablock_postupdate_vf>(output_local, input_local, one_repeat_size, repeat_time);
...
int main()
{
    ...
    for (uint16_t i = 0; i < OUTPUT_ELEMENTS; ++i) {
        const uint32_t source_block = (i / 256) * 32;
        golden[i] = input[source_block + i % 32];
    }
    ...
    asc_loadalign_brc_datablock_postupdate_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), OUTPUT_BYTES, output_device, OUTPUT_BYTES, ACL_MEMCPY_DEVICE_TO_HOST);

    const bool passed = output == golden;
    std::cout << (passed ? "[Success]" : "[Failed]") << " asc_loadalign_brc_datablock_postupdate example." << std::endl;
