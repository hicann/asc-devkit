...
从Unified Buffer（UB）中32字节对齐的起始地址读取VL/4长度的连续数据，在每个源元素后补3个值为0的元素，并将得到的VL长度数据通过函数返回值返回或写入目的矢量数据寄存器。本接口提供三种功能模式：
...
```c
// 通过函数返回值返回结果。
__simd_callee__ inline vector_<dtype> asc_loadalign_unpack4(__ubuf__ <dtype>* src)

// 通过引用参数输出结果。
__simd_callee__ inline void asc_loadalign_unpack4(vector_<dtype>& dst,
                                                  __ubuf__ <dtype>* src)
```

#### dtype支持数据类型

dtype支持的数据类型为`int4b_t`、`int8_t`、`uint8_t`、`fp4x2_e2m1_t`、`fp4x2_e1m2_t`、`hifloat8_t`、`fp8_e8m0_t`、`fp8_e5m2_t`、`fp8_e4m3fn_t`。当dtype为`int4b_t`时，dst的实际类型为`vector_int4x2_t`。
...
__simd_callee__ inline void asc_loadalign_unpack4(vector_int8_t& dst,
                                                  __ubuf__ int8_t* src)
```
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| dst | 输出 | 目的矢量数据寄存器。仅无返回值类型接口包含该参数。dtype必须与`src`一致，每个源元素后补3个值为0的元素，搬入VL长度数据。 |
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| dst | 输出 | 目的矢量数据寄存器。dtype必须与`src`一致，每个源元素后补3个值为0的元素，搬入VL长度数据。 |
| src | 输入 | 源UB地址，实际读取地址必须按32字节对齐。 |
...
对于返回值类型接口，返回保存解包搬入结果的矢量数据寄存器，数据类型与`src`保持一致。
...
constexpr uint32_t INPUT_ELEMENTS = 128;
...
__simd_vf__ inline void asc_loadalign_unpack4_vf(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input,
                                                 uint16_t one_repeat_size, uint16_t repeat_time)
{
    vector_bool mask = asc_create_mask_b8(PAT_ALL);
    vector_uint8_t dst_reg;
    for (uint16_t i = 0; i < repeat_time; ++i) {
        asc_loadalign_unpack4(dst_reg, input + i * 64);
        asc_storealign(output + i * one_repeat_size, dst_reg, mask);
    }
}

__global__ __vector__ void asc_loadalign_unpack4_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
    ...
    asc_vf_call<asc_loadalign_unpack4_vf>(output_local, input_local, one_repeat_size, repeat_time);
...
int main()
{
    ...
    for (uint16_t i = 0; i < OUTPUT_ELEMENTS; ++i) golden[i] = (i % 4 == 0) ? input[i / 4] : 0;
    ...
    asc_loadalign_unpack4_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), OUTPUT_BYTES, output_device, OUTPUT_BYTES, ACL_MEMCPY_DEVICE_TO_HOST);

    const bool passed = output == golden;
    std::cout << (passed ? "[Success]" : "[Failed]") << " asc_loadalign_unpack4 example." << std::endl;
