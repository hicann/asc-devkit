...
根据掩码对源操作数矢量数据寄存器中的元素执行左移，左移位数由矢量数据寄存器`shift`中对应元素指定，并将结果通过函数返回值返回或写入目的操作数。掩码对应位置为1的元素参与计算，为0的元素在输出结果中置零。计算公式如下：
...
- **无符号整数：执行逻辑左移。** 逻辑左移会将二进制数整体向左移动指定的位数，最高位被丢弃，最低位用0填充。例如，二进制数1010101010101010（`uint16_t`类型）逻辑左移1位后，结果为0101010101010100。
- **有符号整数：执行算术左移。** 算术左移会将二进制数整体向左移动指定的位数，最高位被丢弃（符号位会被丢弃），最低位用0填充。例如，二进制数1010101010101010（`int16_t`类型）算术左移1位后，结果为0101010101010100；算术左移3位后，结果为0101010101010000。
...
```c
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_<dtype0> asc_shiftleft(vector_<dtype0> src,
                                                     vector_<dtype1> shift,
                                                     vector_bool mask)

// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_shiftleft(vector_<dtype0>& dst,
                                          vector_<dtype0> src,
                                          vector_<dtype1> shift,
                                          vector_bool mask)
```
...
```c
// 通过函数返回值返回结果
__simd_callee__ inline vector_uint16_t asc_shiftleft(vector_uint16_t src,
...
| mask | 输入 | 源操作数掩码（掩码寄存器），用于指示参与计算的元素。对应位置为1时参与计算，为0时不参与计算且输出结果对应元素置零。 |
...
- 当位移量大于`src`的数据类型位宽时，输出结果中的有效元素置零。
...
__simd_vf__ inline void shiftleft_vf(__ubuf__ uint16_t* output,
                                     ...
                                     __ubuf__ int16_t* shift)
{
    ...
    vector_uint16_t dst = asc_shiftleft(src, shift_reg, mask);
    asc_storealign(output, dst, mask);
}

__global__ __vector__ void asc_shiftleft_kernel(__gm__ uint16_t* output,
                                                ...
                                                __gm__ int16_t* shift)
{
    ...
    shiftleft_vf(output_local, input_local, shift_local);
...
int main()
{
    ...
    asc_shiftleft_kernel<<<1, 0>>>(output_device, input_device, shift_device);
    ...
    std::cout << (passed ? "[Success] asc_shiftleft completed."
                         : "[Failed] asc_shiftleft output mismatch.")
              << std::endl;
