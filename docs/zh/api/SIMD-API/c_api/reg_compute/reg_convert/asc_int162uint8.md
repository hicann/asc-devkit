...
`src`中int16_t类型的元素转换为uint8_t类型，并将结果写入dst。支持非饱和与饱和两种模式。
...
def asc_int162uint8(dst, src, mask, dst_pos):
    offset = 0 if dst_pos == ASC_POSITION_EVEN else 1
    for i in range(VL // 2):
        dst[2 * i] = 0
        dst[2 * i + 1] = 0
        if mask[i]:
            dst[2 * i + offset] = uint8_t(src[i])  # int16_t -> uint8_t
```
...
```c
// 占位符形式
__simd_callee__ inline void asc_int162uint8<sat_mode>(vector_uint8_t& dst,
                                                      vector_int16_t src,
                                                      vector_bool mask,
...
```c
// 非饱和模式，数据写入索引为偶数的位置
__simd_callee__ inline void asc_int162uint8(vector_uint8_t& dst,
                                            vector_int16_t src,
                                            vector_bool mask,
                                            std::integral_constant<asc_position_mode, asc_position_mode::EVEN> dst_pos)
```
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    vector_int16_t src_0;
    asc_loadalign(dst_0, reinterpret_cast<__ubuf__ uint8_t*>(output));
    asc_loadalign(src_0, reinterpret_cast<__ubuf__ int16_t*>(input));
    asc_int162uint8_sat(dst_0, src_0, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ uint8_t*>(output), dst_0, mask);
    vector_uint8_t dst_1;
    vector_int16_t src_1;
    asc_loadalign(dst_1, reinterpret_cast<__ubuf__ uint8_t*>(output));
    asc_loadalign(src_1, reinterpret_cast<__ubuf__ int16_t*>(input));
    asc_int162uint8(dst_1, src_1, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ uint8_t*>(output), dst_1, mask);
}
__global__ __vector__ void asc_int162uint8_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    asc_int162uint8_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_int16_t -> vector_uint8_t" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_int162uint8/asc_int162uint8_sat/ASC_POSITION_EVEN passed."
                          : "[Failed] asc_int162uint8/asc_int162uint8_sat/ASC_POSITION_EVEN failed.")
              << std::endl;
