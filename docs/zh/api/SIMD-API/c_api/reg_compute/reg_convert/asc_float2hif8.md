...
根据`mask`将`src`中参与计算的`float`类型元素转换为`hifloat8_t`类型，结果写入`dst`。由于源操作数与目的操作数的类型位宽比为4:1，`dst`中的元素按索引分为四组，参数`dst_quarter_pos`从`dst`中离散选取每隔4个元素的索引位置写入转换结果，具体为索引0、4、8、…，索引1、5、9、…，索引2、6、10、…或索引3、7、11、…，其他位置置零。
...
// 占位符形式
// 通过引用参数输出结果
__simd_callee__ inline void asc_float2hif8_<round_mode><sat_mode>(vector_hifloat8_t& dst,
                                     vector_float src,
                                     vector_bool mask,
                                     std::integral_constant<asc_position_quarter_mode, <quarter_pos>> dst_quarter_pos)

// 通过函数返回值返回结果
__simd_callee__ inline vector_hifloat8_t asc_float2hif8_<round_mode><sat_mode>(vector_float src,
                                     vector_bool mask,
                                     std::integral_constant<asc_position_quarter_mode, <quarter_pos>> dst_quarter_pos)
```

**占位符说明如下：**
- `<round_mode>`支持`rna`（ROUND）和`rh`（HIFLOAT8专用舍入模式）。
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    asc_float2hif8_rna(dst, src, mask, ASC_DISPERSE_FIRST_QUARTER);
    asc_storealign(reinterpret_cast<__ubuf__ hifloat8_t*>(output), dst, mask);
}

__global__ __vector__ void asc_float2hif8_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    asc_float2hif8_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), BUFFER_BYTES, output_device, BUFFER_BYTES, ACL_MEMCPY_DEVICE_TO_HOST);
    const bool passed = output == golden;
    std::cout << (passed ? "[Success] asc_float2hif8_rna passed." : "[Failed] asc_float2hif8_rna failed.") << std::endl;
