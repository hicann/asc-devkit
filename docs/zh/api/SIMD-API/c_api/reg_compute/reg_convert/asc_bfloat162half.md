...
```c
// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_bfloat162half<round_mode><sat_mode>(vector_half& dst,
                                                                    vector_bfloat16_t src,
                                                                    vector_bool mask)

// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_half asc_bfloat162half<round_mode><sat_mode>(vector_bfloat16_t src,
                                                                           vector_bool mask)
```
