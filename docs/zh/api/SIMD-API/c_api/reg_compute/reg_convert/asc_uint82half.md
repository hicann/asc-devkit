...
def asc_uint82half(dst, src, mask, src_pos):
...
```c
// 通过引用参数输出结果
__simd_callee__ inline void asc_uint82half(vector_half& dst,
                                             vector_uint8_t src,
                                             vector_bool mask,
...
// 通过函数返回值返回结果
__simd_callee__ inline vector_half asc_uint82half(vector_uint8_t src,
                                             vector_bool mask,
                                             std::integral_constant<asc_position_mode, asc_position_mode::EVEN> src_pos)
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    asc_uint82half(dst, src, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ half*>(output), dst, mask);
}
__global__ __vector__ void asc_uint82half_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    asc_uint82half_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_uint8_t -> vector_half" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_uint82half passed." : "[Failed] asc_uint82half failed.") << std::endl;
