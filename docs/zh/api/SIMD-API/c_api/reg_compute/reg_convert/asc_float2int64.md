...
将`float`类型数据转换为`int64_t`类型，并支持多种舍入模式与饱和/非饱和模式。
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    vector_int64_t dst;
    vector_float src;
    asc_loadalign(dst, reinterpret_cast<__ubuf__ int64_t*>(output));
    asc_loadalign(src, reinterpret_cast<__ubuf__ float*>(input));
    asc_float2int64_rn(dst, src, mask, ASC_POSITION_EVEN);
    asc_store(reinterpret_cast<__ubuf__ int64_t*>(output), dst);
}

__global__ __vector__ void asc_float2int64_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    asc_float2int64_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_float -> vector_int64_t" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_float2int64_rn/ASC_POSITION_EVEN passed."
                         : "[Failed] asc_float2int64_rn/ASC_POSITION_EVEN failed.")
              << std::endl;
