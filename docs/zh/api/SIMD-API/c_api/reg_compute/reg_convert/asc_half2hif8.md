...
根据`mask`将`src`中的每个参与计算的`half`类型元素转换为`hifloat8_t`类型，结果写入`dst`。由于源操作数与目的操作数类型位宽比为2:1，写入数据时需要将一个`VL`大小的数据分为两部分，根据参数`dst_pos`选择写入`dst`索引为偶数的位置或奇数的位置。
...
```c
// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_half2hif8_<round_mode><sat_mode>(vector_hifloat8_t& dst,
                                                                vector_half src,
                                                                vector_bool mask,
...
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_hifloat8_t asc_half2hif8_<round_mode><sat_mode>(vector_half src,
                                                                             vector_bool mask,
                                                                             std::integral_constant<asc_position_mode, asc_position_mode::EVEN> dst_pos)
...
**占位符说明如下：**
- `<round_mode>`支持`rna`（ROUND）和`rh`（HYBRID）。
...
```c
// 通过引用参数输出结果
__simd_callee__ inline void asc_half2hif8_rna(vector_hifloat8_t& dst,
                                              vector_half src,
                                              vector_bool mask,
                                              std::integral_constant<asc_position_mode, asc_position_mode::EVEN> dst_pos)

// 通过函数返回值返回结果
__simd_callee__ inline vector_hifloat8_t asc_half2hif8_rna(vector_half src,
                                                           vector_bool mask,
                                                           std::integral_constant<asc_position_mode, asc_position_mode::EVEN> dst_pos)
```
...
| dst_pos | 输入 | 位置选择标签（编译期标签分发，通过编译期重载选择对应实现），类型为std::integral_constant<asc_position_mode, asc_position_mode::EVEN>或std::integral_constant<asc_position_mode, asc_position_mode::ODD>。取`ASC_POSITION_EVEN`时选择将结果写入目的操作数索引为偶数的位置，其他位置清零；取`ASC_POSITION_ODD`时选择将结果写入目的操作数索引为奇数的位置，其他位置清零。 |
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    vector_half src;
    asc_loadalign(dst, reinterpret_cast<__ubuf__ hifloat8_t*>(output));
    asc_loadalign(src, reinterpret_cast<__ubuf__ half*>(input));
    asc_half2hif8_rna(dst, src, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ hifloat8_t*>(output), dst, mask);
}

__global__ __vector__ void asc_half2hif8_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    constexpr uint8_t golden_pattern[] = {0x08, 0x00};
    ...
    asc_half2hif8_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_half -> vector_hifloat8_t" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_half2hif8_rna passed." : "[Failed] asc_half2hif8_rna failed.") << std::endl;
