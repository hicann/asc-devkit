...
将`src`中的浮点数元素按照ROUND（四舍五入）舍入模式舍入到整数值，结果仍保持原浮点数据类型。接口支持通过函数返回值返回结果，也支持通过引用和参数输出结果：前者直接返回舍入结果，后者将舍入结果写入`dst`。舍入规则等价于C标准库`round`语义：舍入到最接近的整数；当输入值恰好位于两个整数中间时，向远离0的方向舍入（如1.4 -> 1.0、1.5 -> 2.0、-1.5 -> -2.0），整数值保持不变。未被`mask`筛选的元素置零。
...
```cpp
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_<dtype> asc_round(vector_<dtype> src,
                                                 vector_bool mask)

// 通过引用和参数输出结果（占位符形式）
__simd_callee__ inline void asc_round(vector_<dtype>& dst,
                                      vector_<dtype> src,
                                      vector_bool mask)
```
...
```cpp
// 通过函数返回值返回结果
__simd_callee__ inline vector_half asc_round(vector_half src,
                                              vector_bool mask)

// 通过引用和参数输出结果
__simd_callee__ inline void asc_round(vector_half& dst,
                                      vector_half src,
                                      vector_bool mask)
```
...
__simd_vf__ inline void round_vf(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    vector_bool mask = asc_create_mask_b16(PAT_ALL);
    vector_half src;
    asc_loadalign(src, reinterpret_cast<__ubuf__ half*>(input));
    vector_half dst = asc_round(src, mask);
    asc_storealign(reinterpret_cast<__ubuf__ half*>(output), dst, mask);
}

__global__ __vector__ void asc_round_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
    ...
    round_vf(output_local, input_local);
...
int main()
{
    ...
    constexpr uint8_t golden_pattern[] = {0x00, 0x42};
