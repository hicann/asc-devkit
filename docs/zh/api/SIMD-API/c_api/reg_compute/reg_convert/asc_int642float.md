...
def asc_int642float_rn(dst, src, mask, dst_pos):
    offset = 0 if dst_pos == ASC_POSITION_EVEN else 1
    for i in range(32):
        dst[2 * i] = 0
        dst[2 * i + 1] = 0
        if mask[i]:
            dst[2 * i + offset] = float(src[i])
```
...
```c
// 示例：向最近且偶数舍入（rn）偶数分块位置主原型，将`int64_t`矢量数据寄存器转换为`float`。
__simd_callee__ inline void asc_int642float_rn(vector_float& dst,
                                               vector_int64_t src,
                                               vector_bool mask,
                                               std::integral_constant<asc_position_mode, asc_position_mode::EVEN> dst_pos)
```
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    asc_int642float_rn(dst, src, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ float*>(output), dst, mask);
}
__global__ __vector__ void asc_int642float_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    ...
    constexpr uint8_t golden_pattern[] = {0x00, 0x00, 0x80, 0x3f, 0x00, 0x00, 0x00, 0x00};
    ...
    asc_int642float_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_int64_t -> vector_float" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_int642float_rn passed."
                          : "[Failed] asc_int642float_rn failed.")
              << std::endl;
