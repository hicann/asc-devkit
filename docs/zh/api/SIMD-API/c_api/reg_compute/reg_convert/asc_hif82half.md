...
`src`中hifloat8_t数据类型的元素转换为half数据类型，并将结果写入dst。
...
def asc_hif82half(dst, src, mask, src_pos):
    offset = 0 if src_pos == ASC_POSITION_EVEN else 1
    for i in range(VL // 2):
        dst[i] = 0
        if mask[2 * i + offset]:
            dst[i] = half(src[2 * i + offset])  # hifloat8_t -> half
```
...
```c
// 通过引用参数输出结果
__simd_callee__ inline void asc_hif82half(vector_half& dst,
...
// 通过函数返回值返回结果
__simd_callee__ inline vector_half asc_hif82half(vector_hifloat8_t src,
                                          vector_bool mask,
                                          std::integral_constant<asc_position_mode, asc_position_mode::EVEN> src_pos)
...
__simd_vf__ inline void convert(__ubuf__ uint8_t* output, __ubuf__ uint8_t* input)
{
    ...
    asc_loadalign(src, reinterpret_cast<__ubuf__ hifloat8_t*>(input));
    asc_hif82half(dst, src, mask, ASC_POSITION_EVEN);
    asc_storealign(reinterpret_cast<__ubuf__ half*>(output), dst, mask);
}
__global__ __vector__ void asc_hif82half_kernel(__gm__ uint8_t* output, __gm__ uint8_t* input)
{
    asc_init();
...
int main()
{
    constexpr uint8_t input_pattern[] = {0x08};
    ...
    asc_hif82half_kernel<<<1, 0>>>(output_device, input_device);
    aclrtSynchronizeDevice();
    aclrtMemcpy(output.data(), output.size() * sizeof(uint8_t), output_device, output.size() * sizeof(uint8_t),
        ACL_MEMCPY_DEVICE_TO_HOST);
    std::cout << "Conversion: vector_hifloat8_t -> vector_half" << std::endl;
    ...
    std::cout << (passed ? "[Success] asc_hif82half passed." : "[Failed] asc_hif82half failed.") << std::endl;
