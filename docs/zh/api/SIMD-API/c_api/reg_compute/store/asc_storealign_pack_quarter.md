...
将矢量数据寄存器中的数据压缩搬出到Unified Buffer（UB）。`mask`用于指示参与搬出的元素，`mask`对应位置为1时，将`src`中每个32bit的有效元素中低8bit数据写入压缩后对应的目的位置，`mask`对应位置为0时，压缩后对应的目的位置保持原值。目的操作数为UB地址。
...
- `mask`必须为`vector_bool`类型掩码寄存器，未赋值的掩码寄存器内容不确定，会导致有效元素位置错误。
