...
- `src`为矢量数据寄存器；无返回值原型中的`dst`为掩码寄存器。
...
__simd_vf__ inline void compare(__ubuf__ float* dst, __ubuf__ float* src)
{
    ...
    asc_lt_scalar(predicate, src_reg, COMPARE_VALUE, mask);
...
__global__ __vector__ void asc_lt_scalar_kernel(__gm__ float* dst, __gm__ float* src)
{
    asc_init();
...
int main()
{
    ...
    asc_lt_scalar_kernel<<<1, 0>>>(dst_device, src_device);
    ...
    std::cout << (passed ? "[Success] asc_lt_scalar passed." : "[Failed] asc_lt_scalar failed.") << std::endl;
