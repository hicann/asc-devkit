...
根据`mask`将源操作数`src`按元素与标量`value`进行比较，若$src_i > value$，则对应结果位为1，否则为0。每个元素的比较结果占一个比特。计算公式如下：
...
```c
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_bool asc_gt_scalar(vector_<dtype> src,
                                                  <dtype> value,
                                                  vector_bool mask)

// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_gt_scalar(vector_bool& dst,
                                          vector_<dtype> src,
                                          <dtype> value,
                                          vector_bool mask)
```
...
__simd_vf__ inline void compare(__ubuf__ float* dst, __ubuf__ float* src)
{
    ...
    asc_gt_scalar(predicate, src_reg, COMPARE_VALUE, mask);
...
__global__ __vector__ void asc_gt_scalar_kernel(__gm__ float* dst, __gm__ float* src)
{
    asc_init();
...
int main()
{
    ...
    asc_gt_scalar_kernel<<<1, 0>>>(dst_device, src_device);
    ...
    std::cout << (passed ? "[Success] asc_gt_scalar passed." : "[Failed] asc_gt_scalar failed.") << std::endl;
