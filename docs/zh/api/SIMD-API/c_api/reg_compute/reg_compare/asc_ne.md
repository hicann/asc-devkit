...
ne表示不等于（not equal），该接口用于逐元素比较两个源操作数大小，将比较结果（$src0_i \neq src1_i$）作为返回值返回或写入目的操作数`dst`对应比特位。如果比较结果为真，则对应比特位为1，否则为0。
...
```c
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_bool asc_ne(vector_<dtype> src0,
                                          vector_<dtype> src1,
                                          vector_bool mask)

// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_ne(vector_bool& dst,
                                   vector_<dtype> src0,
                                   vector_<dtype> src1,
                                   vector_bool mask)
```
...
```c
// 典型示例：half类型逐元素不等比较
__simd_callee__ inline vector_bool asc_ne(vector_half src0,
                                          vector_half src1,
                                          vector_bool mask)
...
- `src0`和`src1`为矢量数据寄存器；无返回值原型中的`dst`为掩码寄存器。
- `mask`比特位为0时，计算结果对应比特位写0。
- 浮点比较时，+0.0与-0.0视为相等。
- 浮点比较输入含nan时，计算结果对应比特位写1。
...
__simd_vf__ inline void compare(__ubuf__ float* dst, __ubuf__ float* src0, __ubuf__ float* src1)
{
    ...
    asc_ne(predicate, src0_reg, src1_reg, mask);
...
__global__ __vector__ void asc_ne_kernel(__gm__ float* dst, __gm__ float* src0, __gm__ float* src1)
{
    asc_init();
...
int main()
{
    ...
    aclrtMemcpy(src1_device, src1.size() * sizeof(float), src1.data(), src1.size() * sizeof(float),
        ACL_MEMCPY_HOST_TO_DEVICE);
    asc_ne_kernel<<<1, 0>>>(dst_device, src0_device, src1_device);
    ...
    std::cout << (passed ? "[Success] asc_ne passed." : "[Failed] asc_ne failed.") << std::endl;
