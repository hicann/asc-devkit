...
该接口根据`mask`，对源操作数`src0`、`src1`进行按元素求差操作，将结果写入目的操作数`dst`。

Carry flag（进位/借位标志）用于表示加法进位或者减法无借位。减法运算在硬件底层通过补码加法实现，该接口可以在`carry`（掩码寄存器）中标记每次补码加法是否产生进位，若`src0`，`~src1`，`33'b1`，按位相加后最高位有进位，在`carry`中对应位置每4bit的最低位写1，否则写0。

**[无借位输出减法](#无借位输出减法)**：单目的操作数，矢量数据寄存器写入减法结果，计算公式如下：

$$
dst_i = src0_i - src1_i
$$

**[带借位输出减法](#带借位输出减法)**：双目的操作数，硬件计算时按照32位无符号数处理，矢量数据寄存器写入补码加法和的低32位，目的掩码寄存器写入每个元素补码加法产生的进位输出，计算公式如下：
...
输出`carry`适用场景请参考[asc_subc功能说明](asc_subc.md#功能说明)。
...
```c
// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_sub(vector_<dtype>& dst,
                                    vector_<dtype> src0,
                                    vector_<dtype> src1,
                                    vector_bool mask)

// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_<dtype> asc_sub(vector_<dtype> src0,
                                              vector_<dtype> src1,
                                              vector_bool mask)
```
...
```c
// 通过引用参数输出结果示例：float类型的无借位输出减法
__simd_callee__ inline void asc_sub(vector_float& dst,
                                    vector_float src0,
                                    vector_float src1,
                                    vector_bool mask)

// 通过函数返回值返回结果示例：float类型的无借位输出减法
__simd_callee__ inline vector_float asc_sub(vector_float src0,
                                            vector_float src1,
                                            vector_bool mask)
```
...
```c
__simd_callee__ inline void asc_sub(vector_bool& carry,
                                    vector_<dtype>& dst,
                                    vector_<dtype> src0,
                                    vector_<dtype> src1,
                                    vector_bool mask)
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| dst | 输出 | 目的操作数（矢量数据寄存器），用于存储减法结果，仅适用于通过引用参数输出结果的函数原型。 |
...
| 参数名 | 输入/输出 | 描述 |
|---|---|---|
| carry | 输出 | 目的操作数（掩码寄存器），用于存储减法借位输出。减法不借位等价于补码加法进位。 |
| dst | 输出 | 目的操作数（矢量数据寄存器），用于存储计算结果的低32位。 |
| src0 | 输入 | 源操作数（矢量数据寄存器）。 |
| src1 | 输入 | 源操作数（矢量数据寄存器）。 |
| mask | 输入 | 源操作数掩码（掩码寄存器），用于指示在计算过程中哪些元素参与计算。对应位置为1时参与计算，为0时不参与计算。mask未筛选的元素在输出中置零。 |
...
- 通过引用参数输出结果的函数原型无返回值。
- 通过函数返回值返回结果的函数原型返回无借位减法结果，返回值类型与源操作数类型一致。
- 带借位输出减法仅支持通过引用参数输出结果，无返回值。
...
- 整型dtype减法溢出时按环绕（wrap-around）策略处理：有符号类型溢出回绕到对应dtype的最小值或最大值（MIN-1->MAX，MAX+1->MIN），无符号类型下溢回绕到对应dtype的最大值（0-1->UMAX）。
- 浮点dtype（half、bfloat16_t、float）减法按IEEE 754浮点减法语义执行，溢出与无效输入（如nan、inf）的处理遵循浮点运算规则。

### 带借位输出减法约束

- 仅支持`uint32_t`类型。
- 运算输出完整计算结果（包含无借位标志），不受[asc_set_ctrl](../../spr/asc_set_ctrl.md)影响，硬件不会对输出进行饱和或截断。
- 差值截断为32位：`dst`写入`(src0_i + ~src1_i + 0b1) & 0xFFFFFFFF`，超出32位的部分不保留。
...
__simd_vf__ inline void sub_vf(__ubuf__ float* dst, __ubuf__ float* src0, __ubuf__ float* src1)
{
...
__global__ __vector__ void asc_sub_kernel(__gm__ float* dst, __gm__ float* src0, __gm__ float* src1)
{
    asc_init();
    ...
    sub_vf(dst_local, src0_local, src1_local);
...
int main()
{
    ...
    for (uint32_t i = 0; i < ELEMENT_COUNT; ++i) {
        src0[i] = static_cast<float>(i) * 0.5f;
        src1[i] = static_cast<float>(ELEMENT_COUNT - i) * 0.25f;
    ...
    asc_sub_kernel<<<1, 0>>>(dst_device, src0_device, src1_device);
    ...
    std::cout << (passed ? "[Success] asc_sub passed." : "[Failed] asc_sub failed.") << std::endl;
