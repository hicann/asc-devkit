...
该接口根据mask，对源操作数src0、src1进行按元素求最大值操作，将结果写入目的操作数dst。
...
```c
// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_max(vector_<dtype>& dst,
                                    vector_<dtype> src0,
                                    vector_<dtype> src1,
                                    vector_bool mask)

// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_<dtype> asc_max(vector_<dtype> src0,
                                              vector_<dtype> src1,
                                              vector_bool mask)
```
...
__simd_callee__ inline void asc_max(vector_float& dst,
                                    vector_float src0,
                                    vector_float src1,
                                    vector_bool mask)

// 通过函数返回值返回结果示例：float类型
__simd_callee__ inline vector_float asc_max(vector_float src0,
                                            vector_float src1,
                                            vector_bool mask)
```
...
- 通过引用参数输出结果的函数原型无返回值。
- 通过函数返回值返回结果的函数原型返回逐元素最大值计算结果，返回值类型与源操作数类型一致。
...
__simd_vf__ inline void max_vf(__ubuf__ float* dst, __ubuf__ float* src0, __ubuf__ float* src1)
{
    ...
    asc_max(dst_reg, src0_reg, src1_reg, mask);
    asc_storealign(dst, dst_reg, mask);
}

__global__ __vector__ void asc_max_kernel(__gm__ float* dst, __gm__ float* src0, __gm__ float* src1)
{
    asc_init();
    ...
    max_vf(dst_local, src0_local, src1_local);
...
int main()
{
    ...
    for (uint32_t i = 0; i < ELEMENT_COUNT; ++i) {
        ...
        golden[i] = std::max(src0[i], src1[i]);
    }
    ...
    asc_max_kernel<<<1, 0>>>(dst_device, src0_device, src1_device);
    ...
    std::cout << (passed ? "[Success] asc_max passed." : "[Failed] asc_max failed.") << std::endl;
