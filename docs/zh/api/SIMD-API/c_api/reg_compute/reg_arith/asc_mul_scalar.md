...
根据`mask`对源操作数`src`按元素与标量`value`相乘，得到计算结果。计算公式如下：

$$
dst_i = src_i \times value
$$
...
```c
// 通过函数返回值返回结果（占位符形式）
__simd_callee__ inline vector_<dtype> asc_mul_scalar(vector_<dtype> src,
                                                     <dtype> value,
                                                     vector_bool mask)

// 通过引用参数输出结果（占位符形式）
__simd_callee__ inline void asc_mul_scalar(vector_<dtype>& dst,
                                            vector_<dtype> src,
                                            <dtype> value,
                                            vector_bool mask)
```
...
```c
// 示例：对half矢量数据寄存器与half标量执行乘法运算
__simd_callee__ inline void asc_mul_scalar(vector_half& dst,
                                           vector_half src,
                                           half value,
                                           vector_bool mask)
```
...
__simd_vf__ inline void compute(__ubuf__ float* dst, __ubuf__ float* src)
{
    ...
    asc_mul_scalar(dst_reg, src_reg, SCALAR_VALUE, mask);
    asc_storealign(dst, dst_reg, mask);
}

__global__ __vector__ void asc_mul_scalar_kernel(__gm__ float* dst, __gm__ float* src)
{
    asc_init();
...
int main()
{
    ...
    asc_mul_scalar_kernel<<<1, 0>>>(dst_device, src_device);
    ...
    std::cout << (passed ? "[Success] asc_mul_scalar passed." : "[Failed] asc_mul_scalar failed.") << std::endl;
