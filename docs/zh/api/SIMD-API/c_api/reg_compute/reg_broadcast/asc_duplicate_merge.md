...
将源操作数`src`的最低位元素，即下标为0的元素，广播到目的操作数`dst`中被`mask`筛选的位置。`mask`仅筛选`dst`中的元素位置，不影响从`src`中读取的元素；`dst`中未被`mask`筛选的元素保留原值。

本接口与[asc_duplicate](asc_duplicate.md)的广播数据来源相同，区别是本接口采用合并模式保留`dst`中未参与计算的元素。
...
```cpp
// 占位符形式
__simd_callee__ inline void asc_duplicate_merge(vector_<dtype>& dst,
                                                vector_<dtype> src,
                                                vector_bool mask)
```
...
```cpp
__simd_callee__ inline void asc_duplicate_merge(vector_float& dst,
                                                vector_float src,
                                                vector_bool mask)
```
...
| mask | 输入 | 目的操作数元素操作的有效指示（掩码寄存器）。对应比特位为1时，dst中的元素写入src的最低位元素；为0时，dst中的元素保留原值。 |
...
- `mask`仅筛选`dst`中写入广播值的位置，不筛选`src`中的元素。无论`mask`的最低位是否有效，待广播的数据均为`src`的最低位元素。
...
以下示例将`src`的最低位元素广播到`dst`的前4个元素，`dst`中的其余元素保留原值。

```cpp
__simd_vf__ inline void duplicate_merge(__ubuf__ float* dst, __ubuf__ float* src)
{
    ...
    asc_duplicate_merge(dst_reg, src_reg, active_mask);
