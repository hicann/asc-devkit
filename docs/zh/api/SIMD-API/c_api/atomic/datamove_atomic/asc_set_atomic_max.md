...
原子取大过程：将待搬运到GM的数据和GM上已有数据进行比较，然后将两者中的最大值写入GM。本接口对后续目的地址为GM的数据搬运指令开启原子比较取大，不同产品支持的数据搬运通路请参考[约束说明](#约束说明)。

接口可选择不同的函数原型来设定不同的比较取大数据类型。
...
- 该接口执行前不会自动将GM上已有数据置零。若开发者期望在原子取大前GM上的原始数据为零，则需手动清零。
...
- 本接口与紧邻的后续搬运指令之间的同步由硬件保证，因此以下示例中插入的多流水同步是不必要的：
    ...
    /*
    asc_set_atomic_max_int8与asc_copy_ub2gm之间的同步由硬件保证，因此以下同步是不必要的。
...
__global__ __vector__ void asc_set_atomic_max_kernel(__gm__ float* output, __gm__ float* input0, __gm__ float* input1)
{
    asc_init();
...
int main()
{
    ...
    asc_set_atomic_max_kernel<<<1, 0>>>(output_device, input0_device, input1_device);
    ...
    std::cout << (passed ? "[Success] asc_set_atomic_max passed." : "[Failed] asc_set_atomic_max failed.") << std::endl;
