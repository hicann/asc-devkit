...
本接口用于配置3D img2col搬运模式下右矩阵的Feature Map属性描述参数，该配置供参数`f_matrix_ctrl`取值为`true`的[asc_copy_l12l0b（3D矩阵搬运）](asc_copy_l12l0b/asc_copy_l12l0b_3d_arch_3510.md)接口使用。
...
```cpp
// 逐参数形式
__aicore__ inline void asc_set_l13d_fmatrix_b(uint16_t fmatrix_w,
                                              uint16_t fmatrix_h,
                                              uint8_t pad_left,
                                              uint8_t pad_right,
                                              uint8_t pad_top,
...
__global__ __cube__ void asc_set_l13d_fmatrix_b_kernel(
...
int main()
{
    ...
    asc_set_l13d_fmatrix_b_kernel<<<1, 0>>>(fmap_device, weight_device, output_device);
