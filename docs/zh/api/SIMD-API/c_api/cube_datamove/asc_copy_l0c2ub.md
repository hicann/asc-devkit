...
| dual_dst_ctrl | 输入 | 双目标模式控制参数。当同一AI Core内有一个Cube Core和两个Vector Core时，如果启用双目标模式控制，L0C Buffer中的M×N矩阵将被分成两半，并同时分别写入两个Vector Core各自的UB中，其中前半部分写入SUB BLOCK0，后半部分写入SUB BLOCK1。参数取值说明如下：<br>&nbsp;&nbsp;&bull; `asc_dual_dst_mode::DUAL_DST_DISABLE`：单目标模式，将整个矩阵写入通过`sub_blockid`参数配置的目标UB。<br>&nbsp;&nbsp;&bull; `asc_dual_dst_mode::DUAL_DST_SPLIT_M`：双目标模式，按M维度拆分成形状为$M/2\times N$的两个矩阵，分别写入两个UB，M必须为2的倍数。<br>&nbsp;&nbsp;&bull; `asc_dual_dst_mode::DUAL_DST_SPLIT_N`：双目标模式，按N维度拆分成形状为$M\times N/2$的两个矩阵，分别写入两个UB，N须为32的倍数。<br>该参数仅支持普通搬运模式（Nz2Nz）或Nz2ND搬运场景，不支持其他随路功能场景。 |
    ...
    __gm__ int32_t* output)
{
    ...
    if ASC_IS_AIC {
        ...
        asc_copy_l0c2ub(output_ub, c_l0, N, M, N, M, 0, asc_dual_dst_mode::DUAL_DST_DISABLE,
