...
**`asc_fill_l0a_sync`接口已废弃，请使用[对应的非同步接口](../cube_datamove/asc_fill_l0a.md)和同步接口[asc_sync](../sync/intra_core_sync/asc_sync.md)替代。**
...
| value | 输入 | 源操作数（标量）。 |
| config | 输入 | 使用的初始化相关参数，详细说明请参考[asc_fill_value_config](../defs/union/asc_fill_value_config.md)。 |
...
config.dst_gap = 0;
asc_fill_l0a_sync(dst0, value0, config);    // 将dst0中256个float类型的元素初始化为1
...
config.dst_gap = 0;
asc_fill_l0a_sync(dst1, value1, config);    // 将dst1中256个half类型的元素初始化为1
```
