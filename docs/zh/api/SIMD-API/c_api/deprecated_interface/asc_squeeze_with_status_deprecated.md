...
本接口会将有效数据大小保存至AR寄存器，搬出时配合[asc_storeunalign_postupdate](../reg_compute/store/asc_storeunalign_postupdate.md)接口使用该值作为存储偏移量。AR寄存器中的数值可以通过[asc_get_squeeze_status](../spr/asc_get_squeeze_status.md)接口获取。
