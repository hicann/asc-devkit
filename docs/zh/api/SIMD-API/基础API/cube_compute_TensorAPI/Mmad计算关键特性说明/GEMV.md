...
通过[Copy](../矩阵计算的搬入/Copy_GMToL1.md)接口将输入矩阵A从Global Memory搬运到L1 Buffer上并保持ND格式不变，然后在L1 Buffer上将A矩阵转换为NZ格式，再通过[Copy](../矩阵计算的搬入/Copy_L1ToL0A.md)接口搬运到L0A Buffer，从而以GEMV模式执行矩阵乘。另外注意GEMV模式默认是关闭的，需要自定义[MmadTrait](../Mmad计算/Mmad.md)来开启。
