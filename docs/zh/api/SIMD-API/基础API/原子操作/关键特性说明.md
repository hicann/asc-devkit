...
搬运指令和开启原子操作的指令流水类型如下表所示，当上述指令在同一个核内执行时，开发者按需插入[单流水同步](../同步控制/核内同步/PipeBarrier_ISASI.md)或者[多流水同步](../同步控制/核内同步/SetFlag_WaitFlag_ISASI.md)，可参考[样例中scenarioNum=1的分支](#zh-cn_topic_0000002583421469_example1)。
...
| 算子类型 | 软件同步方案 | 说明 |
| --- | --- | --- |
| 纯Vector算子 | 方案1：通过多对[IBSet](../同步控制/核间同步/IBSet.md)和[IBWait](../同步控制/核间同步/IBWait.md)的接口组合，可实现多个AIV间的同步，可参考[样例中scenarioNum=2的分支](#zh-cn_topic_0000002583421469_example2)。| 方案1支持指定部分AIV参与同步，并可控制各AIV的执行顺序。|
| 纯Vector算子 | 方案2：通过[InitDetermineComputeWorkspace](../同步控制/核间同步/InitDetermineComputeWorkspace.md)、[NotifyNextBlock](../同步控制/核间同步/NotifyNextBlock.md)和[WaitPreBlock](../同步控制/核间同步/WaitPreBlock.md)三个接口配合使用，确保所有AIV核按照blockIdx升序执行。| 方案2要求所有AIV必须参与同步，且执行顺序固定为blockIdx升序。|
...
- 不通过DCache访问
    ...
    使用[WriteGmByPassDCache](../标量计算/WriteGmByPassDCache_ISASI.md)和[ReadGmByPassDCache](../标量计算/ReadGmByPassDCache_ISASI.md)。这种方式无需额外操作即可保证多核间数据的一致性。
