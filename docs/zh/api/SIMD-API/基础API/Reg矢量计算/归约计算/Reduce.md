...
| T | 目的操作数dstReg的数据类型。 |
| U | 源操作数srcReg的数据类型。 |
| mode | [MaskMergeMode](../辅助数据类型/MaskMergeMode.md)，选择MERGING模式或ZEROING模式。<br>&bull; ZEROING，mask未选中的元素在dst中置零。<br>&bull; MERGING，当前不支持。 |
...
MAX/MIN归约产生两个有效结果，需要分别提取。以下示例中dstAddr为32B对齐地址，如果需要写出到非对齐地址，请参考[连续非对齐搬出\(StoreUnAlign\)](../Reg数据搬出/StoreUnAlign_continuous.md)。
