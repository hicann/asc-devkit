...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| id | 输入 | 事件ID，由用户自己指定，推荐通过[AllocEventID](../../资源管理/TPipe/AllocEventID.md)或者[FetchEventID](../../资源管理/TPipe/FetchEventID.md)来获取。定义如下：<br>`using TEventID = int8_t;`<br>不同产品对id的取值范围说明请参见[id取值范围说明](#id取值范围说明)。 |
