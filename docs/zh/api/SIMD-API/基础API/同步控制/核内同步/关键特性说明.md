...
- 以下接口都属于PIPE_S，这些接口与其它流水的接口之间的多流水同步由硬件保证：
    ...
    - 数据搬运随路原子操作接口：[SetAtomicAdd](../../原子操作/SetAtomicAdd.md)、[SetAtomicMax](../../原子操作/SetAtomicMax(ISASI).md)、[SetAtomicMin](../../原子操作/SetAtomicMin(ISASI).md)、[DisableDmaAtomic](../../原子操作/DisableDmaAtomic.md)、[SetAtomicType](../../原子操作/SetAtomicType.md)。
...
- 属于PIPE\_S的接口必须在以下接口集合内：
    - Tensor标量读写：[LocalTensor::GetValue](../../数据结构/LocalTensor/GetValue.md)、[LocalTensor::SetValue](../../数据结构/LocalTensor/SetValue.md)、[LocalTensor::operator()](../../数据结构/LocalTensor/operator().md)、[GlobalTensor::GetValue](../../数据结构/GlobalTensor/GetValue.md)、[GlobalTensor::operator()](../../数据结构/GlobalTensor/operator().md)。
    - Vector归约辅助：[GetReduceRepeatMaxMinSpr](../../Memory矢量计算/归约计算辅助配置接口/GetReduceRepeatMaxMinSpr(ISASI).md)、[GetReduceRepeatSumSpr](../../Memory矢量计算/归约计算辅助配置接口/GetReduceRepeatSumSpr(ISASI).md)。
    - 排序组合：[GetMrgSortResult](../../Memory矢量计算/排序组合（ISASI）/GetMrgSortResult.md)。
...
- 单流水同步
    ...
    - PIPE\_MTE2/PIPE\_MTE3在搬运地址有重叠的情况下需要开发者插入同步（具体示例请参考[PipeBarrier约束说明](../核内同步/PipeBarrier(ISASI).md)）。
