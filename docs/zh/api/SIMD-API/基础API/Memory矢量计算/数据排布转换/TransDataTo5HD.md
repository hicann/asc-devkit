...
| 参数名称 | 输入/输出 | 含义 |
| :------- | :-------- | :--- |
| dstList | 输出 | 目的操作数地址序列。<br><br>NCHW_CONV_ADDR_LIST_SIZE固定长度为16。<br>类型为[LocalTensor](../../数据结构/LocalTensor/LocalTensor.md)或者LocalTensor的地址值，存储位置为UB（TPosition为VECIN/VECCALC/VECOUT）。<br>LocalTensor的起始地址需要按照32字节对齐。 |
