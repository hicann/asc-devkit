...
- 该接口需要配合[LoadData（卷积数据搬运）](../矩阵计算的搬入/LoadData_3D.md)接口一起使用，需要在[LoadData（卷积数据搬运）](../矩阵计算的搬入/LoadData_3D.md)接口之前调用，其中fmatrixMode参数需要和LoadData（卷积数据搬运）接口参数中的fMatrixCtrl值保持一致。
