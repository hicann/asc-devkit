...
| NDDMA Cache | NDDMA Cache用于缓存最近或即将被[DataCopy（GMToUB多维数据搬运NDDMA）](../../基础API/Memory矢量计算/数据搬运/DataCopy_GMToUB_NDDMA.md)接口搬运的数据。 |
...
| Cache类型 | 接口名称 | 功能简述 |
|-----------|---------|---------|
| L2 Cache | [SetL2CacheHint](../../基础API/数据结构/GlobalTensor/SetL2CacheHint.md) | 读写GM的数据都默认被缓存在L2 Cache（默认启用L2 Cache），SetL2CacheHint能够设置GlobalTensor是否启用L2 Cache。 |
...
| NDDMA Cache | NdDmaDci | 在使用[DataCopy（GMToUB多维数据搬运NDDMA）](../../基础API/Memory矢量计算/数据搬运/DataCopy_GMToUB_NDDMA.md)接口进行数据搬运前，需要使用NdDmaDci接口刷新NDDMA Cache缓存保证为最新状态。 |
