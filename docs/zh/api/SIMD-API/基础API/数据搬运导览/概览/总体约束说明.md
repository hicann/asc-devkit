...
如果需要执行多个DataCopy指令，且DataCopy的目的地址存在重叠，需要调用[PipeBarrier](../../同步控制/核内同步/PipeBarrier_ISASI.md)插入同步指令，保证多个DataCopy指令的串行化，防止出现异常数据。如下图左侧示意图，执行两个DataCopy指令，搬运的目的GM地址存在重叠，两条搬运指令之间需要通过调用PipeBarrier<PIPE\_MTE3>\(\)进行MTE3流水的同步；如下图右侧示意图所示，搬运的目的地址UB存在重叠，两条搬运指令之间需要调用PipeBarrier<PIPE\_MTE2>\(\)进行MTE2流水的同步。多指令同步的核心代码示例如下：
