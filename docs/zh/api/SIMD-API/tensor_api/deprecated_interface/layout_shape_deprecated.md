...
**该接口已废弃，请使用[layout::shape](../layout/layout.md#shape)替代。**

获取Layout中的Shape对象或指定子维度对应的Shape子结构。
...
返回Shape对象或其子结构的引用；const重载返回常量引用。
...
auto& old_shape = layout.Shape();           // 废弃写法
auto& new_shape = layout.shape();           // 替代写法
```
