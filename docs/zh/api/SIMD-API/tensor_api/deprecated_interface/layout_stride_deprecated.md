...
**该接口已废弃，请使用[layout::stride](../layout/layout.md#stride)替代。**

获取Layout中的Stride对象或指定子维度对应的Stride子结构。
...
返回Stride对象或其子结构的引用；const重载返回常量引用。
...
auto& old_stride = layout.Stride();         // 废弃写法
auto& new_stride = layout.stride();         // 替代写法
```
