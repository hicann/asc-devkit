# copy（Unified Buffer到L1 Buffer数据搬运）
...
本接口将源Tensor的数据从Unified Buffer搬运到L1 Buffer。接口支持完整Tensor搬运，也支持通过`dst_coord`、`src_coord`和`copy_shape`指定目的Tensor中的起始坐标、源Tensor中的起始坐标和搬运区域的形状，执行区域搬运。

## 函数原型

- 执行Unified Buffer到L1 Buffer搬运。
...
- 按指定源坐标、目的坐标和搬运形状执行Unified Buffer到L1 Buffer搬运。
...
| 参数名 | 输入/输出 | 描述 |
| :--- | :---: | :--- |
| operation | 输入 | 搬运操作对象。Unified Buffer到L1 Buffer搬运取`copy_ub_to_l1{}`。 |
| trait | 输入 | 搬运trait对象。Unified Buffer到L1 Buffer默认取`ub_to_l1_trait_default{}`。 |

### copy_ub_to_l1说明

`copy_ub_to_l1`用于标识Unified Buffer到L1 Buffer的数据搬运通路，其静态`copy`成员负责分发该通路的数据搬运。
...
`ub_to_l1_trait`表示Unified Buffer到L1 Buffer搬运的编译期特性，当前不包含可配置成员。`ub_to_l1_trait_default`提供该通路的默认Trait类型和默认值。
...
- 搬运长度和块数需要满足Unified Buffer到L1 Buffer搬运指令约束。
