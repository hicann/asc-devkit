# copy（Unified Buffer到Unified Buffer数据搬运）
...
本接口在Unified Buffer内部搬运Tensor数据。接口支持完整Tensor搬运，也支持通过`dst_coord`、`src_coord`和`copy_shape`指定目的Tensor中的起始坐标、源Tensor中的起始坐标和搬运区域的形状，执行区域搬运。

## 函数原型

- 执行Unified Buffer到Unified Buffer搬运。
...
- 按指定源坐标、目的坐标和搬运形状执行Unified Buffer到Unified Buffer搬运。
...
| 参数名 | 输入/输出 | 描述 |
| :--- | :---: | :--- |
| operation | 输入 | 搬运操作对象。Unified Buffer到Unified Buffer搬运取`copy_ub_to_ub{}`。 |
| trait | 输入 | 搬运trait对象。Unified Buffer到Unified Buffer默认取`ub_to_ub_trait_default{}`。 |

### copy_ub_to_ub说明

`copy_ub_to_ub`用于标识Unified Buffer内部的数据搬运通路，其静态`copy`成员负责分发该通路的数据搬运。
...
`ub_to_ub_trait`表示Unified Buffer内部搬运的编译期特性，当前不包含可配置成员。`ub_to_ub_trait_default`提供该通路的默认Trait类型和默认值。
...
- 源和目的Tensor的元素类型必须一致。
- 支持相同格式间搬运：`ND`、`NDExt`、`DN`、`DNExt`、`NZ`和`ZN`。
- 搬运长度、块数和Stride需要满足Unified Buffer内部搬运指令约束。
- 源和目的区域重叠时，需要满足底层搬运指令对地址重叠的限制。
...
auto atom = make_copy(copy_ub_to_ub{}, ub_to_ub_trait_default{});
copy(atom, dst_tensor, src_tensor);
```
