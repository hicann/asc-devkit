...
L0C Buffer到Global Memory搬运支持不量化输出、`float`到`half`或`bfloat16_t`的直接转换输出，以及配合标量或张量量化参数的随路量化输出。随路Relu、通道拆分和量化舍入方式通过`l0c_to_gm_trait`配置。`mmad`与`Fixpipe`细粒度并行相关的`unit_flag`通过`l0c_to_gm_params`配置。

接口支持batch模式。batch模式用于一次完成多块矩阵计算结果的搬运。layout在原矩阵layout最外层增加Batch维度。源张量为`NZ`格式，分形固定为16×16，可通过`make_frame_layout<nz_layout_ptn>(batch, m, n)`构造。目的张量可通过`make_frame_layout<nd_layout_ptn>(batch, m, n)`、`make_frame_layout<dn_layout_ptn>(batch, m, n)`、`make_frame_layout<nd_ext_layout_ptn>(batch, m, n)`、`make_frame_layout<dn_ext_layout_ptn>(batch, m, n)`或`make_frame_layout<nz_layout_ptn, DstType>(batch, m, n)`构造。`NZ`格式可通过模板参数`DstType`指定目的数据类型，`C0`表示NZ格式的列分形大小，默认为16。卷积输出通路当前仅支持N为1，不支持batch模式。
...
- 根据源张量和目的张量的存储位置自动推导搬运通路，使用默认trait执行L0C Buffer到Global Memory的非量化搬运。
...
- 根据源张量和目的张量的存储位置自动推导搬运通路，使用默认trait执行L0C Buffer到Global Memory的量化搬运。
...
如需配置运行时搬运参数，先通过`atom.with(params)`将`l0c_to_gm_params`对象绑定到搬运原子对象；未绑定时使用默认值。

**表3**  `make_copy`接口参数说明

| 参数名 | 输入/输出 | 描述 |
| :--- | :---: | :--- |
| operation | 输入 | 搬运操作对象。L0C Buffer到Global Memory搬运采用`copy_l0c_to_gm{}`。 |
| trait | 输入 | 搬运静态特性对象。L0C Buffer到Global Memory搬运默认采用`l0c_to_gm_trait_default{}`。 |



### copy_l0c_to_gm说明

`copy_l0c_to_gm`用于标识L0C Buffer到Global Memory数据搬运通路，仅列出其public内容：
...
`copy`静态成员函数用于接收Trait和搬运参数，并分发L0C Buffer到Global Memory数据搬运。

### l0c_to_gm_trait说明

`l0c_to_gm_trait`用于配置L0C Buffer到Global Memory搬运的静态特性。
...
struct l0c_to_gm_trait {
    ...
    __aicore__ constexpr l0c_to_gm_trait(asc::te::round_mode round_mode,
...
|enable_channel_split|`false`|是否使能输出数据通道拆分。只支持NZ格式且源类型和目的类型均为`float`的场景。|
...
`l0c_to_gm_trait_default`提供该通路的默认Trait类型和默认值：
...
**表5**  `l0c_to_gm_params`成员说明

|成员|默认值|描述|
|--------|--------|--------|
|unit_flag|`unit_flag_mode::disable`|控制mmad指令和Fixpipe指令的细粒度并行。`unit_flag_mode::disable`表示不使能，`unit_flag_mode::enable_keep`表示使能且执行后不复位单元标记位，`unit_flag_mode::enable_update`表示使能且执行后复位单元标记位。|
...
- `enable_channel_split`仅在源类型和目的类型均为`float`，且目的格式为`NZ`时生效。详情参见[F32 Channel Split](../cube_store_key_features/f32_channel_split.md)。
- 通道合并特性硬件自动使能，不能通过参数配置。详情参见[Int8 Channel Merge](../cube_store_key_features/int8_channel_merge.md)。
- `NZ`到`NC1HWC0`、`NHWC`或`NCHW`的卷积输出通路当前仅支持N为1。`NZ`到`NHWC`或`NCHW`仅支持不传入`quant`的调用形式。
- 卷积输出通路中，源`NZ`张量的逻辑形状需要为`(H * W, C)`。目的张量为`NC1HWC0`时需要满足`C = C1 * C0`。

## 关键特性

L0C Buffer到Global Memory搬运涉及[随路量化](../cube_store_key_features/quant_pre.md)、[随路Relu](../cube_store_key_features/relu_pre.md)、[F32 Channel Split](../cube_store_key_features/f32_channel_split.md)、[Int8 Channel Merge](../cube_store_key_features/int8_channel_merge.md)和[batch搬运](../cube_store_key_features/batch_copy.md)等关键特性。

## 调用示例

### 卷积输出

以下示例将L0C Buffer中形状为`(H * W, C)`的NZ矩阵搬运到Global Memory，并转换为NHWC格式。将目的layout替换为`nchw_layout_ptn`或`nc1hwc0_layout_ptn`，可分别完成NCHW或NC1HWC0格式输出。
...
__aicore__ inline void copy_conv_output_l0c_to_gm(__gm__ float* gm_addr)
{
    __cc__ float l0c_buf[h * w * c];

    auto l0c = make_tensor(
        make_mem_ptr(l0c_buf),
        make_frame_layout<nz_layout_ptn, 16>(h * w, c));
    auto gm = make_tensor(
        make_mem_ptr(gm_addr), make_frame_layout<nhwc_layout_ptn>(1, h, w, c));

    auto copy_l0c_to_gm_atom = make_copy(copy_l0c_to_gm{}, l0c_to_gm_trait_default{});
    copy(copy_l0c_to_gm_atom, gm, l0c);
}
```
...
__aicore__ inline void copy_l0c_to_gm_example()
{
    auto l0c = make_tensor(make_mem_ptr(l0c_addr), make_frame_layout<nz_layout_ptn, 16>(m, n));
    auto gm = make_tensor(make_mem_ptr(gm_addr), make_frame_layout<nd_ext_layout_ptn>(m, n));

    l0c_to_gm_params params;
    auto atom = make_copy(copy_l0c_to_gm{}).with(params);
    copy(atom, gm, l0c);
}
```
...
__aicore__ inline void copy_batch_l0c_to_gm(__gm__ half* gm_addr)
{
    constexpr uint32_t src_batch = 3;
    constexpr uint32_t dst_batch = 9;
    constexpr uint32_t m = 32;
    constexpr uint32_t n = 64;

    __cc__ float l0c_buf[src_batch * m * n];

    auto l0c = make_tensor(make_mem_ptr(l0c_buf), make_frame_layout<nz_layout_ptn>(src_batch, m, n));
    auto gm = make_tensor(make_mem_ptr(gm_addr), make_frame_layout<nd_ext_layout_ptn>(dst_batch, m, n));

    auto atom = make_copy(copy_l0c_to_gm{}).with(l0c_to_gm_params{});
    for (uint32_t i = 0; i < dst_batch / src_batch; ++i) {
        auto gm_slice = gm.slice(make_coord(i * src_batch, make_coord(0, 0)), make_shape(src_batch, make_shape(m, n)));
        copy(atom, gm_slice, l0c);
    }
}
```
