...
- 按指定源坐标、目的坐标和搬运形状执行L1 Buffer到L0ScaleB Buffer搬运。
...
| 参数名 | 输入/输出 | 描述 |
| :--- | :---: | :--- |
| operation | 输入 | 搬运操作对象。L1 Buffer到L0ScaleB Buffer通路使用`copy_l1_to_l0scaleb{}`。 |
| trait | 输入 | 搬运Trait对象，用于指定搬运特性。L1 Buffer到L0ScaleB Buffer默认Trait使用`l1_to_l0scaleb_trait_default{}`。 |

### copy_l1_to_l0scaleb说明

`copy_l1_to_l0scaleb`用于标识L1 Buffer到L0ScaleB Buffer数据搬运通路，仅列出其public内容：
...
`copy`静态成员函数用于接收Trait和搬运参数，并分发L1 Buffer到L0ScaleB Buffer数据搬运。
