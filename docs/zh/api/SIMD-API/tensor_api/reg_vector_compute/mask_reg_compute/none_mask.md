...
返回`reg_tensor<bool>`，其`reg`成员是类型为vector_bool的掩码寄存器，其对应的元素全为无效数据。
...
```cpp
auto disabled = asc::te::experimental::none_mask<uint32_t>();
auto value = asc::te::experimental::load(src, asc::te::make_coord(0)).with_mask(disabled);
```
