...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/tool_interface/system_resources_and_variables/GetSubBlockNum_ISASI_res.md#id1 -->

## 功能说明

头文件路径为：`"basic_api/kernel_operator_sys_var_intf.h"`。

在[分离模式架构](../../../../../guide/programming_guide/advanced_programming/hardware_implementation/basic_architecture.md)下，获取当前配置中，一个逻辑AI Core上Cube Core（AIC）或者Vector Core（AIV）的数量。
...
在核函数（Kernel）直调工程场景下，不同[算子类型](../../../../../guide/programming_guide/language_extension/simd_builtin_keywords.md#section1074418132518)在AIC和AIV上调用该接口的返回值如下：

**表1** 通过函数执行空间限定符设置核函数（Kernel）类型时的返回值
...
**表2** 通过Kernel Type接口设置核函数（Kernel）类型时的返回值
...
**注意，两种场景中返回值的含义相同，仅设置类型的方式不同，编写算子时推荐使用核函数（Kernel）直调工程。**
