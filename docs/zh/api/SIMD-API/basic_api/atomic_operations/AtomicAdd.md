...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/atomic_operations/AtomicAdd_res.md#id1 -->
...
- 毕昇编译器能够在一部分属于PIPE_S的接口与属于PIPE_V、PIPE_MTE2、PIPE_MTE3的接口之间自动插入同步（详见[使用约束](../sync_control/intra_core_sync/key_features.md#使用约束-1)）。但标量原子操作接口均不在该范围内，因此AtomicAdd与搬运接口（属于PIPE_MTE2、PIPE_MTE3）在读写GM时存在数据依赖时，毕昇编译器无法自动插入同步，开发者需根据实际情况手动插入同步。
