...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/cube_compute_ISASI/cube_load_aux_config/SetLoadDataRepeatWithStride_res.md#id1 -->

## 功能说明<a id="section618mcpsimp"></a>

头文件路径为：`"basic_api/kernel_operator_mm_intf.h"`。

用于设置[LoadDataWithStride](../cube_compute_load/LoadDataWithStride.md)接口的repeat参数。设置repeat参数后，可以通过调用一次LoadDataWithStride接口完成多个迭代的数据搬运，还用于配置输出矩阵K轴方向偏移。
...
- 调用[LoadDataWithStride](../cube_compute_load/LoadDataWithStride.md)接口时，必须配置本接口中的dstStride参数，若不使能repeat模式，repeatTime、repeatStride、repeatMode使用默认构造值。
