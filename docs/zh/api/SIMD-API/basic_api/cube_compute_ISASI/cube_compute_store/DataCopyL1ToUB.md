...
提供从L1 Buffer搬运数据到Unified Buffer（UB）的基础能力，数据在传输过程中保持原始格式和内容不变，支持连续和非连续的数据搬运。
...
- 如果需要执行多个DataCopyL1ToUB指令，且DataCopyL1ToUB的目的地址存在重叠，需要通过调用[PipeBarrier\(ISASI\)](../../sync_control/intra_core_sync/PipeBarrier_ISASI.md)来插入同步指令，保证多个DataCopyL1ToUB指令的串行化，防止出现异常数据。
