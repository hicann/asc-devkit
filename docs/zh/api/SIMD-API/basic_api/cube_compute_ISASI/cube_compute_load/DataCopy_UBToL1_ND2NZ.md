# DataCopy（UBToL1随路转换-ND2NZ搬运）<a id="ZH-CN_TOPIC_0000002349187356"></a>
...
| src | 输入 | 源操作数，类型为LocalTensor，存储位置为UB（TPosition为VECIN/VECCALC/VECOUT）。 |
...
- 本接口仅支持配置ndNum为1。
- 位于UB的源地址必须32字节对齐，位于L1 Buffer的目的地址必须32字节对齐。
