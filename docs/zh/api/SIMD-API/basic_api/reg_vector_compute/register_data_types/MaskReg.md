...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/register_data_types/MaskReg_res.md#id1 -->
...
在Mask设置中，Reg矢量计算支持多种灵活配置方式，可根据实际计算场景选择固定Mask模式、从Unified Buffer（UB）搬运或从RegTensor搬运等策略，以满足不同计算场景的需求。具体设置方式如下表所示：
...
- 方式3<a id="method3"></a>
  ...
  采用[LoadAlign](../reg_data_load/LoadAlign_MaskReg.md)从UB搬运Mask到MaskReg。在UB中设置Mask。
...
- 方式4<a id="method4"></a>
  ...
  采用[MaskGenWithRegTensor](../reg_data_load/MaskGenWithRegTensor_MaskReg.md)从RegTensor搬运Mask到MaskReg。在RegTensor中设置Mask。
...
- 方式5<a id="method5"></a>
  ...
  采用[MoveMask](../MaskReg_compute/MoveMask.md)，从SetVectorMask设置的掩码寄存器{MASK1,MASK0}中读取Mask值，并按模板参数T对应的数据格式转换后写入返回值MaskReg。b16类型读取完整128bit的{MASK1,MASK0}，将每个bit复制为2bit；b32类型读取64bit的MASK0，并将每个bit复制为4bit。
