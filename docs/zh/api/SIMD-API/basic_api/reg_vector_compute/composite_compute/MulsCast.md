...
| T1 | 源操作数的数据类型。 |
| T2 | 标量源操作数的数据类型。 |
| layout | [RegLayout](../aux_data_types/RegLayout.md)枚举类型。<br>支持RegLayout::ZERO、RegLayout::ONE。 |
...
| mask | 输入 | 源操作数元素级操作的有效性指示，详细说明请参考[MaskReg](../register_data_types/MaskReg.md)。 |
...
- 本接口操作数为寄存器，不涉及地址对齐。
- 本接口不修改全局寄存器的值。
- [Cast](../type_conversion/Cast.md)计算按照CAST\_ROUND模式舍入。
