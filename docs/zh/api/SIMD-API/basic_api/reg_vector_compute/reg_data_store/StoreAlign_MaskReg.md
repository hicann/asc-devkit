...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/reg_data_store/StoreAlign_MaskReg_res.md#id1 -->

## 功能说明<a name="section618mcpsimp"></a>

头文件路径为：`"basic_api/reg_compute/kernel_reg_compute_datacopy_intf.h"`。

StoreAlign能够实现数据从[MaskReg](../register_data_types/MaskReg.md)搬运至Unified Buffer（UB）。有三类搬运接口：
...
- 使用[AddrReg](../register_data_types/AddrReg.md)寄存器存储偏移量场景：在每次迭代中，需要先调用CreateAddrReg手动设定地址偏移量，再调用搬运指令。
...
- 使用[AddrReg](../register_data_types/AddrReg.md)存储偏移量接口，需要配合CreateAddrReg接口使用
...
| mask | 输入 | 源操作数，类型为[MaskReg](../register_data_types/MaskReg.md)。 |
...
- 位于UB的地址约束、offset的对齐约束、AddrReg寄存器中存储的偏移量 \* sizeof(T)的对齐约束与MaskDist有关，具体地址约束请参考：[表3 MaskDist参数说明](#表3-MaskDist参数说明)。
