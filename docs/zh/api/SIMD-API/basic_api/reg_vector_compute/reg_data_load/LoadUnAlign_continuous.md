...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/reg_data_load/LoadUnAlign_continuous_res.md#id1 -->

## 功能说明<a name="section618mcpsimp"></a>

头文件路径为：`"basic_api/reg_compute/kernel_reg_compute_datacopy_intf.h"`。

为提升对不规则内存地址的处理能力，Reg矢量计算支持在数据搬运过程中对非32字节对齐的地址进行访问，降低非对齐访问带来的性能开销。LoadUnAlign能够实现数据从非对齐的Unified Buffer（UB）连续搬运至[RegTensor](../register_data_types/RegTensor.md)，利用非对齐寄存器[UnalignRegForLoad](../register_data_types/UnalignRegForLoad-UnalignRegForStore.md)作为临时缓存区，暂存跨对齐边界的数据，从而实现高效的连续非对齐数据传输。

非对齐搬运有三类接口：普通搬运接口、PostUpdate扩展搬运接口、使用[AddrReg](../register_data_types/AddrReg.md)寄存器存储偏移量接口。
...
- 使用[AddrReg](../register_data_types/AddrReg.md)存储偏移量接口（需要配合[CreateAddrReg](../register_data_types/AddrReg.md)接口使用）：
