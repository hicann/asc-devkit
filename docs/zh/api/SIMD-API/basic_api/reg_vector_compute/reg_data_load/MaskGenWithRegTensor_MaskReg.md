...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/reg_data_load/MaskGenWithRegTensor_MaskReg_res.md#id1 -->

## 功能说明

头文件路径为：`"basic_api/reg_compute/kernel_reg_compute_maskreg_intf.h"`。

MaskGenWithRegTensor能够实现数据从[RegTensor](../register_data_types/RegTensor.md)搬运至[MaskReg](../register_data_types/MaskReg.md)。
...
| 参数名 | 输入/输出 | 描述 |
|-----|-----|-----|
| dst | 输出 | 目的操作数，类型为[MaskReg](../register_data_types/MaskReg.md)。 |
