...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/reg_data_load/LoadAlign_MaskReg_res.md#id1 -->

## 功能说明<a name="section618mcpsimp"></a>

头文件路径为：`"basic_api/reg_compute/kernel_reg_compute_datacopy_intf.h"`。

LoadAlign能够实现数据从Unified Buffer（UB）搬运至[MaskReg](../register_data_types/MaskReg.md)。有三类搬运接口：
...
- 使用[AddrReg](../register_data_types/AddrReg.md)寄存器存储偏移量场景：在每次迭代中，需要先调用[CreateAddrReg](../register_data_types/AddrReg.md)手动设定地址偏移量，再调用搬运指令。
...
| 参数名 | 输入/输出 | 描述 |
|-----|-----|-----|
| mask | 输出 | 目的操作数，类型为[MaskReg](../register_data_types/MaskReg.md)。 |
