...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/logical_compute/Xor_res.md#id1 -->
...
- 对RegTensor操作
  ...
  **表2**  参数说明
  ...
  | srcReg0 | 输入 | 源操作数。<br>类型为[RegTensor](../register_data_types/RegTensor.md)。<br>数据类型需要与目的操作数保持一致。 |
  | srcReg1 | 输入 | 源操作数。<br>类型为[RegTensor](../register_data_types/RegTensor.md)。<br>数据类型需要与目的操作数保持一致。 |
...
- 对MaskReg操作
  ...
  **表3**  参数说明
  ...
  | mask | 指示在计算过程中哪些bit有效。类型为[MaskReg](../register_data_types/MaskReg.md)。 |
