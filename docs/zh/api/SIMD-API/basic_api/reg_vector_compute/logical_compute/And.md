...
- Atlas 训练系列产品：不支持
<!-- end id7 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/basic_api/reg_vector_compute/logical_compute/And_res.md#id1 -->
...
本节介绍两种接口，分别用于对[RegTensor](../register_data_types/RegTensor.md)和[MaskReg](../register_data_types/MaskReg.md)进行操作：

- 对`RegTensor`操作：

  根据mask对输入数据srcReg0、srcReg1执行按位与（&）操作，将结果写入dstReg。
...
- 对`RegTensor`操作
  ...
  **表2**  参数说明
  ...
  | srcReg0 | 输入 | 源操作数。<br>类型为[RegTensor](../register_data_types/RegTensor.md)。<br>两个源操作数的数据类型需要与目的操作数保持一致。 |
  | srcReg1 | 输入 | 源操作数。<br>类型为[RegTensor](../register_data_types/RegTensor.md)。<br>两个源操作数的数据类型需要与目的操作数保持一致。 |
  | mask | 输入 | 指示源操作数元素操作的有效性，详细说明请参考[MaskReg文档](../register_data_types/MaskReg.md)。 |
