...
Host侧Tiling API使用[AscendC::TensorShape](../数据结构/TensorShape.md)描述输入数据的Tensor形状，使用样例如下：
...
下面以[Exp](Exp接口/Exp接口.md)接口的关键调用代码为例，辅以调用前后数据打印结果，展示模板参数isReuseSource的使用及其相关影响。
