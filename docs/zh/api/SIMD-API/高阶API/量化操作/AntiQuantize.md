...
| sharedTmpBuffer | 输入 | 临时缓存。<br><br>类型为[LocalTensor](../../基础API/数据结构/LocalTensor/LocalTensor.md)，支持的TPosition为VECIN/VECCALC/VECOUT。<br><br>临时空间大小BufferSize的获取方式请参考[GetAntiQuantizeMaxMinTmpSize](GetAntiQuantizeMaxMinTmpSize.md)。 |
...
**表3**  PER_TENSOR/PER_CHANNEL量化策略支持的数据类型组合
...
**表4**  PER_TOKEN/PER_GROUP量化策略支持的数据类型组合
