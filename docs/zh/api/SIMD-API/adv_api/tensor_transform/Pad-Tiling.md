...
| 参数名称 | 输入/输出 | 含义 |
| --- | --- | --- |
| srcShape | 输入 | 输入Tensor的shape信息，参数类型为[AscendC::TensorShape](../data_structures/TensorShape.md)，shape为二维。（有效数据+冗余数据） |
| oriSrcShape | 输入 | 输入Tensor的原始shape信息，参数类型为[AscendC::TensorShape](../data_structures/TensorShape.md)，shape为二维。（有效数据） |
...
3.  对应的kernel侧通过在核函数中调用GET\_TILING\_DATA获取TilingData，继而将TilingData中的Pad Tiling信息传入Pad接口参与计算。完整的kernel侧样例请参考[调用示例](Pad.md#调用示例)。
