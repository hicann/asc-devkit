...
| [Axpy](math_compute/Axpy_interface/Axpy-80.md) | 源操作数中每个元素与标量求积后和目的操作数中的对应元素相加。 |
...
| [Exp](math_compute/Exp_interface/Exp-81.md) | 按元素取自然指数。 |
...
| [Log](math_compute/Log_interface/Log-79.md) | 按元素以e、2、10为底做对数运算。 |
...
| [Xor](math_compute/Xor_interface/Xor-82.md) | 按元素执行Xor（异或）运算。 |
...
| [ReduceSum](reduction_operations/ReduceSum_interface/ReduceSum-90.md) | 对一个多维向量按照指定的维度进行数据累加。 |
| [ReduceMean](reduction_operations/ReduceMean_interface/ReduceMean.md) | 对一个多维向量按照指定的维度求平均值。 |
| [ReduceMax](reduction_operations/ReduceMax_interface/ReduceMax-91.md) | 对一个多维向量在指定的维度求最大值。 |
| [ReduceMin](reduction_operations/ReduceMin_interface/ReduceMin-92.md) | 对一个多维向量在指定的维度求最小值。 |
...
| [MrgSort](sort_operations/MrgSort-93.md) | 将已经排好序的最多4条队列，合并排列成1条队列，结果按照score域由大到小排序。 |

##  数据过滤

| 接口名 | 功能描述 |
| --- | --- |
| [Select](data_filter/Select-95.md) | 给定两个源操作数src0和src1，根据maskTensor相应位置的值（非bit位）选取元素，得到目的操作数dst。 |
...
| 接口名 | 功能描述 |
| --- | --- |
| [Transpose](tensor_transform/Transpose-96.md) | 对输入数据进行数据排布及Reshape操作。 |
...
| [Fill](tensor_transform/Fill-97.md) | 将Global Memory上的数据初始化为指定值。 |

##  索引计算

| 接口名 | 功能描述 |
| --- | --- |
| [Arange](index_compute/Arange-94.md) | 给定起始值，等差值和长度，返回一个等差数列。 |
