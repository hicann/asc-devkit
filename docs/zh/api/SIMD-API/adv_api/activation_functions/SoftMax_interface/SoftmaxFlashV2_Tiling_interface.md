...
| isUpdate | 输入 | 是否开启刷新功能，和核函数（Kernel）侧SoftmaxFlashV2接口一致。 |
| isBasicBlock | 输入 | 是否要开启基本块计算。isBasicBlock参数可以通过[isBasicBlockInSoftmax](IsBasicBlockInSoftMax.md)接口获取，与核函数（Kernel）侧接口的模板参数保持一致，默认false。注意，若核函数（Kernel）侧API启用模板参数SoftmaxConfig，即shape常量化场景，isBasicBlock参数必须通过接口[isBasicBlockInSoftmax](IsBasicBlockInSoftMax.md)获取。 |
