...
另外提供了一个核函数（Kernel）侧tiling计算的接口，当核函数（Kernel）侧的输入shape与通过host侧TilingData传入的shape不一致时，可使用该接口在核函数（Kernel）侧重新计算tiling。该接口的参数含义请参考[SoftmaxFlashV2 Tiling接口](SoftmaxFlashV2_Tiling_interface.md)。
