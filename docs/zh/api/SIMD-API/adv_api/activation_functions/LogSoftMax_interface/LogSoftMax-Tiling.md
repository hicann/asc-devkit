...
核函数（Kernel）侧LogSoftMax接口的计算需要开发者预留/申请临时空间，以下接口用于在host侧获取预留/申请的最大最小临时空间大小，开发者基于此范围选择合适的空间大小，并调用LogSoftMaxTilingFunc函数获取reduceSize，splitSize等参数，作为Tiling参数传递到核函数（Kernel）侧使用。
...
| softmaxTiling | 输出 | 传递到核函数（Kernel）侧使用的Tiling参数。 |
...
// 计算最优分片策略并填充到tilingData结构体中，供核函数（Kernel）端使用。
