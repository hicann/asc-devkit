...
本接口支持的同一系数的量化/反量化模式、向量的量化/反量化模式分别与核函数（Kernel）侧接口[SetQuantScalar](../Matmul_Kernel/SetQuantScalar.md)和[SetQuantVector](../Matmul_Kernel/SetQuantVector.md)对应，本接口设置的量化/反量化模式必须与核函数（Kernel）侧使用的接口保持一致。
