...
核函数（Kernel）侧实现Matmul矩阵乘运算的步骤概括为：
...
1.  创建Matmul对象。
    ...
    **表1**  MatmulType参数说明
    ...
    <!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/adv_api/cube_compute/Matmul_Kernel/Matmul_usage_res.md#id5 -->

    <!-- npu="950,A3,910b,310p,310b" id102 -->
    **表2**  Matmul输入输出数据类型的支持列表
    ...
    | int8_t | int8_t | int32_t | int32_t/half | <!-- npu="950" id93 -->Ascend 950PR/Ascend 950DT<!-- end id93 --><!-- npu="A3" id94 --><br>    <br>Atlas A3 训练系列产品/Atlas A3 推理系列产品<!-- end id94 --><!-- npu="910b" id95 --><br>   <br>Atlas A2 训练系列产品/Atlas A2 推理系列产品<!-- end id95 --><!-- npu="310p" id96 --><br>    <br>Atlas 推理系列产品AI Core<!-- end id96 --><!-- npu="310b" id97 --><br>    <br>Atlas 200I/500 A2 推理产品<!-- end id97 --> |
    ...
    | int8_t | int8_t | int32_t | int8_t | <!-- npu="950" id66 -->Ascend 950PR/Ascend 950DT<!-- end id66 --><!-- npu="A3" id67 --><br>    <br>Atlas A3 训练系列产品/Atlas A3 推理系列产品<!-- end id67 --><!-- npu="910b" id68 --><br>   <br>Atlas A2 训练系列产品/Atlas A2 推理系列产品<!-- end id68 --><!-- npu="310p" id69 --><br>    <br>Atlas 推理系列产品AI Core<!-- end id69 --> |
    ...
    | half | half | half | half/bfloat16_t | <!-- npu="950" id98 -->Ascend 950PR/Ascend 950DT<!-- end id98 --><!-- npu="A3" id99 --><br>    <br>Atlas A3 训练系列产品/Atlas A3 推理系列产品<!-- end id99 --><!-- npu="910b" id100 --><br>   <br>Atlas A2 训练系列产品/Atlas A2 推理系列产品<!-- end id100 --><!-- npu="310b" id101 --><br>    <br>Atlas 200I/500 A2 推理产品<!-- end id101 --> |
...
4.  完成矩阵乘操作。
    ...
    -   用户申请用于存放矩阵乘结果的L0C Buffer（CO1）内存，调用一次或多次[Iterate](Iterate.md#li4843165185812)完成单次或多次迭代计算，在需要搬出计算结果时，调用[Fixpipe](../../../basic_api/cube_compute_ISASI/cube_compute_store/Fixpipe_L0CToGM.md)接口完成L0C Buffer（CO1）上计算结果的搬运，然后释放申请的L0C Buffer（CO1）内存。该方式下，用户可以灵活控制计算和搬运的节奏，根据实际需要，一次计算对应一次结果的搬出，或者将多次计算结果缓存在L0C Buffer（CO1）内存中，再一次性搬出计算结果。
        ...
        - Atlas 200I/500 A2 推理产品暂不支持该方式。
          <!-- end id89 -->
        <!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/adv_api/cube_compute/Matmul_Kernel/Matmul_usage_res.md#id7 -->
...
CubeFormat::NZ格式的矩阵对齐要求为：
- A矩阵/B矩阵：外轴为16的倍数，内轴为C0_size的倍数。
<!-- npu="310p,310b" id103 -->
对于如下产品型号：
...
- C矩阵（开启channel_split功能）：外轴为16的倍数，内轴为C0_size的倍数。
- C矩阵（不开启channel_split功能）：外轴为16的倍数，对于float/int32_t类型内轴为16的倍数，对于half/bfloat16_t/int8_t/fp8_e4m3fn_t/fp8_e5m2_t/hifloat8_t类型内轴为C0_size的倍数。
<!-- end id104 -->
<!-- @ref: asc-devkit/res/docs/zh/api/SIMD-API/adv_api/cube_compute/Matmul_Kernel/Matmul_usage_res.md#id8 -->
