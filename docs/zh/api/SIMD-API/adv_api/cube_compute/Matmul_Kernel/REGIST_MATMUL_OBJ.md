...
-   在Matmul的内部实现中使用了[CrossCoreSetFlag](../../../basic_api/sync_control/inter_core_sync/CrossCoreSetFlag_ISASI.md)和[CrossCoreWaitFlag](../../../basic_api/sync_control/inter_core_sync/CrossCoreWaitFlag_ISASI.md)进行核间同步控制，因此开发者同时使用CrossCoreSetFlag（或CrossCoreWaitFlag）和Matmul时，需注意避免flagId冲突，各接口flagId占用的完整说明（含各NPU架构版本差异）参见[核间同步flagId占用说明](../../../basic_api/sync_control/inter_core_sync/key_features.md#inter_core_sync_flagid_usage)。
