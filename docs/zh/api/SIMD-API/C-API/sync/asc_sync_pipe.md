...
完成同一流水线内不同指令间的同步控制，用于在同一流水线内部约束执行顺序。如图1所示，其作用是保证相同流水(例如PIPE_MTE2)中前序指令中所有数据的读写工作全部完成，后序指令才能执行。

**图1**  asc_sync_pipe.png接口功能示意图    
![](../../C-API/figures/asc_sync_pipe.png)
