...
将src中的浮点数元素按照TRUNC（向零方向截断）截断到整数值，结果仍保持原浮点数据类型写入dst。未被mask筛选的元素置零。
...
```cpp
__simd_vf__ inline void trunc_vf(__ubuf__ half* dst_addr, __ubuf__ half* src_addr, uint32_t count, uint16_t one_repeat_size, uint16_t repeat_time)
{
    vector_half dst;
    vector_half src;
    vector_bool mask;
