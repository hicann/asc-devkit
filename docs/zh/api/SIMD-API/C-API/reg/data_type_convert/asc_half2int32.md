...
将half类型数据转为int32_t类型，并支持多种舍入模式。
...
```cpp
__simd_vf__ inline void half2int32_vf(__ubuf__ int32_t* dst_addr, __ubuf__ half* src_addr, uint32_t one_repeat_size, uint16_t one_block_size, uint16_t repeat_time)
{
    vector_half src;
    vector_int32_t dst;
    ...
    for (uint16_t i = 0; i < repeat_time; ++i) {
        ...
        asc_half2int32_rd(dst, src, mask);
