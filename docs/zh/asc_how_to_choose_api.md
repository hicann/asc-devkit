...
此外，联合生态正持续建设核函数（Kernel）编程C++标准库**asc-stl**、设备侧线性代数库**asc-mathdx**等基础组件，不断丰富Ascend C算子编程生态。
...
- **乐高式高度可组合**：任意atom可适配任意algorithm，任意layout可结合任意atom；通过模块化积木式拼接构建内核，大幅度降低核函数（Kernel）开发成本。
