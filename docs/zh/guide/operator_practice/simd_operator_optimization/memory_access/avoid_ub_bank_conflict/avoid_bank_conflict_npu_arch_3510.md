...
同一个bank group单拍支持两次读操作，或者一次读操作和一次写操作；不支持同一个bank group单拍完成两次写操作。根据内存结构，bank冲突主要可以分为以下三种类型：
...
- 通过地址重叠减少读指令
  ...
  同一个bank group单拍仅支持两次读操作，或者一次读操作和一次写操作。两读一写会超出支持的并行访问组合而引发bank冲突；当两条读指令的起始地址重叠时，硬件会将其优化为一条。
