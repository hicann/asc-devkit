# 多核对齐切分<a name="ZH-CN_TOPIC_0000002532228165"></a>
...
-   纯Cube模式（只有矩阵计算）场景，本节内容以纯Cube模式举例。
    ...
    [SetDim](../../../../../api/SIMD-API/adv_api/cube_compute/Matmul_Tiling/SetDim.md)设置当前AI处理器可用的核数，通过[Tiling计算](../../../../../api/SIMD-API/adv_api/cube_compute/Matmul_Tiling/GetTiling.md)得到执行Matmul计算实际使用的核数，实际使用的核数小于等于AI处理器可用的核数。SetSimdNumBlocks按照实际使用的核数由用户进行配置。

-   MIX模式（包含矩阵计算和矢量计算）的设置规则请参考[MIX场景核数设置规则](../../fusion_operator_programming/cv_fusion/operator_impl.md#zh-cn_topic_0000001644252364_li4790115115920)。
...
该场景的关键代码示例如下。Matmul多核对齐场景的完整样例请参考：多核切M、N的样例：[Matmul多核场景的核函数（Kernel）直调样例](../../../../../../../examples/01_simd_cpp_api/00_introduction/02_matrix/matmul_advanced_api)；多核切K的样例：[多核切K场景的算子样例](../../../../../../../examples/01_simd_cpp_api/04_advanced_api/00_matmul/matmul_splitk)。
