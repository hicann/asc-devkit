...
-   核内同步
    ...
    AI Core内部包括MTE1、MTE2、MTE3、Cube、Vector、Scalar等多条流水线。Ascend C框架默认开启auto sync（自动插入同步）编译选项，编译器可以正常插入同步；Ascend C编程模型也会帮助开发者完成部分流水的同步控制。流水类型的详细介绍、同步类型的分类、编译器自动同步的约束限制、何时需要开发者手动插入同步可参考[核内同步能力概述](../../../api/SIMD-API/基础API/同步控制/核内同步/核内同步能力概述.md)。

-   核间同步

    上文描述的都是核内同步的情况。特别的，当算子使用多核同步时（多核同步概念可参考[多核同步](../../../api/SIMD-API/基础API/同步控制/核间同步/IBSet.md)），逻辑核数NumBlocks必须保证不大于实际运行该算子的AI处理器核数，否则框架插入同步会出现异常，导致Kernel“卡死”现象。
...
![](../../figures/多核并行处理示意图-53.png "多核并行处理示意图-53")
