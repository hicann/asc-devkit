...
|------|------|
| 算子类型（OpType） | Matmul |
| 算子输入a | shape：(m, k) = (32, 32)；data type：half；format：[ND](../../../技术附录/概念原理和术语/神经网络和算子/数据排布格式.md) |
| 算子输入b | shape：(k, n) = (32, 32)；data type：half；format：[ND](../../../技术附录/概念原理和术语/神经网络和算子/数据排布格式.md) |
| 算子输出c | shape：(m, n) = (32, 32)；data type：half；format：[ND](../../../技术附录/概念原理和术语/神经网络和算子/数据排布格式.md) |
| 核函数名称 | matmul_custom |
| 使用的主要接口 | [DataCopy（GMToL1随路转换-ND2NZ搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/DataCopy_GMToL1_ND2NZ.md)：Global Memory到L1 Buffer数据搬运 + ND转NZ格式转换接口<br>[LoadData（2D矩阵搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_2D.md)：L1 Buffer到L0 Buffer数据搬运 + NZ转ZZ/ZN格式转换接口<br>[Mmad](../../../../api/SIMD-API/基础API/cube_compute_ISASI/Mmad计算/Mmad.md)：矩阵乘计算接口<br>[Fixpipe（L0C到GM数据搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬出/Fixpipe_L0CToGM.md)：L0C Buffer到Global Memory数据搬出 + NZ转ND格式转换 + 精度转换接口 |
...
1.  CopyIn任务（Globale Memory --> L1 Buffer）  
    使用[DataCopy（GMToL1随路转换-ND2NZ搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/DataCopy_GMToL1_ND2NZ.md)接口将Global Memory 中的数据搬运到L1 Buffer，将ND格式转换为NZ格式。

2.  Split任务（L1 Buffer--> L0 Buffer）    
    使用[LoadData（2D矩阵搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬入/LoadData_2D.md)接口将数据从L1 Buffer中搬运到L0A Buffer/L0B Buffer，同时完成NZ到ZZ/ZN的格式转换。

3.  Compute任务（矩阵乘计算）    
    使用[Mmad](../../../../api/SIMD-API/基础API/cube_compute_ISASI/Mmad计算/Mmad.md)接口在完成矩阵乘计算，结果存放在L0C Buffer。

4.  CopyOut任务（L0C Buffer--> Globale Memory）    
    使用[Fixpipe（L0C到GM数据搬运）](../../../../api/SIMD-API/基础API/cube_compute_ISASI/矩阵计算的搬出/Fixpipe_L0CToGM.md)接口将结果从L0C Buffer直接搬运到Global Memory，同时利用Fixpipe的随路格式转换功能将NZ格式转回ND格式。
...
> [!NOTE]说明
> CopyIn、DataLoad、Compute和CopyOut四个阶段的操作分别在不同的硬件流水线上执行。为保证数据依赖正确——即每个阶段读取的数据必须由前一阶段写入完毕，需要在各阶段之间设置同步屏障。这里使用全局流水同步[PipeBarrier](../../../../api/SIMD-API/基础API/同步控制/核内同步/PipeBarrier_ISASI.md)。
