...
根据上述分析，明确实现过程中会使用到[Matmul高阶API](../../../../../api/SIMD-API/高阶API/矩阵计算/Matmul-Kernel侧接口/Matmul-Kernel侧接口.md)接口，[LeakyRelu Vector计算接口](../../../../../api/SIMD-API/基础API/Memory矢量计算/基础算术/LeakyRelu.md)、[DataCopy](../../../../../api/SIMD-API/基础API/数据搬运导览/概览/数据搬运概念.md)、[EnQue](../../../../../api/SIMD-API/基础API/资源管理/TQue/EnQue.md)、[DeQue](../../../../../api/SIMD-API/基础API/资源管理/TQue/DeQue.md)接口。
...
2.  **设置A、B、Bias的数据类型和格式。**
    ...
    设置示例如下，关于TPosition的详细内容请参考[TPosition](../../../../../api/SIMD-API/基础API/辅助数据结构/TPosition.md)。
...
>- 特别的对于多核场景，需要通过[SetDim](../../../../../api/SIMD-API/高阶API/矩阵计算/Matmul-Tiling类/SetDim.md)接口设置Matmul计算所用的核数，MIX模式（包含矩阵计算和矢量计算）的设置规则如下：
...
>    - 在host侧Tiling实现时，设置总的workspace的数值大小（包含用户workspace和系统workspace），workspace空间由框架来申请并管理。系统workspace的空间大小通过[GetLibApiWorkSpaceSize](../../../../../api/Utils-API/平台信息获取/PlatformAscendC/GetLibApiWorkSpaceSize.md)获取。
>        ```
...
>    - 使用[设置Kernel类型接口](../../../../../api/SIMD-API/基础API/Kernel-Tiling/设置Kernel类型.md)设置Kernel类型为KERNEL\_TYPE\_MIX\_xxx，同时启用AIV核和AIC核。
>    ```
