...
1.  明确算子的数学表达式及计算逻辑。
    ...
    计算逻辑是：Ascend C提供的[矢量计算接口](../../../../api/SIMD-API/基础API/Memory矢量计算/基础算术/Add.md)的操作元素都为[LocalTensor](../../../../api/SIMD-API/基础API/数据结构/LocalTensor/LocalTensor.md)，输入数据需要先从外部存储（Global Memory）搬运进片上存储（Unified Buffer），然后使用计算接口完成两个输入参数相加，得到最终结果，再搬出到外部存储上。Ascend C  Add算子的计算逻辑如下图所示。
...
4.  确定算子实现所需接口。
    -   实现涉及外部存储和内部存储间的数据搬运，查看Ascend C  API参考中的数据搬运接口，需要使用[DataCopy](../../../../api/SIMD-API/基础API/数据搬运导览/概览/数据搬运概念.md)来实现数据搬运。
    -   本样例只涉及矢量计算的加法操作，查看Ascend C  API参考中的[矢量计算](../../../../api/SIMD-API/基础API/Memory矢量计算/Memory矢量计算.md)接口，初步分析可使用基础算术Add接口[Add](../../../../api/SIMD-API/基础API/Memory矢量计算/基础算术/Add.md)实现x+y。
    -   使用[Queue队列](../../../../api/SIMD-API/基础API/资源管理/TQue/TQue简介.md)管理计算中使用的Tensor数据结构，具体使用[EnQue](../../../../api/SIMD-API/基础API/资源管理/TQue/EnQue.md)、[DeQue](../../../../api/SIMD-API/基础API/资源管理/TQue/DeQue.md)等接口。
...
-   任务间交互使用到的内存、临时变量的内存统一使用[Pipe内存管理对象](../../../../api/SIMD-API/基础API/资源管理/TPipe/TPipe.md)进行管理。
...
-   设置输入输出Global Tensor的Global Memory内存地址。
    ...
    本样例中的分配方案是：数据整体长度TOTAL\_LENGTH为1 \* 2048，使用GlobalTensor类的[SetGlobalBuffer](../../../../api/SIMD-API/基础API/数据结构/GlobalTensor/SetGlobalBuffer.md)接口设定该核上Global Memory的起始地址以及长度。

    ```
    xGm.SetGlobalBuffer((__gm__ half *)x, TOTAL_LENGTH);
    ```

-   通过[Pipe内存管理对象](../../../../api/SIMD-API/基础API/资源管理/TPipe/TPipe.md)为输入输出Queue分配内存。
...
<a id="copyin-implementation"></a> 
    ...
    1.  使用[DataCopy](../../../../api/SIMD-API/基础API/数据搬运导览/概览/数据搬运概念.md)接口将GlobalTensor数据拷贝到LocalTensor。
    2.  使用[EnQue](../../../../api/SIMD-API/基础API/资源管理/TQue/EnQue.md)将LocalTensor放入UB（VECIN）的Queue中。
...
2.  Stage2：Compute实现。
    ...
    1.  使用[DeQue](../../../../api/SIMD-API/基础API/资源管理/TQue/DeQue.md)从UB（VECIN）中取出LocalTensor。
    2.  使用Ascend C接口[Add](../../../../api/SIMD-API/基础API/Memory矢量计算/基础算术/Add.md)完成矢量计算。
    3.  使用[EnQue](../../../../api/SIMD-API/基础API/资源管理/TQue/EnQue.md)将计算结果LocalTensor放入到UB（VECOUT）的Queue中。
    4.  使用[FreeTensor](../../../../api/SIMD-API/基础API/资源管理/TQue/FreeTensor.md)释放不再使用的LocalTensor。
...
<a id="copyout-implementation"></a>
    ...
    1.  使用[DeQue](../../../../api/SIMD-API/基础API/资源管理/TQue/DeQue.md)接口从UB（VECOUT）的Queue中取出LocalTensor。
    2.  使用[DataCopy](../../../../api/SIMD-API/基础API/数据搬运导览/概览/数据搬运概念.md)接口将LocalTensor拷贝到GlobalTensor上。
    3.  使用[FreeTensor](../../../../api/SIMD-API/基础API/资源管理/TQue/FreeTensor.md)将不再使用的LocalTensor进行回收。
