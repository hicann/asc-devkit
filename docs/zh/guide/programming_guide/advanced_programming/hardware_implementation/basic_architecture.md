# 基本架构<a name="ZH-CN_TOPIC_0000002380477945"></a>
...
-   AIC
    ...
    在AI Core[分离模式](#li188191010204418)下，一组Cube Core和Vector Core组合中的Cube Core。

-   AIV

    在AI Core[分离模式](#li188191010204418)下，一组Cube Core和Vector Core组合中的Vector Core。

## AI Core的工作模式<a name="section1574769433"></a>

-   <a name="li188191010204418"></a>分离模式
    ...
    **图1**  分离模式示意图（N的取值以硬件平台信息获取接口获取的数值为准）<a name="fig4925816477"></a>  
    ![](../../../figures/sep_mode.png "分离模式示意图（N的取值以硬件平台信息获取接口获取的数值为准）")

-   <a name="li1414517184416"></a>耦合模式

    AI Core的一种工作模式，矩阵计算单元、矢量计算单元对应同一个Scalar调度单元，部署在一个AI Core上。

    **图2**  耦合模式示意图<a name="fig583116255714"></a>  
    ![](../../../figures/coupled_mode.png "耦合模式示意图")
...
-   Cube
    ...
    **图3**  Cube计算单元数据访问<a name="fig643417174212"></a>  
    ![](../../../figures/cube_access.png "Cube计算单元数据访问")
...
-   Scalar
    ...
    **图5**  Scalar对指令和数据的访问<a name="fig9952125415559"></a>  
    ![](../../../figures/scalar_access.png "Scalar对指令和数据的访问")

    >[!NOTE]说明 
    >硬件提供L2Cache用于缓存访问GM的数据（包括代码段、数据段），以此加快访问速度，提高访问效率。核外L2Cache以Cache Line为单位加载数据，根据硬件规格不同，Cache Line大小不同（128/256/512Byte等）。

## 存储单元和搬运单元<a name="section123639375417"></a>
...
<tbody><tr id="row892518618215"><td class="cellrowborder" valign="top" width="30.11%" headers="mcps1.2.3.1.1 "><p id="p1292586202112"><a name="p1292586202112"></a><a name="p1292586202112"></a>L1 Buffer</p>
</td>
<td class="cellrowborder" valign="top" width="69.89%" headers="mcps1.2.3.1.2 "><p id="p1492517613219"><a name="p1492517613219"></a><a name="p1492517613219"></a>L1缓冲区，通用内部存储，是AI Core内比较大的一块数据中转区，可暂存Cube计算单元需要反复使用的一些数据从而减少从总线读写的次数。</p>
</td>
</tr>
<tr id="row1925146182113"><td class="cellrowborder" valign="top" width="30.11%" headers="mcps1.2.3.1.1 "><p id="p1192512632115"><a name="p1192512632115"></a><a name="p1192512632115"></a>L0A Buffer / L0B Buffer</p>
</td>
<td class="cellrowborder" valign="top" width="69.89%" headers="mcps1.2.3.1.2 "><p id="p192611610219"><a name="p192611610219"></a><a name="p192611610219"></a>Cube指令的输入。</p>
</td>
</tr>
<tr id="row1192610642119"><td class="cellrowborder" valign="top" width="30.11%" headers="mcps1.2.3.1.1 "><p id="p109268610212"><a name="p109268610212"></a><a name="p109268610212"></a>L0C Buffer</p>
</td>
...
<td class="cellrowborder" valign="top" width="69.89%" headers="mcps1.2.3.1.2 "><p id="p7587111381113"><a name="p7587111381113"></a><a name="p7587111381113"></a><span>BiasTable</span><span> Buffer，</span><span>存放矩阵计算中的</span><span>Bias。</span></p>
</td>
</tr>
<tr id="row13135416151111"><td class="cellrowborder" valign="top" width="30.11%" headers="mcps1.2.3.1.1 "><p id="p17491298118"><a name="p17491298118"></a><a name="p17491298118"></a><span>FP Buffer</span></p>
</td>
<td class="cellrowborder" valign="top" width="69.89%" headers="mcps1.2.3.1.2 "><p id="p1213671651111"><a name="p1213671651111"></a><a name="p1213671651111"></a><span>Fixpipe</span><span> Buffer，</span><span>存放量化参数、</span><span>Relu</span><span>参数等</span><span>。</span></p>
</td>
</tr>
</tbody>
</table>
...
<tbody><tr id="row199091854122011"><td class="cellrowborder" valign="top" width="30.049999999999997%" headers="mcps1.2.3.1.1 "><p id="p990975412200"><a name="p990975412200"></a><a name="p990975412200"></a>MTE1</p>
</td>
<td class="cellrowborder" valign="top" width="69.95%" headers="mcps1.2.3.1.2 "><p id="p69911469917"><a name="p69911469917"></a><a name="p69911469917"></a>负责如下通路的数据搬运：</p>
<a name="ul15750125414917"></a><a name="ul15750125414917"></a><ul id="ul15750125414917"><li>L1-&gt;L0A/L0B</li><li><span>L1-</span><span>&gt;</span><span>BT Buffer</span></li></ul>
</td>
</tr>
<tr id="row83637216"><td class="cellrowborder" valign="top" width="30.049999999999997%" headers="mcps1.2.3.1.1 "><p id="p16440784214"><a name="p16440784214"></a><a name="p16440784214"></a>MTE2</p>
</td>
<td class="cellrowborder" valign="top" width="69.95%" headers="mcps1.2.3.1.2 "><p id="p12680536151314"><a name="p12680536151314"></a><a name="p12680536151314"></a>负责如下通路的数据搬运：</p>
<a name="ul1420163841318"></a><a name="ul1420163841318"></a><ul id="ul1420163841318"><li>GM-&gt;{L1, L0A/B}，在该通路下，基于分形大小搬运，搬运时满足Cache Line大小对齐，性能更优。</li><li>GM-&gt;UB，基于Cache Line大小搬运性能更优。</li></ul>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="69.95%" headers="mcps1.2.3.1.2 "><p id="p18986147134310"><a name="p18986147134310"></a><a name="p18986147134310"></a>负责如下通路的数据搬运，搬运过程中可以完成随路数据格式/类型转换：</p>
<a name="ul189261456194111"></a><a name="ul189261456194111"></a><ul id="ul189261456194111"><li>L0C-&gt;{GM/L1}</li><li>L1-&gt;FP Buffer</li></ul>
</td>
</tr>
</tbody>
</table>
...
>[!NOTE]说明 
>- 不同类型的AI处理器，存储单元大小不同，开发者可通过[GetCoreMemSize](../../../../api/Utils-API/platform_info/PlatformAscendC/GetCoreMemSize.md)接口获取。
>- 所有通过搬运单元读写GM的数据都缺省被缓存在L2Cache，以此加快访问速度，提高访问效率。核外L2Cache以Cache Line为单位加载数据，根据硬件规格不同，Cache Line大小不同（128/256/512Byte等）。

## 典型的数据流<a name="section147001014511"></a>

-   Vector计算典型的数据流如下：

    GM → UB → Vector → UB → GM

    ![](../../../figures/sep_arch_10.png)
...
## 典型的指令流<a name="section1296816241258"></a>

多条指令从系统内存通过总线接口进入到ICache\(Instruction Cache\)中，后续的指令执行过程，根据指令的类型，有两种可能：

-   如果指令是Scalar指令，指令会被Scalar单元直接执行。
-   其他指令会被Scalar单元顺序发射到独立的分类队列（Vector指令队列、Cube指令队列、MTE1/MTE2/MTE3指令队列等），然后再被对应的执行单元执行。

**图7**  指令分类处理机制<a name="fig1993349810"></a>  
![](../../../figures/instr_class.png "指令分类处理机制")

各指令队列存在队列深度限制。队列深度表示同一指令队列中可缓存的待执行指令数量。当其中任一待执行指令（Vector指令队列、Cube指令队列、MTE1/MTE2/MTE3指令队列等）数量达到队列深度时，对应的Scalar单元如果再次向该指令队列发射指令会被阻塞，导致Scalar单元无法继续顺序发射后续指令，可能影响整体流水并行效率，[NPU架构版本2201](../../language_extension/simd_builtin_keywords.md)各指令队列深度参数参考[指令队列深度](./architecture_spec/npu_arch_2201.md#指令序列深度)，[NPU架构版本3510](../../language_extension/simd_builtin_keywords.md)各指令队列深度参数参考[指令队列深度](./architecture_spec/npu_arch_3510.md#指令序列深度)。其中各待执行指令队列一般包含三类指令：流水线待执行指令、流水线同步指令、搭配指令使用的寄存器设置指令。若待执行指令队列满影响到后续流水并行效率，可尝试减少对应指令队列里的指令数，缓解Scalar单元发射指令的阻塞状态；或者尝试重新编排受影响的并行流水线，将受影响的流水线指令发射提前。

同一个指令队列中的指令是按照进入指令队列的顺序执行的，不同指令队列之间可以并行执行，通过多个指令队列的并行执行可以提升整体执行效率。对于并行执行过程中可能出现的数据依赖，通过事件同步模块插入同步指令来控制流水线的同步，提供PipeBarrier、SetFlag/WaitFlag两种API，保证队列内部以及队列之间按照逻辑关系执行。
