...
- 默认安装到`$ASCEND_OPP_PATH/vendors/<vendor_name>`的场景下，算子包优先级根据`opp/vendors/config.ini`中的`load_priority`决定，排在前面的算子包优先级更高。
- 动态库/静态库集成场景的加载方式与RUN包不同，应用直接链接或通过`ASCEND_CUSTOM_OPP_PATH`配置动态库时，应按[算子动态库和静态库编译](../compilation_and_deployment/dynamic_static_lib_compilation.md)中的优先级规则处理。
