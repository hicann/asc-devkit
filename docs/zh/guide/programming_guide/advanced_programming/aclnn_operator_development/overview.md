...
| 名称 | 含义 |
|------|------|
| 自定义算子工程 | 根据算子原型定义编写或者生成的工程骨架，包含原型定义、Tiling实现、Kernel实现和CMake编译配置。 |
| 算子原型 | 描述算子的输入、输出、属性、数据类型、格式、Shape推导等信息。 |
| Host侧 | 运行在CPU侧的准备逻辑，例如Shape推导、Tiling计算、workspace大小设置。 |
| Kernel侧 | 运行在AI Core上的计算逻辑，是算子真正执行计算的部分。 |
| Tiling | 根据输入Shape、数据类型等信息切分计算任务，并把Kernel需要的参数传递到Kernel侧。 |
| aclnn单算子API | 算子工程编译后基于算子原型自动生成的C语言API，应用程序可通过该接口直接调用自定义算子。 |

### 工程化开发是什么

工程化算子开发是指基于**自定义算子工程**完成算子实现、编译部署和调用验证的开发方式，主要面向CANN相关场景对接：单算子调用场景通过aclnn API完成接入，算子入图场景通过GE图执行链路完成接入。通过工程化开发，算子原型、Host侧Tiling、Kernel实现和编译部署配置可以在两类场景中最大化复用同一份交付件。相比手动组织编译流程和分别对接调用链路，工程化开发由msOpGen生成工程骨架，CMake管理编译，编译产物自动包含aclnn API以及入图所需的原型定义等文件，部署时只需安装.run包或者链接静态/动态库。
...
`[基础]`（按顺序阅读）：
- [算子功能设计](./design_and_implementation/operator_function_design.md) — 支持范围、场景划分、Kernel组织。
...
- [Kernel侧算子实现](./design_and_implementation/kernel_operator_implementation.md) — 核函数编程与API使用。

`[扩展]`（遇到具体问题时查阅）：
- [通过TilingData传递属性信息](./design_and_implementation/tiling_data_attributes.md) — 适用于算子属性需要传递到Kernel侧的场景。
- [多分支策略](./design_and_implementation/multi_branch_strategy.md) — 适用于算子需要按场景拆分Kernel入口的场景。
...
- [命名转换规则对照表](./appendix/naming_conversion_table.md) — 算子类型名、文件名、核函数名的转换规则。
