...
| 类型 | 代表接口 | 典型用途 |
| --- | --- | --- |
| 同步屏障 | `asc_syncthreads` | Thread Block内阶段同步，例如先写Unified Buffer（UB），再统一读取。 |
...
- 多个Thread并发写同一地址时，同步屏障和内存栅栏都不能替代原子操作；需要避免写冲突，可使用[原子操作](atomic_operations.md)保证读改写过程的原子性。
