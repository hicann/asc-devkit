...
![](../../../../figures/pipe_parallel.png "流水线并行示意图")

结合上一章的阐述可总结出：Ascend C流水线体系由两大核心模块共同支撑：
- **[TPipe](../../../../../api/SIMD-API/basic_api/resource_management/TPipe/TPipe.md)** 资源管理器：统一管理系统内存资源和用于同步的事件等；
- **[TQue](../../../../../api/SIMD-API/basic_api/resource_management/TQue/TQue.md)** 队列：完成Stage任务之间的同步和通信。

基于这套机制，Ascend C固化了矢量、矩阵、融合三类算子的标准化流水线编程范式。下文将结合硬件特性与工程实例逐一展开。

-   **[矢量编程范式](./tpipe_tque_paradigm.md#section116515238815)**
-   **[矩阵编程范式](./tpipe_tque_paradigm.md#section8213173433312)**
-   **[融合算子编程范式](./tpipe_tque_paradigm.md#section57815481855)**
...
任务间数据传递所使用的内存、事件等资源，统一由TPipe管理。如下内存管理示意图所示，TPipe通过[InitBuffer](../../../../../api/SIMD-API/basic_api/resource_management/TPipe/InitBuffer.md)接口对外提供队列内存初始化功能，开发者可通过该接口为指定队列分配内存。

队列内存初始化完成后，当需要内存时通过[AllocTensor](../../../../../api/SIMD-API/basic_api/resource_management/TQue/AllocTensor.md)为LocalTensor分配内存；当该LocalTensor完成相关计算不再使用时，再调用[FreeTensor](../../../../../api/SIMD-API/basic_api/resource_management/TQue/FreeTensor.md)回收其内存。

**图2**  内存管理示意图<a name="fig375042942717"></a>  
![](../../../../figures/mem_mgmt.png "内存管理示意图")

编程过程中使用的临时变量内存同样由TPipe管理。临时变量可使用TBuf数据结构申请指定TPosition上的存储空间。通过TBuf申请的内存仅能参与计算，不可执行队列的入队、出队操作。具体接口使用说明请参见[TBuf](../../../../../api/SIMD-API/basic_api/resource_management/TBuf/TBuf.md)。
...
![](../../../../figures/pipe_task_run.png "流水任务运行示意图")

## 矩阵编程范式<a name="section8213173433312"></a>

Cube计算的典型数据流图如下所示：

![](../../../../figures/mat_pos_2.png)
