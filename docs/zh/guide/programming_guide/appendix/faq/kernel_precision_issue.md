# 核函数运行验证时算子存在精度问题<a name="ZH-CN_TOPIC_0000001664188149"></a>
...
1.  进行CPU域的功能验证，观察是否有日志报错。
    ...
    参考[基于样例工程完成Kernel直调](../kernel_direct_call_from_sample.md)章节，编写CPU侧的运行验证代码，并进行运行验证，发现CPU域的精度比对也存在不一致的问题。
