...
如果算子带有标量、维度、阈值、开关等配置项，需要在功能设计阶段确定它们的承载方式。值较小且用于描述算子行为的配置项适合作为属性；需要按Tensor传入、参与动态图数据依赖或数据量较大的内容适合作为输入。属性识别和传递的完整示例请参考[通过TilingData传递属性信息](./Host侧Tiling实现/通过TilingData传递属性信息.md)。
...
将前两步确定的输入输出、dtype/format列表等合并，即得到完整的算子原型定义。完整的算子原型定义编写方法和API说明请参考[算子原型定义](./算子原型定义.md)。
...
TilingData结构体、Tiling函数写法、workspace设置、`numBlocks`配置和相关launch参数会在[Host侧Tiling实现](./Host侧Tiling实现/基本流程.md)章节详细展开。
...
功能设计阶段只需要明确这条计算映射和基础数据流。Kernel入口参数、TilingData读取等代码组织细节，会在[Kernel侧算子实现](./Kernel侧算子实现/基本流程.md)章节展开。

## 针对不同场景扩展Kernel实现

完成基础Kernel设计后，还需要判断当前实现能覆盖哪些场景。后续若需要支持更多数据类型、更大shape范围或运行时配置时，需要在基础实现上增加扩展策略。功能设计阶段需要先判断扩展维度和触发条件，后续再通过[多分支策略](./多分支策略.md)说明TilingKey、Kernel模板或差异化编译配置的具体写法。
...
关于TilingKey和Kernel模板的选择准则，请参考[多分支策略](./多分支策略.md)。

## 下一步

  - [算子原型定义](./算子原型定义.md)：输入、输出、属性、dtype/format。
  - [Host侧Tiling实现](./Host侧Tiling实现/基本流程.md)：TilingData定义和Tiling函数编写。
  - [Kernel侧算子实现](./Kernel侧算子实现/基本流程.md)：数据搬运、计算接口和Kernel编程流程。
  - [多分支策略](./多分支策略.md)：TilingKey和不同Kernel分支的选择方法。
  - [通过TilingData传递属性信息](./Host侧Tiling实现/通过TilingData传递属性信息.md)：属性读取和传递介绍。
