...
如果算子带有标量、维度、阈值、开关等配置项，需要在功能设计阶段确定它们的承载方式。值较小且用于描述算子行为的配置项适合作为属性；需要按Tensor传入、参与动态图数据依赖或数据量较大的内容适合作为输入。属性识别和传递的完整示例请参考[通过TilingData传递属性信息](./通过TilingData传递属性信息.md)。
...
TilingData结构体、Tiling函数写法、workspace设置、`numBlocks`配置和相关launch参数会在[Host侧Tiling实现](./Host侧Tiling实现.md)章节详细展开。
...
功能设计阶段只需要明确这条计算映射和基础数据流。Kernel入口参数、TilingData读取等代码组织细节，会在[Kernel侧算子实现](./Kernel侧算子实现.md)章节展开。
...
## 下一步
  ...
  - [Host侧Tiling实现](./Host侧Tiling实现.md)：TilingData定义和Tiling函数编写。
  - [Kernel侧算子实现](./Kernel侧算子实现.md)：数据搬运、计算接口和Kernel编程流程。
  - [多分支策略](./多分支策略.md)：TilingKey和不同Kernel分支的选择方法。
  - [通过TilingData传递属性信息](./通过TilingData传递属性信息.md)：属性读取和传递介绍。
