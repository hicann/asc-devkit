...
使用[REGISTER_TILING_FOR_TILINGKEY](../../../../../api/SIMD-API/基础API/Kernel-Tiling/REGISTER_TILING_FOR_TILINGKEY.md)时，必须同时通过[REGISTER_TILING_DEFAULT](../../../../../api/SIMD-API/基础API/Kernel-Tiling/REGISTER_TILING_DEFAULT.md)注册默认结构体。Host侧也需要在key 2分支中调用`GetTilingData<TilingData1>()`写入数据，保证两侧类型和字段布局一致。
...
关于编译选项的完整说明见[编译与部署基本流程](../编译与部署/基本流程.md#custom-compile-options)。
...
Kernel模板编程常用宏如下，具体API参考见[模板参数定义](../../../../../api/Utils-API/Tiling模板编程/模板参数定义.md)：
...
- [算子功能设计](./算子功能设计.md)：判断是否需要按dtype、shape或运行配置拆分Kernel。
- [Host侧Tiling实现](./Host侧Tiling实现.md)：设置TilingData、TilingKey和Kernel启动参数。
- [Kernel侧算子实现](./Kernel侧算子实现.md)：注册和解析不同TilingData结构体。
- [TILING_KEY_IS](../../../../../api/SIMD-API/基础API/Kernel-Tiling/TILING_KEY_IS.md)：Kernel侧分支判断接口。
- [ASCENDC_TPL_SEL_PARAM](../../../../../api/Utils-API/Tiling模板编程/ASCENDC_TPL_SEL_PARAM.md)：Host侧模板参数选择接口。
