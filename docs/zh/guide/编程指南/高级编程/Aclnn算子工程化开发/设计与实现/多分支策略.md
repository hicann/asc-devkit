...
使用`REGISTER_TILING_FOR_TILINGKEY`时，必须同时通过`REGISTER_TILING_DEFAULT`注册默认结构体。Host侧也需要在key 2分支中调用`GetTilingData<TilingData1>()`写入数据，保证两侧类型和字段布局一致。
...
关于编译选项的完整说明见[编译与部署基本流程](./算子功能设计.md)。
...
- [Host侧Tiling实现](./Host侧Tiling实现/基本流程.md)：设置TilingData、TilingKey和Kernel启动参数。
- [Kernel侧算子实现](./Kernel侧算子实现/基本流程.md)：注册和解析不同TilingData结构体。
