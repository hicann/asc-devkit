...
> - 指定目录安装后，在使用算子前需要执行`source <path>/vendors/<vendor_name>/bin/set_env.bash`，将安装路径追加到`ASCEND_CUSTOM_OPP_PATH`环境变量。
...
1.  使用**npu\_op\_kernel\_options**添加算子编译选项，支持的选项以当前编译框架能力为准。
2.  使用**npu\_op\_kernel\_sources**指定算子特定目录与编译源文件。
    ...
    -   若算子的Kernel实现cpp文件需要自定义命名，需同时指定`OP_TYPE`（算子类型）和`KERNEL_FILE`（Kernel实现cpp文件名），以配置两者之间的对应关系。不配置时，Kernel实现cpp文件名和OpType之间需满足转换规则。
...
-   `--tiling_key`，设置该选项后，只编译指定的[TilingKey](../Host侧Tiling实现/基本流程.md)相关的Kernel代码，用于加速编译过程。若不指定TilingKey编译，则默认编译所有的TilingKey。配置多个TilingKey时，TilingKey之间不能有空格。示例如下，其中1、2为TilingKey取值。
