...
完成算子代码实现后，如果编译平台和运行平台架构一致，直接参考[基本流程](基本流程.md)编译即可。如需交叉编译（编译平台架构与运行平台架构不同），参考本文档。
...
```
安装后，按照安装日志提示给LD_LIBRARY_PATH设置环境变量，之后调用算子的使用方式与普通编译部署一致，参考[算子包调用](../Aclnn算子工程化开发快速入门.md#部署)。

## 相关文档

- [基本流程](基本流程.md) — 标准编译与部署流程。
- [编译过程调试](编译过程调试.md) — 编译问题排查方法。
