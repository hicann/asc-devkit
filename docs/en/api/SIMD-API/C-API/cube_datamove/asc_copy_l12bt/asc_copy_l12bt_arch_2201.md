...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-08-28T10:16:51.524Z pushedAt=2026-08-29T07:21:05.192Z -->
...
Copies data from the L1 Buffer to the BiasTable Buffer, which stores the bias used in matrix computation.
...
- High-dimensional split transfer
    ```cpp
...
| conv_control | Input | Whether to enable in-path conversion. |
...
| source_gap | Input | Gap between adjacent consecutive data blocks of the source operand (the gap between the tail of the preceding data block and the head of the following data block).<br>Unit: DataBlock (32 bytes). |
| dst_gap | Input | Gap between adjacent consecutive data blocks of the destination operand (the gap between the tail of the preceding data block and the head of the following data block).<br>Unit: DataBlock (32 bytes). |
...
- When the first-n-data-block copy API is used, the size of the data to be copied must be 32-byte aligned.
