...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-08-28T10:14:53.701Z pushedAt=2026-08-29T07:20:58.682Z -->
...
After matrix computation is complete, the result is quantized and then moved to the GM. There are two quantization parameters: **quant_pre** and **quant_post**, corresponding to the pre-processing and post-processing stages, respectively.
...
|:-----------------|:------|:---------------------------------------------------------------------------------------------------------------------------------|
| **dst_addr**              | Output    | Start address of the destination operand (vector).                                                                                                                  |
| **src_addr**              | Input    | Start address of the source operand (vector).                                                                                                                   |
...
| **loop_dst_stride**       | Input    | <br> - When the NZ2ND function is not enabled, the start address offset between adjacent Z layouts in the destination NZ matrix, with a non-zero value, unit: element.<br> - When the NZ2ND/NZ2DN function is enabled, the number of elements in each row of the destination ND matrix, with a non-zero value, unit: element.  |
...
| **l2_cache_ctl** | Input | Configures the management policy of data in the L2 Cache. The values are described as follows:  <br>&bull; 0: DISABLE mode, applicable to data that needs to be accessed only once. <br>&bull; 1: NORMAL mode, applicable to data whose reuse pattern is unknown or not extreme. <br>&bull; 2: LAST mode, applicable to data that is accessed frequently and repeatedly. <br>&bull; 4: PERSISTENT mode, applicable to data that needs to reside in the cache for a long time. |
| **clip_relu_pre**    | Input    | Enables clip_relu in the pre-processing stage. It must be used together with normal relu (normalized relu function) and requires the quantization function to be enabled.                                                                        |
| **unit_flag_ctl**   | Input    | Related to the **unit_flag** parameter. The values are as follows:<br/>&bull; 0: Reserved value;<br/>&bull; 2: Enables **unit_flag**. After the hardware finishes executing the instruction, the register is not set;<br/>&bull; 3: Enables **unit_flag**. After the hardware finishes executing the instruction, **unit_flag** is turned off. |
...
| **split_en**    | Input    | Whether to enable the channel splitting function. The default value is **false**, which means the function is not enabled. Channel splitting can be enabled only when both **src_addr** and **dst_addr** are of the float type, and **split_en** and the NZ2ND function cannot be enabled at the same time.                                               |
...
| **eltwise_op**       | Input    | Defines the destination operand address and channel stride when data is moved from l0c to gm.                                                                                                     |
...
| **broadcast_en**        | Input    | Whether to enable the broadcast capability.<br/>&bull; **false**: Not enabled;<br/>&bull; **true**: Enabled, broadcasting data along the M axis during data movement.                                                                                                         |
...
```cpp
// dst is an externally input GM address of the float type.
