...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-04T11:27:45.973Z pushedAt=2026-09-05T12:14:28.951Z -->
...
Converts data of the uint32_t type to the uint16_t type.

Depending on the size of the input data, the conversion is performed in non-saturation mode or saturation mode.
- Non-saturation mode: The result is truncated to the data width of the target format, and the least significant bits (LSB) are retained.
- Saturation mode: The result is limited to the representable range of the target format.

Because the bit-width ratio between the source operand and the destination operand is 2:1, the data of one VL size must be divided into two parts when written, and index 0 or index 1 is selected according to the API.

## Prototype

```cpp
// Non-saturation mode, data is written to the position with index 0.
__simd_callee__ inline void asc_uint322uint16(vector_uint16_t& dst, vector_uint32_t src, vector_bool mask)
// Saturation mode, data is written to the position with index 0.
__simd_callee__ inline void asc_uint322uint16_sat(vector_uint16_t& dst, vector_uint32_t src, vector_bool mask)
// Non-saturation mode, data is written to the position with index 1.
__simd_callee__ inline void asc_uint322uint16_v2(vector_uint16_t& dst, vector_uint32_t src, vector_bool mask)
// Saturation mode, data is written to the position with index 1.
