...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-04T09:15:17.658Z pushedAt=2026-09-05T12:14:28.796Z -->
...
Converts data of the bfloat16_t type to int32_t and supports multiple rounding modes.
...
- Non-saturation mode:
  - When the input data exceeds the maximum/minimum value of the output type, the result is truncated to the data width of the target format.
  - When the input data is +/-inf, the maximum/minimum value of the output type is returned.
  - When the input data is nan, 0 is returned.
- Saturation mode:
  - When the input data exceeds the maximum/minimum value of the output type, the corresponding maximum/minimum value of the output type is returned.
  - When the input data is +/-inf, the maximum/minimum value of the output type is returned.
  - When the input data is nan, 0 is returned.

Because the bit-width ratio between the source operand and the destination operand is 1:2, when reading data, one VL-sized data block is divided into two parts, and index 0 or index 1 is selected based on the API.
...
- When either saturation mode or non-saturation mode is enabled, the ctrl register must be configured. For details about the ctrl register, see [asc_set_ctrl.md](../../sys_var/asc_set_ctrl.md).
