...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-04T09:51:18.058Z pushedAt=2026-09-05T12:14:28.836Z -->
...
Greater than or equal to (ge) performs an element-wise comparison on the source operand. For src_i >= value, if the condition holds, the corresponding result bit of the destination operand is set to 1; otherwise, it is set to 0. The comparison result of each element occupies one bit.
...
| mask | Input | Source operand mask (mask register), which indicates the elements that participate in the computation. An element participates in the computation when its corresponding bit is 1, and does not participate when the bit is 0. Elements not selected by mask are set to 0 in the output. |
