...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-04T10:49:38.549Z pushedAt=2026-09-05T12:14:28.906Z -->
...
Performs an element-wise minimum operation on the source operands **src0** and **src1** according to **mask**, and writes the result to the destination operand **dst**.
...
| mask | Input | Source operand mask (mask register), which indicates the elements that participate in the calculation. An element participates in the calculation when its corresponding position is 1, and does not participate when it is 0. Elements not selected by **mask** are set to zero in the output. |
...
asc_loadalign(src0, src0_addr); // src0_addr is the UB memory space address input externally.
asc_loadalign(src1, src1_addr); // src1_addr is the UB memory space address input externally.
