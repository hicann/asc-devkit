...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-04T11:13:58.261Z pushedAt=2026-09-05T12:14:28.935Z -->
...
Performs a left shift on the input data **src0** according to the corresponding elements of **src1** under the mask, and writes the result to **dst**.
...
- When the data type is unsigned, a logical left shift is performed. A logical left shift moves the binary number to the left by the specified number of bits, discards the most significant bit, and fills the least significant bit with 0.
...
- When the data type is signed, an arithmetic left shift is performed. An arithmetic left shift moves the binary number to the left by the specified number of bits, fills the most significant bit with the original sign bit, discards the second most significant bit, and fills the least significant bit with 0.
- For an arithmetic shift (signed data type), if **src0** is less than 0, **src1** is less than 0, and the shift amount of **src1** is greater than the bit width of the data type, the output is -1.
