...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-05T15:00:15.354Z pushedAt=2026-09-08T03:45:05.558Z -->
...
When the result is output to a register, call [asc_get_cmp_mask](asc_get_cmp_mask.md) after the computation is complete to obtain the comparison result.

## Prototype

- High-dimensional split computation.
    ```cpp
    // Output the result to a register.
...
| **dst_block_stride** | Input | Address stride between different DataBlocks within a single iteration of **dst**. |
| **src0_block_stride** | Input | Address stride between different DataBlocks within a single iteration of **src0**. |
| **src1_block_stride** | Input | Address stride between different DataBlocks within a single iteration of **src1**. |
| **dst_repeat_stride** | Input | Address stride of the same DataBlock between adjacent iterations of **dst**. |
| **src0_repeat_stride** | Input | Address stride of the same DataBlock between adjacent iterations of **src0**. |
| **src1_repeat_stride** | Input | Address stride of the same DataBlock between adjacent iterations of **src1**. |
...
- When the register-based API is used, [asc_get_cmp_mask](asc_get_cmp_mask.md) must be used to obtain the comparison result.
- When the result is output to a register, 256B of data is processed by default, and the **repeat** field does not take effect.
- When the result is output to a register, if a bit of **mask** is 0 and the value of the corresponding bit of the output register does not change, the final result may retain residue from the previous computation. To initialize the register, call the [asc_set_cmp_mask](asc_set_cmp_mask.md) API.
