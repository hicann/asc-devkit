...
<!-- md-trans-meta sourceCommit=0ece502ce5e30f12c91b14fc87b28b1b051cb796 translatedAt=2026-09-08T04:58:24.725Z pushedAt=2026-09-08T13:17:42.540Z -->
...
When moving data from Unified Buffer (UB) to Global Memory (GM), call this API to set the data block stride of the source operand between adjacent iterations in the outer loop, as well as the data block stride of the destination operand between adjacent iterations in the outer loop.
...
| :----- | :------- | :------- |
| loop2_src_stride | Input | Sets the stride between the data blocks of the source operand in adjacent iterations of the outer loop, in bytes. The value range is [0, 2^21), and **loop2_src_stride** must be 32B-aligned. |
| loop2_dst_stride | Input | Sets the stride between the data blocks of the destination operand in adjacent iterations of the outer loop, in bytes. The value range is [0, 2^40). |
