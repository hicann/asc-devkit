...
<!-- md-trans-meta sourceCommit=2bc2b38d5648dc61f632186b01ba7f36c2295b13 translatedAt=2026-09-09T15:59:30.201Z -->
...
Determines, element by element, whether the input floating-point number is NaN, and outputs the result as a floating-point number or a Boolean value. When the output is a floating-point type, for input data that is NaN, the result at the corresponding position is 1 of the floating-point type; otherwise, it is 0. When the output is a bool type, for input data that is NaN, the result at the corresponding position is true; otherwise, it is false. The calculation formula is as follows:
...
| --- | --- |
| IsNanConfig | Related configuration of the IsNan algorithm. This parameter is optional and of the **IsNanConfig** type. Its specific definition is shown in the following code, where the parameters have the following meanings:<br><br>**isReuseSource**: Whether the source operand is allowed to be modified. This parameter is reserved. Pass the default value **false**. |
...
| sharedTmpBuffer | Input | Temporary buffer.<br><br>The type is [LocalTensor](../../../basic_api/data_structures/LocalTensor/LocalTensor.md), and the supported TPosition is VECIN/VECCALC/VECOUT.<br><br>Used to store intermediate variables during complex computation inside IsNan, provided by yourself.<br><br>For how to obtain the temporary space size BufferSize, see [GetIsNanMaxMinTmpSize](GetIsNanMaxMinTmpSize.md). |
...
-   Pass the temporary space through the **sharedTmpBuffer** input parameter.
    ...
    pipe.InitBuffer(tmpQue, 1, bufferSize);  // Obtain bufferSize from the tiling parameter on the Host side.
    ...
    static constexpr AscendC::IsNanConfig isNanConfig = { false }; // Do not modify the source operand.
...
-   The API framework allocates temporary space.
    ...
    // The input tensor length is 1024, the operator input data type is half, and the actual number of computed elements is 512.
    static constexpr AscendC::IsNanConfig isNanConfig = { false }; // Do not modify the source operand.
    // dst is a LocalTensor of the bool type, and src is a LocalTensor of the half type.
...
Input data (src): [1.0 nan 3.0 4.0 nan 6.0 nan 8.0]
