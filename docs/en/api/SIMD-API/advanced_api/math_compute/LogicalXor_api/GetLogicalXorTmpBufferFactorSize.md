...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-09-10T09:01:48.035Z -->

## Description

This API is used to obtain **maxLivedNodeCount** and **extraBuf**. With a fixed space size, the maximum number of elements that the operator can compute in a single computation can be derived from **maxLivedNodeCount** and **extraBuf**. **maxLivedNodeCount** indicates how many times the temporary space is the size of the space occupied by the data of a single computation; **extraBuf** indicates the size of the additional temporary space used.

The following is a derivation example:

-   The operator implementation needs to call the **LogicalXor** API. You reserve a space of **currBuff** size for it, and use the **GetLogicalXorTmpBufferFactorSize** API to obtain the output values **maxLivedNodeCount** and **extraBuf**. The maximum number of elements that the operator can compute in a single computation can then be derived as:
