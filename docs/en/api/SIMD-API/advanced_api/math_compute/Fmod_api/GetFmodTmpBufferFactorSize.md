...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-09-09T15:22:57.898Z -->
...
-   The operator implementation needs to call the Fmod API. Reserve a space of **currBuff** size for it, use the **GetFmodTmpBufferFactorSize** API to obtain the output values of **maxLiveNodeCount** and **extraBuf**, and then derive the maximum number of elements that the operator can compute at a time as follows:

    **currentShapeSize = \(currBuff - extraBuf\) / maxLiveNodeCount / typeSize**

-   The operator implementation needs to call two kernel-side APIs **KernelIntf1** and **KernelIntf2**. Using the two sets of output values \(maxLiveNodeCount and extraBuf\) obtained from the two **GetXxxTmpBufferFactorSize** APIs (where Xxx is the two high-level APIs to be called) and the currently available temporary space, derive the maximum number of elements computed at a time, **currentShapeSize**, as follows:
...
uint32_t extraBuf = 0;
AscendC::GetFmodTmpBufferFactorSize(typeSize, maxLiveNodeCount, extraBuf);
```
