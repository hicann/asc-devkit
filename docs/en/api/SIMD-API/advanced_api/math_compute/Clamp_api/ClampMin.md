...
<!-- md-trans-meta sourceCommit=0364dcce9ab16d714270f168ec9546fd7620f90f translatedAt=2026-09-09T13:50:50.105Z -->
...
Replaces the elements in **srcTensor** that are smaller than **scalar** with **scalar**, keeps the elements greater than or equal to **scalar** unchanged, and outputs the result as **dstTensor**.
...
| sharedTmpBuffer | Input | Temporary buffer.<br><br>The type is [LocalTensor](../../../basic_api/data_structures/LocalTensor/LocalTensor.md), and the supported TPosition is VECIN/VECCALC/VECOUT.<br><br>Used to store intermediate variables during complex computation inside ClampMin, provided by yourself.<br><br>For how to obtain the temporary space size BufferSize, see [GetClampMaxMinTmpSize](GetClampMaxMinTmpSize.md). |
| scalar | Input | Scalar data, with the same data type as srcTensor. The supported data types are half and float. |
...
Output data (dstLocal): [2 2 2  ... 126 127]
```
