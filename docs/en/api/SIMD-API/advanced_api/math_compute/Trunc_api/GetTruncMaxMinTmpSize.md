...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-09-10T10:43:30.426Z -->

## Description

The computation of the **Trunc** API on the kernel side requires you to reserve/apply for temporary space. This API is used to obtain the maximum and minimum temporary space sizes to reserve/apply for on the host side. Based on this range, you can select an appropriate space size and pass it to the kernel side as a Tiling parameter.
...
-   Within the range from the minimum to the maximum temporary space size, as the temporary space increases, the kernel-side API computation performance will be optimized to a certain extent. To achieve better performance, you can reserve/apply for space based on the actual memory usage.
...
uint32_t maxValue = 0;
uint32_t minValue = 0;
AscendC::GetTruncMaxMinTmpSize(shape, 2, false, maxValue, minValue);
```
