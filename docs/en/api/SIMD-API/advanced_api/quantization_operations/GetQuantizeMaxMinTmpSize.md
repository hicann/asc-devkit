...
| --- | --- | --- |
| srcShape | Input | Shape information of the input **srcTensor** of the Quantize API. |
| typeSize | Input | Data type size of the input **srcTensor** of the Quantize API, in bytes. For example, if the input data type is half, pass 2 here. |
| maxValue | Output | Maximum temporary space size required for the Quantize API to complete computation. Space exceeding this value is not used by this API. Within the range from the minimum to the maximum temporary space size, as the temporary space increases, the computation performance of the kernel-side API improves to a certain extent. To achieve better performance, you can reserve/allocate space based on the actual memory usage. A maximum space size of 0 indicates that no temporary space is required for computation.<br>**maxValue** is only a reference value and may be larger than the remaining space of the Unified Buffer. In this case, select an appropriate temporary space size based on the remaining space of the Unified Buffer. |
| minValue | Output | Minimum temporary space size required for the Quantize API to complete computation. To ensure functional correctness, the temporary space reserved/allocated during API computation must not be smaller than this value. A minimum space size of 0 indicates that no temporary space is required for computation. |
...
// In the QuantizeParams passed to the Quantize API, m = 2 and n = 1024; the input data type of the operator is int32_t.
