...
<!-- md-trans-meta sourceCommit=93c993b00c4f6138f380f7a2d742660ee1403179 translatedAt=2026-09-09T10:10:20.440Z -->
...
Add is a binary vector computation API that sums two input tensors element by element. The calculation formula is as follows:
...
<!-- npu="950" id10 -->
- For Ascend 950PR/Ascend 950DT, the data types supported by T are: int8_t, uint8_t, int16_t, uint16_t, half, bfloat16_t, int32_t, uint32_t, float, complex32, int64_t, uint64_t, and complex64. The data types int8_t, uint8_t, complex32, int64_t, uint64_t, and complex64 are supported only by the APIs that compute the first n data of a tensor and the operator overloads in which the entire tensor participates in computation.
<!-- end id10 -->
<!-- npu="A3" id11 -->
...
- When using the API symbol overload that involves the entire tensor in computation, the computation amount is the total length of the destination LocalTensor.
...
- Example of computation involving the entire tensor
...
Output data dstLocal: [514 516 518 ... 1536]
```
