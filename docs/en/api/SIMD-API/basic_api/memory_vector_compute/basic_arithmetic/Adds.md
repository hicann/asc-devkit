...
<!-- md-trans-meta sourceCommit=93c993b00c4f6138f380f7a2d742660ee1403179 translatedAt=2026-09-09T10:17:26.677Z -->
...
Adds is a binary scalar computation API. Its inputs are a vector tensor and a scalar. Adds sums each element in the vector tensor with the scalar. The calculation formula is as follows:
...
| src | Input | Source operand.<br>The type is LocalTensor, and the supported TPosition is VECIN/VECCALC/VECOUT.<br>For the address alignment constraint, see [General Address Alignment Constraints](../../../general_description_and_constraints.md).<br>The data type must be the same as that of the destination operand. |
| scalarValue | Input | Source operand. The data type must be the same as the element type of the destination operand. |
...
| repeatTime | Input | Number of repeated iterations. The vector computation unit reads 256 bytes of consecutive data for computation each time. To process the input data, multiple iterations (repeats) are required to complete reading and computing all data. repeatTime indicates the number of iterations.<br>For details about this parameter, see [High-Dimensional Slicing](../SIMD_compute/high_dimension_slicing.md). |
...
- Description of Unified Buffer space usage. For Ascend 950PR/Ascend 950DT:
  ...
  - Continuous computation of the first n data elements of a tensor does not involve occupation of the 8 KB Unified Buffer.
...
- Sample of the API for computing the first n data of a tensor
...
Input data scalar: 2
Output data dstLocal: [3 4 5 ... 514]
```
