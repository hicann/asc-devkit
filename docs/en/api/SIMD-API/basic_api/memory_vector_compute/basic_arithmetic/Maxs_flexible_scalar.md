# Maxs (Flexible Scalar Position)<a name="ZH-CN_TOPIC_0000002134881128"></a>

<!-- md-trans-meta sourceCommit=e5451c7feba11f9e259f4fbc19c4bf82f8c0496c translatedAt=2026-09-09T10:55:04.115Z -->
...
Maxs is a binary scalar computation API that takes the larger value between each element in a vector and a scalar, supporting both scalar-first and scalar-last scenarios. The scalar input supports configuring a LocalTensor single element. The computation formulas are as follows:

$dst_i = \operatorname{Max}(src_i, scalar)$

$dst_i = \operatorname{Max}(scalar, src_i)$
...
| ---- | ---- |
| T | Data type of the operand.<br>For the flexible scalar position API, this is a reserved parameter that is not enabled yet and is reserved for future function extension. When it needs to be specified, pass the default value BinaryDefaultType. |
...
| config | Type: BinaryConfig. It takes effect when the scalar is a LocalTensor single element and is used to specify the position of the single-element operand. The default value DEFAULT_BINARY_CONFIG indicates that the right operand is a scalar.<br>struct BinaryConfig {<br>    int8_t scalarTensorIndex = 1; // Specifies the position of the scalar when the scalar is a LocalTensor single element. 0 indicates the left operand, and 1 indicates the right operand.<br>};<br>constexpr BinaryConfig DEFAULT_BINARY_CONFIG = {1};<br>|
| U | LocalTensor type. The corresponding data type is automatically deduced based on the input parameter dst. The developer does not need to configure this parameter, but only needs to ensure that dst meets the data type constraint. |
| S | LocalTensor type or scalar type. The corresponding data type is automatically deduced based on the input parameter src0. The developer does not need to configure this parameter, but only needs to ensure that src0 meets the data type constraint. |
| V | LocalTensor type or scalar type. The corresponding data type is automatically deduced based on the input parameter src1. The developer does not need to configure this parameter, but only needs to ensure that src1 meets the data type constraint. |
...
| src0/src1 | Input | Source operands in the flexible scalar position API.<br>&bull; When the type is LocalTensor: it can be used as a vector operand or a scalar single element, and the supported TPosition values are VECIN/VECCALC/VECOUT. For the address alignment constraints, see [address alignment constraints](../../../general_description_and_constraints.md). <br>&bull; When the type is scalar. |
...
<!-- npu="950" id8 -->
For Ascend 950PR/Ascend 950DT, the supported data types are int8_t, uint8_t, int16_t, half, bfloat16_t, int32_t, float, int64_t, and uint64_t. Among them, the int8_t, uint8_t, int64_t, and uint64_t data types are supported only by the API that computes the first n data of the tensor.
<!-- end id8 -->
...
- When the flexible scalar position API is called and the source operand is a single element of a LocalTensor, address overlap with the destination operand is not supported.
- At least one of the left operand and the right operand must be a vector. Having both the left and right operands as scalars is not supported.
- When a single element of a LocalTensor is passed as a scalar to this API, the idx parameter must be a compile-time constant. If a variable is passed, it must be declared as constexpr.

<!-- npu="950" id9 -->
- For Ascend 950PR/Ascend 950DT: This API is implemented for compatibility by calling the [Reg Vector Computation API](../../reg_vector_compute/reg_vector_computation.md) through VF. When the count or repeatTime parameter is set to 0, the software simulation behavior does not guarantee that this API is treated as a NOP (no operation).
- Description of UB space usage. For Ascend 950PR/Ascend 950DT:
  ...
  - Continuous computation of the first n data elements of a tensor does not involve the occupation of the 8 KB Unified Buffer.
...
- Example of tensor high-dimensional slicing computation - bitwise mask mode
    ...
    // Example with the scalar at the front
    static constexpr AscendC::BinaryConfig config = { 0 };
    AscendC::Maxs<BinaryDefaultType, true, config>(dstLocal, src0Local[0], src1Local, mask, 4, { 1, 1, 8, 8 });
    ```
