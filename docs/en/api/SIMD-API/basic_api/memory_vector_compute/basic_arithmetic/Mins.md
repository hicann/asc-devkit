...
<!-- md-trans-meta sourceCommit=93c993b00c4f6138f380f7a2d742660ee1403179 translatedAt=2026-09-09T11:11:22.094Z -->
...
Mins is a binary scalar computation API. Its inputs are a vector tensor and a scalar. Mins computes the minimum value between each element in the vector tensor and the scalar. The calculation formula is as follows:
...
| isSetMask | Whether to set the mask inside the API.<br>&bull; true: the mask is set inside the API.<br>&bull; false: the mask is set outside the API. In this case, you need to use [SetVectorMask](../mask_operations/SetVectorMask.md) to set the mask value. In this mode:<br>&nbsp;&nbsp;&bull; For the API that computes the first n elements of a tensor, the count input parameter does not take effect. It is recommended to set it to 1.<br>&nbsp;&nbsp;&bull; For the API that computes a high-dimensional slicing of a tensor, set the mask input parameter to the placeholder `MASK_PLACEHOLDER`, which is used only as a placeholder and has no actual meaning. |
...
| scalarValue | Input | Source operand. Its data type must be the same as the element type of the destination operand. |
...
| repeatTime | Input | Number of repeated iterations. The vector computation unit reads 256 bytes of consecutive data for computation each time. To process all input data, multiple iterations (repeats) are required to read and compute all the data. repeatTime indicates the number of iterations.<br>For details about this parameter, see [High-Dimensional Split](../SIMD_compute/high_dimension_slicing.md). |
...
- Description of UB space usage. For Ascend 950PR/Ascend 950DT:
  - The tensor high-dimensional slicing computation occupies 8 KB of the Unified Buffer.
  - The continuous computation of the first n data of the tensor does not involve the occupation of the 8 KB Unified Buffer.
- For Ascend 950PR/Ascend 950DT, the isSetMask parameter in the API for computing the first n data of the tensor does not take effect. Keep its default value.
<!-- end id31 -->

## Example<a name="section633mcpsimp"></a>

- Example of tensor high-dimensional slicing computation - mask continuous mode
...
- Example of tensor high-dimensional slicing computation - mask bitwise mode
...
- Example of the computation API for the first n data of a tensor
