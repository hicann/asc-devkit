...
<!-- md-trans-meta sourceCommit=e5451c7feba11f9e259f4fbc19c4bf82f8c0496c translatedAt=2026-09-09T10:34:41.637Z -->
...
Divs is a binary scalar computation API. It computes the smaller value between each element in a vector and a scalar, and supports two scenarios: scalar on the left and scalar on the right. The scalar input supports configuring a LocalTensor single-point element. The computation formulas are as follows:
...
<!-- npu="950" id8 -->
For Ascend 950PR/Ascend 950DT, the supported data types are half, float, complex32, int64_t, uint64_t, and complex64. Among them, the complex32, int64_t, uint64_t, and complex64 data types are supported only by the API that computes the first n data of a tensor.
<!-- end id8 -->
...
- When using the Tensor high-dimensional slicing computation API, to save address space, developers can define a single Tensor for both the source operand and the destination operand (that is, address overlap). The related constraints are as follows:
    ...
    - When the source operand is a LocalTensor single-point element, address overlap with the destination operand is not supported.
...
- When LocalTensor single-point data is passed to this API as a scalar, the idx parameter must be a compile-time constant. If a variable is passed, it must be declared as constexpr.
...
- Description of UB space usage. For Ascend 950PR/Ascend 950DT:
  - Tensor high-dimensional slicing computation occupies 8KB Unified Buffer.
  - Continuous computation of the first n data of the tensor does not involve occupation of the 8KB Unified Buffer.
- For Ascend 950PR/Ascend 950DT, the isSetMask parameter in the API for computing the first n data of the tensor does not take effect. Keep the default value.
<!-- end id9 -->
...
- Tensor high-dimensional slicing computation sample - mask continuous mode
    ...
    uint64_t mask = 128;
    // repeatTime = 4, process 128 numbers per iteration, and iterate 4 times to compute 512 numbers
    // dstBlkStride, srcBlkStride = 1, the address interval of the data in src0 participating in computation within each iteration is 1 datablock, indicating that data is read and written continuously within a single iteration
...
- Tensor high-dimensional slicing computation example - mask bitwise mode
    ...
    // dstBlkStride, srcBlkStride = 1, the address interval of the data in src0 participating in the computation within each iteration is 1 datablock, indicating that data is read and written continuously within a single iteration
    // dstRepStride, srcRepStride = 8, the address interval between adjacent iterations is 8 datablocks, indicating that data is read and written continuously between adjacent iterations
    // Example with the scalar at the end
    AscendC::Divs(dstLocal, src0Local, src1Local[0], mask, 4, {1, 1, 8, 8});
