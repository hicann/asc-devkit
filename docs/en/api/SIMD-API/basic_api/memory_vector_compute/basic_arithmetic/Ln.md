...
<!-- md-trans-meta sourceCommit=e5451c7feba11f9e259f4fbc19c4bf82f8c0496c translatedAt=2026-09-09T10:47:07.289Z -->
...
Ln is a unary vector computation API that computes the natural logarithm of each element of the input tensor. The computation formula is as follows:
...
- Prototype with config
    ...
    - High-dimensional partitioning computation of the tensor
        - Bitwise mask mode
...
| <!-- npu="950" id19 -->config | This parameter is supported only on Ascend 950PR/Ascend 950DT.<br>It is used to configure the Subnormal computation mode. The type is LnConfig, which is defined as follows:<br>enum&nbsp;class&nbsp;LnAlgo&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;INTRINSIC&nbsp;=&nbsp;0,<br>&nbsp;&nbsp;&nbsp;&nbsp;PRECISION_1ULP_FTZ_TRUE,<br>&nbsp;&nbsp;&nbsp;&nbsp;PRECISION_1ULP_FTZ_FALSE,<br>};<br>struct&nbsp;LnConfig&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;LnAlgo&nbsp;algo&nbsp;=&nbsp;LnAlgo::INTRINSIC;<br>};<br>The Subnormal computation mode is configured through the algo parameter of the LnConfig structure. The values of algo are as follows:<br>&bull; LnAlgo::INTRINSIC and LnAlgo::PRECISION_1ULP_FTZ_TRUE: The result is computed using a single instruction, and all Subnormal values are approximated to 0.<br>&bull; LnAlgo::PRECISION_1ULP_FTZ_FALSE: Subnormal data computation is supported.<br>The default value DEFAULT_LN_CONFIG of this parameter is as follows:<br>constexpr&nbsp;LnConfig&nbsp;DEFAULT_LN_CONFIG&nbsp;=&nbsp;{&nbsp;LnAlgo::INTRINSIC&nbsp;};<br>When this prototype is called without explicitly passing the config parameter, DEFAULT_LN_CONFIG is used by default, and the behavior is equivalent to that of the prototype without the config parameter.<!-- end id19 --> |
...
- Description of UB space usage. For Ascend 950PR/Ascend 950DT:
  - Tensor high-dimensional partitioning computation occupies 8 KB of Unified Buffer.
...
For Ascend 950PR/Ascend 950DT, the following key features are provided:

**Maximum precision error**:

- LnAlgo::INTRINSIC, LnAlgo::PRECISION\_1ULP\_FTZ\_TRUE, with a maximum precision error of 1 ulp.
- LnAlgo::PRECISION\_1ULP\_FTZ\_FALSE, implemented by software simulation, with a maximum precision error of 1 ulp.

**Configure Subnormal mode**:
...
Only when algo is set to LnAlgo::PRECISION\_1ULP\_FTZ\_false does the Ln API retain and correctly output Subnormal results; in other modes, Subnormal results are all flushed to zero by FTZ.

Because the Subnormal computation behavior is implemented through a software simulation algorithm, it is recommended to use LnAlgo::INTRINSIC and LnAlgo::PRECISION\_1ULP\_FTZ\_TRUE, which offer better performance, in general scenarios; use LnAlgo::PRECISION\_1ULP\_FTZ\_FALSE in scenarios that require precise Subnormal output (such as algorithms with specific data precision requirements or avoiding division-by-zero errors).
<!-- end id24 -->

## Calling Examples<a name="section176061616102911"></a>

- Tensor high-dimensional partitioning computation example - mask continuous mode
...
- Tensor high-dimensional partitioning computation example - mask bit-by-bit mode
...
Output data dstLocal: [0 0.6931 1.0986 1.3863 ...]
```
