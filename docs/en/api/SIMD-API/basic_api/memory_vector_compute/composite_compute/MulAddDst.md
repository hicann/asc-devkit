# MulAddDst<a name="ZH-CN_TOPIC_0000001788011396"></a>

<!-- md-trans-meta sourceCommit=93c993b00c4f6138f380f7a2d742660ee1403179 translatedAt=2026-09-09T08:59:47.106Z -->
...
Multiplies src0 and src1 element-wise, adds the result to dst, and stores the final result in dst. The calculation formula is as follows:

$$
dst_j = (src0_i * src1_i) + dst_i
$$

## Prototype<a name="section620mcpsimp"></a>

- Calculation on the first n data elements of a tensor
...
- High-dimensional slicing calculation on a tensor
  - Bitwise mask mode
...
| :--- | :--- |
| T | Destination operand data type. For the data type constraints of the destination operand and source operands, see [Data Type Constraints](#data-type). |
...
| src0, src1 | Input | Source operands. The source operand data type and the destination operand data type can be different.<br>The type is LocalTensor, and the supported TPosition values are VECIN/VECCALC/VECOUT. |
...
| mask[]/mask | Input | **mask** controls the elements that participate in the calculation in each iteration. For details about the settings, see [Mask Operation](../SIMD_compute/mask.md). |
| repeatTime | Input | Number of repeated iterations. The vector calculation unit reads 256 bytes of consecutive data for calculation each time. To complete the processing of the input data, multiple iterations (repeat) are required to finish reading and calculating all the data. repeatTime indicates the number of iterations.<br>For details about this parameter, see [High-dimensional Split](../SIMD_compute/high_dimension_slicing.md). |
...
PAR indicates the number of elements that the vector calculation unit can process in one iteration.

<!-- npu="950" id1 -->
**Table**  Data types supported by Ascend 950PR/Ascend 950DT
...
<!-- npu="A3" id2 -->
**Table**  Data types supported by Atlas A3 training products/Atlas A3 inference products
...
<!-- npu="910b" id3 -->
**Table**  Data types supported by Atlas A2 training products/Atlas A2 inference products
...
<!-- npu="310b" id4 -->
**Table**  Data type constraints supported by Atlas 200I/500 A2 inference products

| src0 | src1 | dst | PAR  |
| :----------- | :----------- | :---------- | :--- |
| half         | half         | half        | 128  |
| float        | float        | float       | 64   |
| int16_t      | int16_t      | int16_t     | 128  |
| uint16_t     | uint16_t     | uint16_t    | 128  |
| int32_t      | int32_t      | int32_t     | 64   |
| uint32_t     | uint32_t     | uint32_t    | 64   |
<!-- end id4 -->

<!-- npu="310p" id5 -->
**Table**  Data types supported by the AI Core of Atlas inference products
...
<!-- npu="950" id6 -->
- For Ascend 950PR/Ascend 950DT, the int64_t and uint64_t data types support only the tensor first-n-data calculation API.
<!-- end id6 -->

## Example<a name="section837496171220"></a>

- High-dimensional slicing calculation API example - continuous mask mode (the source operand data type is half and the destination operand data type is float)

  ```cpp
  uint64_t mask = 64;
  // repeatTime = 4, 64 numbers are calculated in one iteration, and 256 numbers are calculated in total.
  // dstBlkStride, src0BlkStride, src1BlkStride = 1, data is read and written continuously within a single iteration.
  // dstRepStride = 8, src0RepStride, src1RepStride = 4, data is read and written continuously between adjacent iterations.
  AscendC::MulAddDst(dstLocal, src0Local, src1Local, mask, 4, { 1, 1, 1, 8, 4, 4 });
  ```

- High-dimensional slicing calculation API example - bitwise mask mode (the source operand data type is half and the destination operand data type is float)
...
- Example of calculating the first n data of a tensor (the source operand data type is half and the destination operand data type is float)
