...
<!-- md-trans-meta sourceCommit=0364dcce9ab16d714270f168ec9546fd7620f90f translatedAt=2026-09-09T12:32:46.042Z -->
...
Performs a bitwise AND operation between each element in a vector and a scalar. Both scenarios are supported: scalar first and scalar last. The scalar input supports configuring a single-point element of a LocalTensor. The calculation formula is as follows, where idx indicates the position coefficient of the single-point element of the LocalTensor.
...
<td class="cellrowborder" valign="top" width="86.09%" headers="mcps1.2.3.1.2 "><p id="zh-cn_topic_0000001429830437_p168351657155818"><a name="zh-cn_topic_0000001429830437_p168351657155818"></a><a name="zh-cn_topic_0000001429830437_p168351657155818"></a><span id="ph1756763773511"><a name="ph1756763773511"></a><a name="ph1756763773511"></a>Reserved parameter, not enabled yet. It is reserved for future feature extension. When it needs to be specified, pass the default value BinaryDefaultType.</span></p></td>
</tr>
...
<a name="zh-cn_topic_0000001429830437_ul1163765616511"></a><a name="zh-cn_topic_0000001429830437_ul1163765616511"></a><ul id="zh-cn_topic_0000001429830437_ul1163765616511"><li>true, indicating that the setting is performed inside the API.<p id="p1715792133310"><a name="p1715792133310"></a><a name="p1715792133310"></a>The Tensor high-dimensional splitting computation API/Tensor first-n-data computation API internally uses the <span id="ph9187131991312"><a name="ph9187131991312"></a><a name="ph9187131991312"></a><a href="https://gitcode.com/cann/asc-devkit/blob/9.1.0/docs/guide/programming_guide/library_api/basic_api/quick_reference/how_to_use_mask_operation_api.md" target="_blank" rel="noopener noreferrer">Normal mode/Counter mode</a></span> of the mask. In general, keep the default value of isSetMask, which means that the mask mode and mask value are set inside the API based on the mask/count parameters passed by the developer.</p>
</li><li>false, indicating that the setting is performed outside the API.<a name="ul687683044913"></a><a name="ul687683044913"></a><ul id="ul687683044913"><li>For the Tensor high-dimensional splitting computation API, in some performance-sensitive scenarios, the developer needs to use <a href="../mask_operations/SetMaskNorm.md">SetMaskNorm</a>/<a href="../mask_operations/SetMaskCount.md">SetMaskCount</a> to set the mask mode, and use the <a href="../mask_operations/SetVectorMask.md">SetVectorMask</a> API to set the mask value. The mask value in the API input parameters is set to the placeholder MASK_PLACEHOLDER, which is used only as a placeholder and has no actual meaning.</li><li>For the Tensor first-n-data computation API, in some performance-sensitive scenarios, the developer needs to use <a href="../mask_operations/SetMaskCount.md">SetMaskCount</a> to set the mask mode to Counter mode, and use the <a href="../mask_operations/SetVectorMask.md">SetVectorMask</a> API to set the mask value. The count in the input parameters of this API does not take effect, and it is recommended to set it to 1.</li></ul>
</li></ul>
<p id="p56771501927"><a name="p56771501927"></a><a name="p56771501927"></a>For the following models, the isSetMask parameter in the Tensor first-n-data computation API does not take effect. Keep the default value.</p>
...
<td class="cellrowborder" valign="top" width="86.09%" headers="mcps1.2.3.1.2 "><p id="p499382311404"><a name="p499382311404"></a><a name="p499382311404"></a>The type is BinaryConfig. It takes effect when the scalar is a LocalTensor single-point element type, and is used to specify the position of the single-point element operand. The default value DEFAULT_BINARY_CONFIG indicates that the right operand is the scalar.</p>
...
<td class="cellrowborder" valign="top" width="86.09%" headers="mcps1.2.3.1.2 "><p id="p3172440173612"><a name="p3172440173612"></a><a name="p3172440173612"></a>LocalTensor type. The corresponding data type is automatically inferred based on the input parameter dst. The developer does not need to configure this parameter, but only needs to ensure that dst meets the data type constraints.</p></td>
</tr>
...
<td class="cellrowborder" valign="top" width="86.09%" headers="mcps1.2.3.1.2 "><p id="p151721640153610"><a name="p151721640153610"></a><a name="p151721640153610"></a>LocalTensor type or scalar type. The data type is automatically inferred from the input parameter src0. Developers do not need to configure this parameter, but must ensure that src0 meets the data type constraints.</p></td>
</tr>
...
<td class="cellrowborder" valign="top" width="86.09%" headers="mcps1.2.3.1.2 "><p id="p11721940193620"><a name="p11721940193620"></a><a name="p11721940193620"></a>LocalTensor type or scalar type. The data type is automatically inferred from the input parameter src1. Developers do not need to configure this parameter, but must ensure that src1 meets the data type constraints.</p></td>
</tr>
</tbody>
</table>
...
<p id="p4432165467"><a name="p4432165467"></a><a name="p4432165467"></a><span id="ph1943116154619"><a name="ph1943116154619"></a><a name="ph1943116154619"></a>Ascend 950PR/Ascend 950DT</span>, the supported data types are: int16_t/uint16_t/int64_t/uint64_t</p></td>
</tr>
...
<td class="cellrowborder" valign="top" width="78.86999999999999%" headers="mcps1.2.4.1.3 "><p id="p10444162462"><a name="p10444162462"></a><a name="p10444162462"></a>Number of repeated iterations. The vector computation unit reads 256 bytes of consecutive data for computation each time. To complete the processing of the input data, multiple iterations (repeats) are required to read and compute all the data. repeatTime indicates the number of iterations.</p>
<p id="p19449164465"><a name="p19449164465"></a><a name="p19449164465"></a>For details about this parameter, see <span id="zh-cn_topic_0000001530181537_ph43961850144019"><a name="zh-cn_topic_0000001530181537_ph43961850144019"></a><a name="zh-cn_topic_0000001530181537_ph43961850144019"></a><a href="../SIMD_compute/high_dimension_slicing.md" target="_blank" rel="noopener noreferrer">high-dimensional splitting API</a></span>.</p></td>
</tr>
...
-   When using the Tensor high-dimensional splitting computation API, address space is saved. Developers can define one Tensor for both the source operand and the destination operand (that is, address overlap). The related constraints are as follows:
    ...
    -   When the source operand is a single-point element of LocalTensor, address overlap between the source operand and the destination operand is not supported.
...
-   For Ascend 950PR/Ascend 950DT, the uint64\_t/int64\_t data type supports only the API for computing the first n data of a Tensor.
...
-   When this API passes single-point data of LocalTensor as a scalar, the idx parameter must be a constant known at compile time. When a variable is passed, it must be declared as constexpr.
...
-   Tensor high-dimensional splitting computation example - continuous mask mode
    ...
    uint64_t mask = 128;
    // repeatTime = 4, 128 numbers are processed per iteration, and 4 iterations are required to compute 512 numbers
...
Input data src1Local: [0]
Output data dstLocal: [0 0 0 ... 0]

// Scalar at the beginning, src0Local[0] as the scalar
Input data src0Local: [0]
