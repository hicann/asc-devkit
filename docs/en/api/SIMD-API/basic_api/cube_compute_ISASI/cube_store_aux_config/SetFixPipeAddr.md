...
<!-- md-trans-meta sourceCommit=e9bec8b34ada6c37c20391d212da5ebf088e2e8b translatedAt=2026-08-27T15:07:58.785Z -->
...
During DataCopy data transfer ([L0C Buffer->GM](../cube_compute_store/DataCopy_L0CToGM.md)) with in-line quantization, call this API to set the address of the **LocalTensor** used in the **Elementwise** operation.
...
| ---------- | ---------- | ---------- |
| eleWiseData | Input | Source operand in the L1 Buffer. The type is **LocalTensor**.<br>The supported physical location is the L1 Buffer (**TPosition** is A1/B1/C1). The start address must be 32-byte aligned, and only the half data type is supported. |
| c0ChStride | Input | C0 channel stride in the L1 Buffer, in units of **C0_SIZE** (32 bytes).<br>The data block obtained by splitting **eleWiseData** along the N direction in units of C0 is called a C0 channel, and the interval between two C0 channels is called the C0 channel stride. |

## Data Type

**LocalTensor** supports the following data type: **half**.
...
After DataCopy performs quantization during data movement, you can add or subtract a LocalTensor of size mSize \* nSize element by element. The specific LocalTensor address-related parameters need to be set by calling `SetFixPipeAddr`.
...
- For Atlas 200I/500 A2 inference products, the complete example is as follows. Data movement with in-path format conversion, path: L0C Buffer-\>GM.

    Example: Mmad contains a cube multiplication bias. The data type of the left cube and the right cube is int8\_t, and the data type of the result cube is int32\_t. The quantization mode is DEQF16, and the scalar quantization parameter is 0.5. The result calculated by Mmad is quantized from int32\_t to half and moved out.
    ...
    private:
        ...
        __aicore__ inline void CopyOut()
        {
            ...
            // Do not enable ReLU and channelSplit.
            ...
            // // Allocate additional GM space for the deq tensor and move the values to workA1.
            ...
            // // // Set the clip ReLU value to the register.
            // uint64_t clipReluVal = 0x3c00; // value 1, half
            // SetFixPipeClipRelu(clipReluVal);
            // //// Move l0c to gm, and after the quantization operation, set the element-wise operation, Add.
