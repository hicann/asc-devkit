...
| ---------- | ---------- | ---------- |
| dst | Output | Destination operand, of type **LocalTensor**.<br>For the fractal constraint, see [Cube Computation Input Transfer Constraints](Cube Computation Input Transfer Constraints.md).<br>For the start address alignment constraint, see [Alignment Constraint](Cube Computation Input Transfer Constraints.md) in Cube Computation Input Transfer Constraints.<br>The data type is consistent with that of **src**.<br>The supported physical storage location is L1 Buffer (**TPosition** is **A1/B1**). |
