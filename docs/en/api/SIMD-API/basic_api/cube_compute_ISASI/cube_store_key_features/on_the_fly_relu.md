...
- When Normal ReLU is enabled (**reluPre**=1), **clipReluPre**=1 is configured at the same time, and the SetFixPipeClipRelu API is called, Normal ReLU + ClipReLU is enabled. In this case, `y = max(0, x)` is performed first, followed by `y = min(clipReluMaxVal, y)`, where **clipReluMaxVal** is set through the SetFixPipeClipRelu API. For details, see [SetFixPipeClipRelu](../Cube Move-out Auxiliary Configuration API/SetFixPipeClipRelu.md).

For details about the combined use with on-the-fly quantization, see [In-path Quantization and In-path ReLU Scenario Combination](In-path Quantization and In-path ReLU Scenario Combination.md).
