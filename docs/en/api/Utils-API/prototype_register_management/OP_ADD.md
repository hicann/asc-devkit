...
An operator prototype mainly describes the inputs, outputs, attributes, and other information of an operator, as well as the related implementation information of the operator on the AI processor, and associates functions such as the [tiling implementation](https://gitcode.com/cann/asc-devkit/blob/9.1.0/docs/en/guide/programming_guide/advanced_programming/aclnn_operator_development/host_tiling_implementation/basic_process.md). The operator prototype is carried by a custom operator class that inherits from the [OpDef class](OpDef/OpDef.md). After completing operations such as the prototype definition of the operator, call the [OP\_ADD](OP_ADD.md) API and pass in the operator type (the class name of the custom operator class) to register the operator prototype. For details, see [Operator Prototype Definition](https://gitcode.com/cann/asc-devkit/blob/9.1.0/docs/en/guide/programming_guide/advanced_programming/aclnn_operator_development/operator_prototype_definition.md).
