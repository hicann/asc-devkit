...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-08-29T10:51:25.106Z pushedAt=2026-09-01T09:20:53.018Z -->
...
Computes the absolute value of each component of the input `bfloat16x2_t` data.

## Function Prototype

```
bfloat16x2_t __habsx2(const bfloat16x2_t x)
```
...
-   SIMT programming scenario:
    ...
    __global__ __launch_bounds__(1024) void simt_habsx2(bfloat16_t* x, bfloat16_t* dst, uint32_t input_total_length)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        // Each thread processes one bfloat16x2_t data element, that is, two bfloat16_t data elements. Therefore, threads with idx equal to or greater than half the input_total_length do not process data.
...
-   SIMD and SIMT hybrid programming scenario:
    ...
    __simt_vf__ __launch_bounds__(1024) inline void simt_habsx2(__gm__ bfloat16x2_t* x, __gm__ bfloat16x2_t* dst, uint32_t input_total_length)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    {
        asc_vf_call<simt_habsx2>(dim3(1024), (__gm__ bfloat16x2_t*)x, (__gm__ bfloat16x2_t*)dst, input_total_length);
    }
    ```
