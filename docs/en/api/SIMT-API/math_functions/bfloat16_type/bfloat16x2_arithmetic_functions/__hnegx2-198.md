...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-08-29T10:57:00.295Z pushedAt=2026-09-01T09:20:53.027Z -->
...
Obtains the negative value of each component of the input **bfloat16x2_t** data.

## Function Prototype

```
bfloat16x2_t __hnegx2(const bfloat16x2_t x)
```
...
Returns the negative value of each component of the input data.
...
-   SIMT programming scenario:
    ...
    __global__ __launch_bounds__(1024) void simt_hnegx2(bfloat16_t* x, bfloat16_t* dst, uint32_t input_total_length)
    {
...
-   SIMD and SIMT hybrid programming scenario:
    ...
    ```
    // Use short vectors to improve data transfer efficiency.
    __simt_vf__ __launch_bounds__(1024) inline void simt_hnegx2(__gm__ bfloat16x2_t* x, __gm__ bfloat16x2_t* dst, uint32_t input_total_length)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        // Each thread processes one bfloat16x2_t data item, that is, two bfloat16_t data items. Therefore, threads with idx equal to or greater than half the input_total_length do not process data.
        if (idx >= input_total_length / 2) {
    ...
    {
        asc_vf_call<simt_hnegx2>(dim3(1024), (__gm__ bfloat16x2_t*)x, (__gm__ bfloat16x2_t*)dst, input_total_length);
    }
    ```
