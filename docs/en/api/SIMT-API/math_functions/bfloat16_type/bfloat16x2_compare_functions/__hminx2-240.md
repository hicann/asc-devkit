...
<!-- md-trans-meta sourceCommit=c17b32aa8d725bcb43135ab25add5a26d62ad5c0 translatedAt=2026-08-29T10:32:20.602Z pushedAt=2026-09-01T09:20:52.985Z -->
...
Obtains the minimum value of each component of two bfloat16x2\_t data items.

## Function Prototype

```
bfloat16x2_t __hminx2(const bfloat16x2_t x, const bfloat16x2_t y)
```
...
The minimum value of each component of the input data. The compared components a and b satisfy the following conditions:
...
-   SIMT programming scenario:
    ...
    __global__ __launch_bounds__(1024) void simt_hminx2(bfloat16_t* x, bfloat16_t* y, bfloat16_t* dst, uint32_t input_total_length)
    {
...
-   SIMD and SIMT hybrid programming scenario:
    ...
    ```
    // Using short vectors improves data transfer efficiency.
    __simt_vf__ __launch_bounds__(1024) inline void simt_hminx2(__gm__ bfloat16x2_t* x, __gm__ bfloat16x2_t* y, __gm__ bfloat16x2_t* dst, uint32_t input_total_length)
    {
        uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        // Each thread processes one bfloat16x2_t value, that is, two bfloat16_t values. Therefore, threads with idx equal to or greater than half the input_total_length do not process data.
        if (idx >= input_total_length / 2) {
    ...
    {
        asc_vf_call<simt_hminx2>(dim3(1024), (__gm__ bfloat16x2_t*)x, (__gm__ bfloat16x2_t*)y, (__gm__ bfloat16x2_t*)dst, input_total_length);
    }
    ```
