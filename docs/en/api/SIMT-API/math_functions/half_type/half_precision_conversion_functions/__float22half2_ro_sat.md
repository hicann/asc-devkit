# \_\_float22half2\_ro\_sat

<!-- md-trans-meta sourceCommit=2bc2b38d5648dc61f632186b01ba7f36c2295b13 translatedAt=2026-09-04T00:45:14.453Z pushedAt=2026-09-04T11:39:11.864Z -->
...
In saturation mode, converts the two components of **float2** type data to half-precision floating-point numbers following the CAST\_ODD mode, and returns the converted **half2** type data.
...
Returns the **half2** type data obtained by converting the two input components following the CAST\_ODD mode in saturation mode.
...
-   SIMD and SIMT hybrid programming scenario:
    ...
    __simt_vf__ __launch_bounds__(1024) inline void simt_float22half2_ro_sat(__gm__ float2* input, __gm__ half2* output, uint32_t input_total_length)
    {
        ...
        output[idx] = __float22half2_ro_sat(input[idx]);
    }

    __global__ __vector__ void cast_kernel(__gm__ float* input, __gm__ half* output, uint32_t input_total_length)
    {
        asc_vf_call<simt_float22half2_ro_sat>(dim3(1024), (__gm__ float2*)input, (__gm__ half2*)output, input_total_length);
    }
    ```
