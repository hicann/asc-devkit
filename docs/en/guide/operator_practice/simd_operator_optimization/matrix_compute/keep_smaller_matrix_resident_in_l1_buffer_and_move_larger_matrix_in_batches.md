# Keeping the Smaller Matrix Resident in the L1 Buffer and Moving Only the Larger Matrix in Batches<a name="ZH-CN_TOPIC_0000001918185186"></a>

<!-- md-trans-meta sourceCommit=d0ede7621d11bdc88cdafb221560ec6e0bfe996d translatedAt=2026-08-26T14:12:38.647Z -->

[Priority] High  __

[Description] During cube computation, when the L1 buffer cannot hold both the left and right matrices at once, keep the smaller matrix resident in L1 and move only the larger matrix in batches, thereby reducing the number of data transfers.

[Negative Example]

Assume that the L1 buffer is 512K, the left and right matrices are 992K and 16K respectively, and the data type is half. The left and right matrices cannot be fully loaded into L1 at once. The developer plans the following splitting strategy: do not split the K axis, split the left matrix evenly into two blocks A1 and A2, each with a shape of \[992, 256\]; split the right matrix evenly into two blocks, each with a shape of \[256, 16\]. The loading order during computation is as follows: first load matrix A1 into L1, then load and compute B1 and B2 in sequence; then load A2 into L1, and load and compute B1 and B2 in sequence.

**Figure 1**  Counterexample splitting strategy diagram<a name="fig9917043192214"></a>  
![](../../../figures/counterexample_tiling_strategy_diagram.png "Counterexample splitting strategy diagram")
...
private:
    __aicore__ inline void CopyInA1(uint32_t i)
    {
        ...
        // Load the left matrix blocks a1/a2 into A1
    ...
    __aicore__ inline void SplitA()
    {
        ...
        // Move the left matrix blocks a1/a2 from A1 to A2
    ...
    __aicore__ inline void CopyInB1(uint32_t j)
    {
        ...
        // Load the right matrix blocks b1/b2 into B1
    ...
    __aicore__ inline void SplitB()
    {
        ...
        // Move the right matrix blocks b1/b2 from B1 to B2
...
In this example, the smaller right matrix is transferred into L1 at once and kept resident in L1. Matrix A is continuously transferred within the loop. When the loop count is 2, a total of 3 transfers are required.
...
private:
    ...
    {
        LocalTensor<half> b1Local = inQueueB1.AllocTensor<half>();
        // Load the entire right matrix into B1.
        Nd2NzParams dataCopyB1Params;
        dataCopyB1Params.ndNum = 1;
        dataCopyB1Params.nValue = baseK;
    ...
    __aicore__ inline void SplitB()
    {
        LocalTensor<half> b1Local = inQueueB1.DeQue<half>();
        LocalTensor<half> b2Local = inQueueB2.AllocTensor<half>();
        // Transfer the entire right matrix from B1 to B2.
        LoadData2dTransposeParams loadL0BParams;
        loadL0BParams.startIndex = 0;
        loadL0BParams.repeatTimes = baseK / nBlockSize;
        loadL0BParams.srcStride = 1;
    ...
    {
        LocalTensor<half> a1Local = inQueueA1.AllocTensor<half>();
        // Load the left matrix a1/a2 into A1 in blocks.
        Nd2NzParams dataCopyA1Params;
        dataCopyA1Params.ndNum = 1;
        dataCopyA1Params.nValue = baseM;
        dataCopyA1Params.dValue = baseK;
    ...
    __aicore__ inline void SplitA()
    {
        LocalTensor<half> a1Local = inQueueA1.DeQue<half>();
        LocalTensor<half> a2Local = inQueueA2.AllocTensor<half>();
        // Transfer the left matrix a1/a2 from A1 to A2 in blocks.
        LoadData2dParams loadL0AParams;
        loadL0AParams.repeatTimes = baseM * baseK * sizeof(half) / 512;
        loadL0AParams.srcStride = 1;
        loadL0AParams.dstGap = 0;
