...
├── zh                             # 中文文档目录
│   ├── api                        # API文档目录
│   ├── contributing               # 贡献指南目录（API贡献指南、代码贡献规范等）
│   ├── figures                    # 图片目录
...
| [高阶API贡献指南](./zh/contributing/asc_adv_api_contributing.md) | 基于Ascend C开源仓，开发定制化API的开发者。 | 介绍如何扩展或开发Ascend C高阶API。高阶API基于单核对常见算法进行抽象和封装，实现常用的计算算法，旨在提高算子开发效率。  |
| [基础API贡献指南](./zh/contributing/asc_basic_api_contributing.md) | 基于Ascend C开源仓，开发定制化API的开发者。 | 介绍如何扩展或开发Ascend C基础API。基础API实现对硬件能力的抽象，开放芯片的能力，保证完备性和兼容性。  |
| [C API贡献指南](./zh/contributing/asc_c_api_contributing.md) | 基于Ascend C开源仓，开发定制化API的开发者。 | 介绍如何扩展或开发Ascend C C API。C API提供纯C风格的接口，符合C语言算子开发习惯，开放芯片完备编程能力。  |
| [资料贡献指南](./zh/contributing/asc_doc_contributing.md) | 完善Ascend C现有资料的开发者。 |  介绍Ascend C资料各大章节的编写与贡献规范。|
...
| [Ascend C算子实践参考](./zh/guide/operator_practice/document_structure.md) | 基于已完成开发的Ascend C算子，需要进一步优化算子性能的开发者。 | 异构计算的特点、算子功能的调试方法以及算子性能的优化策略。通过介绍Ascend C编程中的调试与优化思路，结合多种性能优化手段，并辅以具体案例，旨在帮助开发者实现高性能算子的开发。|
