...
在Mix场景中，AIV核的完整逻辑位置由block_idx和sub_block_idx共同确定：`logic_idx = block_idx * sub_block_num + sub_block_idx`。开发者应使用[GetBlockIdx](../../../../api/SIMD-API/基础API/工具接口/系统资源与变量/GetBlockIdx.md)和[GetSubBlockIdx](../../../../api/SIMD-API/基础API/工具接口/系统资源与变量/GetSubBlockIdx_ISASI.md) API组合获取逻辑位置，而非直接使用内置变量（更多说明参见[内置变量](../../语言扩展层/SIMD-BuiltIn关键字.md#内置变量)中的注意事项）。
