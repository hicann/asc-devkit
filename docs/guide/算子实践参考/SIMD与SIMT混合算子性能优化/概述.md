...
2. 实际数据度量
   ...
   实际数据度量用于观察算子运行时的GM访问情况，以及SIMT VF在AIV核执行时间中的占比。可使用[msOpProf工具](https://gitcode.com/Ascend/msopprof/blob/26.1.0/docs/zh/user_guide/msopprof_user_guide.md)采集性能数据：
...
完成理论性能评估和实际数据度量后，应优先分析GM读写带宽和实际读写数据量，再决定优化方向。对于SIMD与SIMT混合算子，优化通常可从内存访问、执行控制和指令优化三个方面展开。
...
- 指令优化
  ...
  指令优化通常在计算瓶颈，或内存访问已经较充分但性能仍未达到预期时重点考虑。它的作用是减少SIMT VF内部的高开销指令、分支发散和冗余计算，提升线程级计算逻辑的执行效率。

  对复杂分支、离散索引等逻辑，可使用SIMT表达线程级控制流，但SIMT内部仍需控制分支发散，通过优化计算逻辑减少除法、取模等高开销指令的使用，并避免在线程内重复执行可提前计算或可复用的逻辑。
...
| 优化建议 | 样例 |
| --- | --- |
| [通过SIMT实现分支判断](./计算优化/通过SIMT实现分支判断.md) | [simd_simt_high_performance](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/05_simd_simt_hybrid/02_best_practices/simd_simt_high_performance) |

表2 内存访问优化建议总览表

| 优化建议 | 样例 |
| --- | --- |
| [使用Unified Buffer提升内存访问效率](./内存访问/使用Unified-Buffer提升内存访问效率.md) | [simd_simt_high_performance](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/05_simd_simt_hybrid/02_best_practices/simd_simt_high_performance) |
| [访存合并](../SIMT算子性能优化/内存访问/访存合并.md) | [simd_simt_matrix_transpose](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/05_simd_simt_hybrid/02_best_practices/simd_simt_matrix_transpose) |
| [避免UB的Bank冲突](../SIMT算子性能优化/内存访问/避免UB的Bank冲突.md) | [matrix_transpose_practice](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/03_simt_api/03_best_practices/00_memory_optimizations/matrix_transpose_practice) |
| [DCache访问优化](../SIMT算子性能优化/内存访问/DCache访问优化.md) | [cache_hint](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/03_simt_api/03_best_practices/00_memory_optimizations/cache_hint) |
| [使用短向量类型提升效率](../SIMT算子性能优化/内存访问/短向量类型提升效率.md) | [short_vector_add](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/03_simt_api/03_best_practices/00_memory_optimizations/short_vector_add) |
| [通过类型对齐提升搬运效率](../SIMT算子性能优化/内存访问/通过类型对齐提升搬运效率.md) | [aligned_types](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/03_simt_api/03_best_practices/00_memory_optimizations/aligned_types) |

表3 执行控制优化建议总览表

| 优化建议 | 样例 |
| --- | --- |
| [线程块数量配置优化](./执行控制/线程块数量配置优化.md) | [simd_simt_grid_dim_config](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/05_simd_simt_hybrid/02_best_practices/simd_simt_grid_dim_config) |

表4 指令优化建议总览表

| 优化建议 | 样例 |
| --- | --- |
| [整数除法的快速算法](./指令优化/整数除法的快速算法.md) | [simd_simt_integer_fast_div](https://gitcode.com/cann/asc-devkit/tree/9.1.0/examples/05_simd_simt_hybrid/02_best_practices/simd_simt_integer_fast_div) |
