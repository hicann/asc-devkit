...
4.  明确分核策略、SIMT线程配置和SIMD Reg矢量计算API循环调用次数。
    ...
    对于SIMD Reg矢量计算实现，单核处理数据量为1024，Reg矢量计算API单次处理的数据长度oneRepeatSize为[GetVecLen](../../../api/SIMD-API/基础API/工具接口/系统资源与变量/GetVecLen.md)/sizeof\(float\)，API的循环调用次数repeatTimes为1024/oneRepeatSize。
...
2.  启动SIMT VF函数simt\_gather，从input中获取指定索引的数据。
    ...
    2.  使用[Alloc](../../../api/SIMD-API/基础API/资源管理/LocalMemAllocator/Alloc.md)接口申请Unified Buffer内存空间，并将该Tensor作为simt\_gather函数的输出。
    3.  使用[asc_vf_call](../../../api/SIMT-API/SIMD与SIMT混合编程简介/扩展语法/核函数配置-147.md)接口启动SIMT\_VF函数simt\_gather。第一个参数为[dim3](../../编程指南/语言扩展层/SIMT-BuiltIn关键字.md#li1136665405)结构，代表线程的三维层次结构，本例中初始化为dim3\(1024\)，使用一维定义方式，线程总数为1024。
...
3.  启动SIMD VF函数simd\_adds，对Unified Buffer上的数据做加1计算。
    ...
    1.  使用[Alloc](../../../api/SIMD-API/基础API/资源管理/LocalMemAllocator/Alloc.md)接口申请Unified Buffer内存空间，并将该Tensor作为simt\_adds函数的输出。
    2.  使用[GetVecLen](../../../api/SIMD-API/基础API/工具接口/系统资源与变量/GetVecLen.md)接口除以单个数据长度，计算Reg矢量计算API单次处理的数据量oneRepeatSize。使用单核应处理的数据量indexTotalLengthPerBlock除以单次处理数据量oneRepeatSize，计算Reg矢量计算API循环调用次数。
    3.  使用[asc_vf_call](../../../api/SIMD-API/基础API/Reg矢量计算/VF调用/asc_vf_call.md)接口启动SIMD VF函数simd\_adds。
...
4.  使用[DataCopy](../../../api/SIMD-API/基础API/数据搬运导览/概览/数据搬运概念.md)接口将结果数据搬运到Global Memory。
...
2.  实现函数。
    ...
    [blockIdx](../../编程指南/语言扩展层/SIMT-BuiltIn关键字.md#li1676053814914)用于获取当前核id。[blockDim](../../编程指南/语言扩展层/SIMT-BuiltIn关键字.md#blockdim)用于获取线程三维层次结构\{x, y, z\}，本例中为\{1024, 1, 1\}，其中第2，3维度均为1，使用一维层次结构，因此线程数可写作blockDim.x。[threadIdx](../../编程指南/语言扩展层/SIMT-BuiltIn关键字.md#li7760123814919)用于获取三维线程索引\{x, y, z\}，本例中仅使用第1维x，可通过threadIdx.x获取当前线程的id。
...
2.  循环调用repeatTimes次Reg矢量计算API完成加1运算。
    ...
    1.  使用[连续对齐搬入](../../../api/SIMD-API/基础API/Reg矢量计算/Reg数据搬入/LoadAlign_continuous.md)接口将数据从Unified Buffer搬运到Reg矢量计算基本单元[RegTensor](../../../api/SIMD-API/基础API/Reg矢量计算/寄存器数据类型/RegTensor.md)。
    2.  使用[Adds](../../../api/SIMD-API/基础API/Memory矢量计算/基础算术/Adds.md)接口完成将数据加1运算。
    3.  使用[连续对齐搬出](../../../api/SIMD-API/基础API/Reg矢量计算/Reg数据搬出/StoreAlign_continuous.md)接口将数据从RegTensor搬运到Unified Buffer。
