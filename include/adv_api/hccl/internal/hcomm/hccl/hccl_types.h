...
typedef enum {
    ...
    HCCL_E_ALG_NOT_SUPPORTED = 1042,  /**< algorithm is not supported */
    HCCL_E_RES_NOT_SUFFICIENT = 1043, /**< resource insufficient */
...
typedef struct HcclCommConfigDef {
    ...
    uint64_t hcclJobID;
    ///< 只有Reduce类算子(单算子和AclGraph下算法选择不一致)受此配置影响
    ///< 0:默认值，关闭aclgraph零拷贝(结果与单算子一致优先) 1:开启aclgraph零拷贝(性能优先)
...
typedef enum { HCCL_SEND = 0, HCCL_RECV = 1, HCCL_SEND_RECV_RESERVED } HcclSendRecvType;
