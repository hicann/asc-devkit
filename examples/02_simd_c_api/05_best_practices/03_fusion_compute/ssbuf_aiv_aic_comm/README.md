# SSBuffer AIV/AIC通信C API样例

## 概述

本样例演示C API场景下如何在同一个`__mix__(1, 2)`任务中使用SSBuffer实现AIV到AIC的消息通信。任务包含1个AIC和2个AIV，两个AIV分别通过独立的环形消息队列发送消息，AIC接收并检查消息，最后将结果写入GM，由Host侧逐条校验。
...
│   ├── ssbuf_aiv_aic_comm.asc         // Ascend C C API算子实现、Kernel调用和结果校验
│   ├── README.md                      // 样例说明文档
...
<tr><td rowspan="1" align="center">样例类型(OpType)</td><td colspan="4" align="center">SSBuffer AIV/AIC Communication C API</td></tr>
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">c_api_ssbuf_aiv_aic_comm</td></tr>
...
C API当前没有与`GetSsbufBaseAddr`同名的接口。在`dav-3510` NPU上，`__ssbuf__`地址空间的零地址表示SSBuffer基地址，样例将其转换为工作区指针：
...
这里的零地址属于`__ssbuf__`地址空间，不是普通GM空地址。转换后的指针以及后续消息指针必须始终保留`__ssbuf__`限定。当前编译器不支持直接声明带`__ssbuf__`限定的数组，因此样例把二维消息队列声明为`SsbufWorkspace`的结构体成员。

### 2. AIC初始化消息队列

SSBuffer初始内容不保证为0，`asc_init`也不会清空SSBuffer。AIC启动后遍历两个AIV的消息队列，清除全部消息头，再分别向AIV0和AIV1发送启动通知：
...
启动通知使用模式4核间同步。AIV先等待`START_SYNC_ID`，收到通知后才能访问消息队列。

### 3. AIV发送消息

AIV通过`asc_get_sub_block_id()`获取自己的Vector核编号，并选择对应的消息队列。发送第`sequence`条消息时，使用下面的公式计算队列位置：
...
该通知表示消息已经准备完成，AIC收到通知后才读取消息。消息序号超过队列深度时会发生队列回绕，AIV在复用队列开头的位置前先等待ACK，确保旧消息已经由AIC处理完成。

### 4. AIC接收消息

AIC按发送方编号和消息序号读取两条消息队列。每次读取前等待对应AIV的消息准备完成通知，然后检查以下内容：
...
检查结果和消息内容写入GM中的`CommResult`。AIC完成读取后清除消息头，并通过发送方对应的ACK同步ID通知AIV释放该消息空间：
...
Kernel使用`ASC_IS_AIC`和`ASC_IS_AIV`区分AIC、AIV执行分支，并在同一个`__mix__(1, 2)`任务中访问同一片SSBuffer。Host侧为24条`CommResult`申请GM空间，通过Kernel调用符启动一个Mix block，等待流同步后回读结果并校验。
...
- 当前编译器不支持直接声明带`__ssbuf__`限定的数组，消息队列应通过`SsbufWorkspace`结构体布局访问。
