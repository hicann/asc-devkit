...
`FUSED_ITERATIONS`可设置为任意正整数，默认值为`14`。每个VF迭代执行`1 + 2 * FUSED_ITERATIONS`条Vector算术指令；`FUSED_ITERATIONS=0`仅对应历史纯Add配置，不属于当前可配置取值。
...
| 满块有效元素数 | 20992 | 每轮处理一个满块。 |
| 尾块有效元素数 | 19968 | 最后一轮处理。 |
| 每核循环次数 | 50 | 包括49个满块和1个尾块。 |
...
`add_baseline`只分配一组连续排列的x、y、z Buffer。每个Buffer预留21152个half元素，实际读写长度由`current_length`指定。
...
| --- | --- | --- | --- |
| `x_local` | 0 | 20992 | 保存当前轮从GM搬入的x。 |
| `y_local` | 21152 | 20992 | 保存当前轮从GM搬入的y。 |
| `z_local` | 42304 | 20992 | 保存当前轮的融合计算结果。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 偶数轮 | Ping | 0 | 2 | 0 | 21152 | 42304 |
| 奇数轮 | Pong | 1 | 3 | 63456 | 84608 | 105760 |

表中的偏移以half元素为单位，每个Buffer预留21152个元素。Double Buffer不改变每轮的搬运量和计算量，`asc_lock`和`asc_unlock`用于控制两组Buffer的读写顺序。
...
| AI Vector Core数 | 64 |
| 单次分块长度 | 20992个half元素，尾块为19968个half元素 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 场景0 | 未开启Ping/Pong Buffer优化 | 285.001 | 283.88 | 121.923 | 0.429 | 1.729 | 0.006 | 141.735 | 0.499 | 16.039 | 0.056 |
| 场景1 | 开启Ping/Pong Buffer优化 | 270.333 | 269.4 | 120.191 | 0.446 | 2.039 | 0.008 | 262.095 | 0.973 | 141.745 | 0.526 |

**表10**  `FUSED_ITERATIONS=14`性能数据

| 场景 | 优化配置 | Task Duration(μs) | aiv_time(μs) | aiv_vec_time(μs) | aiv_vec_ratio | aiv_scalar_time(μs) | aiv_scalar_ratio | aiv_mte2_time(μs) | aiv_mte2_ratio | aiv_mte3_time(μs) | aiv_mte3_ratio |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 场景0 | 未开启Ping/Pong Buffer优化 | 296.913 | 295.81 | 147.748 | 0.499 | 1.775 | 0.006 | 128.354 | 0.434 | 16.308 | 0.055 |
| 场景1 | 开启Ping/Pong Buffer优化 | 272.609 | 271.62 | 144.743 | 0.533 | 2.142 | 0.008 | 260.462 | 0.959 | 161.335 | 0.594 |

**表11**  `FUSED_ITERATIONS=24`性能数据

| 场景 | 优化配置 | Task Duration(μs) | aiv_time(μs) | aiv_vec_time(μs) | aiv_vec_ratio | aiv_scalar_time(μs) | aiv_scalar_ratio | aiv_mte2_time(μs) | aiv_mte2_ratio | aiv_mte3_time(μs) | aiv_mte3_ratio |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 场景0 | 未开启Ping/Pong Buffer优化 | 420.854 | 419.92 | 308.996 | 0.736 | 1.695 | 0.004 | 91.144 | 0.217 | 16.185 | 0.039 |
| 场景1 | 开启Ping/Pong Buffer优化 | 306.743 | 305.88 | 298.033 | 0.974 | 1.864 | 0.006 | 169.029 | 0.553 | 17.077 | 0.056 |
...
场景1的`aiv_mte2_ratio`为95.9%，当前流水仍主要受MTE2阶段限制。各流水指标包含重叠和等待时间，不能相加或用于比较单次搬运速度；性能结论以`Task Duration`为准。
...
- **实际收益**：`FUSED_ITERATIONS=14`时，`Task Duration`从296.913μs降至272.609μs，缩短24.304μs，降幅为8.19%。
...
| `FUSED_ITERATIONS` | 正整数，默认`14` | $t_{i+1}=t_i \times x+y$的递推次数。 |
