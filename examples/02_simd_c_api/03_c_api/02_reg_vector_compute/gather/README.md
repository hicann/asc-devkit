...
## 概述
本样例基于C API编程接口实现按索引从矢量数据寄存器收集元素功能，主要调用[asc_gather](../../../../../docs/zh/api/SIMD-API/c_api/reg_compute/reg_gather/asc_gather.md)接口。
...
## 样例描述
- 样例功能：
  演示asc_gather接口（源操作数为寄存器）的用法。asc_gather根据索引从源向量中收集元素：dst[i] = src[index[i]]。
  ...
  **Gather模式**
  ...
  - 样例规格：
    ...
    <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">gather_custom</td></tr>
    </table>
  - 样例实现：
    gather_vf函数内调用asc_gather接口按索引收集元素。
    - 调用实现
      使用内核调用符`<<<>>>`调用核函数。
