...
## 概述
本样例基于C API编程接口实现Truncate运算，用于将矢量数据寄存器的浮点数截断到整数位，主要调用[asc_floor](../../../../../docs/zh/api/SIMD-API/C-API/reg/data_type_convert/asc_floor.md)接口。
...
- 样例规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">truncate_custom</td></tr>
  </table>
- 样例实现：  
   truncate_vf函数内调用asc_floor接口进行计算，结果写回UB
  - 调用实现  
    使用内核调用符<<<>>>调用核函数。
