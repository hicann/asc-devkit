...
## 概述
本样例基于C API编程接口实现Abs运算，主要调用[asc_abs](../../../../../docs/zh/api/SIMD-API/C-API/reg/arithmetic_compute/asc_abs.md)接口。
以下单目运算的接口皆参考本样例：
- asc_relu
- asc_exp
- asc_sqrt
...
- 样例规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">abs_custom</td></tr>
  </table>
- 样例实现：  
  abs_vf函数内调用asc_abs接口进行计算，结果写回UB
  - 调用实现  
    使用内核调用符<<<>>>调用核函数。
