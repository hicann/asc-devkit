...
## 概述
本样例基于C API编程接口实现Muls运算，主要调用[asc_mul_scalar](../../../../../docs/zh/api/SIMD-API/c_api/reg_compute/reg_arith/asc_mul_scalar.md)接口。
...
- 样例实现：  
  muls_vf函数内调用asc_mul_scalar接口进行计算，结果写回UB。
- 调用实现  
  使用内核调用符<<<>>>调用核函数。
