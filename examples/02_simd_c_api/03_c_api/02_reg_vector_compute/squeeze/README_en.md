...
## Overview
This example implements the Squeeze operation based on the C API programming interface, primarily calling the [asc_squeeze](../../../../../docs/zh/api/SIMD-API/C-API/reg/compare_and_select/asc_squeeze.md) API.
...
- Example functionality:
  This example sets mask to PAT_M4. In each iteration, elements from the input vector x vector data register whose indices are multiples of 4 are selected and sequentially copied into the output vector y vector data register in a contiguous layout.
- Example specifications:
  ...
  <tr><td rowspan="1" align="center">Kernel Function Name</td><td colspan="3" align="center">squeeze_custom</td></tr>
  </table>
- Example implementation:
  - The squeeze_vf function calls the asc_squeeze API for compression computation, using PAT_M4 mask to extract elements at indices that are multiples of 4 from x vector data register in each iteration
  - Calls asc_loadalign to load data into registers, and after asc_squeeze compression, uses asc_storealign to write the result back to UB
