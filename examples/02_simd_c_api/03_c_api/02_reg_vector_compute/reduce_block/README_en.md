...
## Overview
This example implements the ReduceDataBlock operation based on the C API programming interface, primarily calling the [asc_reduce_sum_datablock](../../../../../docs/zh/api/SIMD-API/c_api/reg_compute/reg_reduce/asc_reduce_sum_datablock.md) API (SUM mode).
...
- In each `one_rep_size` block (for example, 64 elements for float type):
  - The first `blocks_per_repeat` (8) positions store valid reduction results
  - The remaining `one_rep_size - blocks_per_repeat` (56) positions contain invalid data (undefined values)
...
- Example specifications:
  ...
  <tr><td align="center">y</td><td align="center">[1, 32]</td><td align="center">float</td></tr>
  <tr><td rowspan="1" align="center">Kernel Function Name</td><td colspan="3" align="center">reduce_block_custom</td></tr>
  </table>
- Example implementation:
   The reduce_block_vf function calls the asc_reduce_sum_datablock API for reduction computation:
   - Uses asc_loadalign to load data into registers
   - Uses asc_reduce_sum_datablock to sum elements within each DataBlock (32B)
   - Results are stored in dst vector data register with an interleaved layout; the first 8 positions of each repeat are valid results
   - Uses asc_storealign to write results back to UB, with offset using one_rep_size
