...
## 概述
本样例基于C API编程接口实现PairReduceElem运算，主要调用[asc_pair_reduce_sum](../../../../../docs/zh/api/SIMD-API/c_api/reg_compute/reg_reduce/asc_pair_reduce_sum.md)接口。
...
│   ├── reduce_pair.asc                // AscendC样例实现 & 调用样例
│   ├── README.md                      // 样例介绍
...
## 样例描述
- 样例功能：  
  对输入向量做PairReduceElem归约求和运算，向量shape为[1, 256]，数据类型为float。  
  PairReduceElem对相邻奇偶元素对求和：dst[i] = src[2i] + src[2i+1]  
  256个float输入产生128个求和结果。
...
- 样例规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">reduce_pair_custom</td></tr>
  </table>
- 样例实现：
   reduce_pair_vf函数内调用asc_pair_reduce_sum接口进行归约计算：
   - 使用asc_loadalign加载数据到寄存器
   - 使用asc_pair_reduce_sum对相邻奇偶元素对求和
