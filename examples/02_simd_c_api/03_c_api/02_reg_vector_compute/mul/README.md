...
## 概述
本样例基于C API编程接口实现逐元素乘法运算，主要调用[asc_mul](../../../../../docs/zh/api/SIMD-API/C-API/reg/arithmetic_compute/asc_mul.md)接口。
...
- 样例规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">mul_custom</td></tr>
  </table>

- 样例实现：
  mul_vf函数内调用asc_mul接口进行计算，结果写回UB。
  - 调用实现
    使用内核调用符`<<<>>>`调用核函数。
