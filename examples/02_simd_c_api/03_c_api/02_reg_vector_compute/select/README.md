...
## 概述
本样例基于C API编程接口实现Select运算，主要调用[asc_select](../../../../../docs/zh/api/SIMD-API/C-API/reg/compare_and_select/asc_select.md)接口。
...
## 样例描述
- 样例功能：  
  对根据mask中的比特位，选择对应位置的矢量数据寄存器x或矢量数据寄存器y向量中的值，当mask的比特位为1时，选取src0中对应的元素；当mask的比特位为0时，选取src1中对应的元素。
- 样例规格：
  <table>
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="3" align="center">select_custom</td></tr>
  </table>
- 样例实现：  
   select_vf函数内调用asc_select接口进行计算，结果写回UB
  - 调用实现  
    使用内核调用符<<<>>>调用核函数。
