...
#ifndef EXAMPLES_TENSOR_API_EXPERIMENTAL_REG_VECTOR_ADD_DATA_UTILS_H
#define EXAMPLES_TENSOR_API_EXPERIMENTAL_REG_VECTOR_ADD_DATA_UTILS_H
...
inline bool ReadFile(const std::string& file_path, size_t& file_size, void* buffer, size_t buffer_size)
{
    ...
    if (stat(file_path.c_str(), &file_stat) == -1 || S_ISREG(file_stat.st_mode) == 0) {
        ERROR_LOG("Failed to get file information. path = %s", file_path.c_str());
        return false;
    }
    ...
    if (size == 0 || size > buffer_size) {
        ERROR_LOG("Invalid file size. path = %s", file_path.c_str());
        return false;
    }
...
inline bool WriteFile(const std::string& file_path, const void* buffer, size_t size)
{
    ...
    (void)close(fd);
    if (write_size != size) {
        ERROR_LOG("Failed to write file. path = %s", file_path.c_str());
        return false;
    }
    return true;
}

#endif // EXAMPLES_TENSOR_API_EXPERIMENTAL_REG_VECTOR_ADD_DATA_UTILS_H
