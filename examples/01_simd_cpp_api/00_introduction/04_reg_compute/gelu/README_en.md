...
This example implements GELU computation using the RegBase programming approach. Compared to the MemBase approach (based on [LocalTensor](../../../../../docs/zh/api/SIMD-API/基础API/数据结构/LocalTensor/LocalTensor简介.md) + Compute API) used in the [01_add/add example](../../01_add/add/README_en.md), RegBase allows intermediate computation results to be temporarily stored in registers rather than in UB, reducing the number of UB read/write operations, which is suitable for multi-step fusion computation scenarios.
...
This example follows the execution flow of "copy in — synchronize — compute — synchronize — copy out". Input data is copied from [GM](../../../../../docs/zh/guide/技术附录/概念原理和术语/术语表.md) (Global Memory) to [UB](../../../../../docs/zh/guide/技术附录/概念原理和术语/术语表.md) (Unified Buffer), then loaded from UB to registers through [asc_vf_call](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/VF调用/asc_vf_call.md) calling the VF function to complete step-by-step computation, and finally the results are written back from registers to UB, then copied from UB back to GM.
...
- **Synchronization Mechanism**: Since data movement and computation are executed asynchronously by different hardware units, synchronization through the [SetFlag/WaitFlag](../../../../../docs/zh/api/SIMD-API/基础API/同步控制/核内同步/SetFlag_WaitFlag_ISASI.md) mechanism is required for pipeline synchronization. `SetFlag` writes an event flag after a hardware unit completes an operation, and `WaitFlag` makes subsequent hardware units wait for that event to complete before starting execution, thus ensuring the correctness of data dependencies. This example uses two types of events:
...
- [PipeBarrier](../../../../../docs/zh/api/SIMD-API/基础API/同步控制/核内同步/PipeBarrier_ISASI.md) is used to wait for all pipeline stages in the current core to complete before exiting the kernel, ensuring data write-back completion.
- **RegBase Programming Paradigm**: Unlike traditional MemBase (based on [LocalTensor](../../../../../docs/zh/api/SIMD-API/基础API/数据结构/LocalTensor/LocalTensor简介.md) + Compute API), RegBase calls VF functions through [asc_vf_call](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/VF调用/asc_vf_call.md), using [RegTensor](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/寄存器数据类型/RegTensor.md) (register tensor) for computation within the function. VF functions are declared with `__simd_vf__`, and parameters and local variables are marked with `__ubuf__` for UB address space. Within VF functions, data is moved through [LoadAlign](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/Reg数据搬入/LoadAlign_continuous.md) (UB→Register) and [StoreAlign](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/Reg数据搬出/StoreAlign_continuous.md) (Register→UB), using [MaskReg](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/寄存器数据类型/MaskReg.md) (mask register) to control the number of elements for each computation, and updating the mask through [UpdateMask](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/寄存器数据类型/MaskReg.md) based on remaining element count. The advantage is that intermediate results can be temporarily stored in registers, reducing the number of UB read/write operations.
...
| Core Partitioning | Get current core index through [GetBlockIdx](../../../../../docs/zh/api/SIMD-API/基础API/工具接口/系统资源与变量/GetBlockIdx.md), calculate offset addresses for `xGm`/`yGm` | Each core only processes its own continuous data segment to avoid data overlap |
| UB Allocation | Use [LocalMemAllocator](../../../../../docs/zh/api/SIMD-API/基础API/资源管理/LocalMemAllocator/LocalMemAllocator简介.md) to request UB buffers `xLocal`/`yLocal` for the current core | UB is on-chip high-speed cache, providing data staging area for subsequent register computation |
| GM → UB Copy In | Call [DataCopy](../../../../../docs/zh/api/SIMD-API/基础API/Memory矢量计算/数据搬运/数据搬运.md) to copy input data from GM to UB | Registers cannot directly access GM, data must first be copied to UB |
...
| Register Computation | Call `GeluVfMethod2` through [asc_vf_call](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/VF调用/asc_vf_call.md), complete [LoadAlign](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/Reg数据搬入/LoadAlign_continuous.md) → multi-step Reg computation → [StoreAlign](../../../../../docs/zh/api/SIMD-API/基础API/Reg矢量计算/Reg数据搬出/StoreAlign_continuous.md) within VF function | RegBase API executes computation at register level with lowest latency; GELU formula needs to be decomposed into 8 computation steps, using the following APIs: Mul/Muls/Add/Exp/Adds/Div |
...
| Pipeline Synchronization | [PipeBarrier](../../../../../docs/zh/api/SIMD-API/基础API/同步控制/核内同步/PipeBarrier_ISASI.md)`<PIPE_ALL>()` | Wait for all pipeline stages (MTE2/V/MTE3) in the current core to complete before exiting the kernel, ensuring data write-back completion |
...
| Fixed Core Count | Fixed use of 2 cores, not dynamically allocated according to actual available cores | Dynamically get available core count ([GetBlockNum](../../../../../docs/zh/api/SIMD-API/基础API/工具接口/系统资源与变量/GetBlockNum.md)), fully utilize multi-core parallel capability |
