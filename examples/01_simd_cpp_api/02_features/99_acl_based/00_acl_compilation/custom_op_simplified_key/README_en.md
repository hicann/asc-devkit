...
This sample already provides the input configuration directory and Binary JSON, so no manual setup is needed. When developing your own operator, provide the Binary JSON yourself; `op_host/config/<soc>` is only the recommended directory layout, and CMake does not create it automatically. Configuration files under `build` are generated by the build process.
