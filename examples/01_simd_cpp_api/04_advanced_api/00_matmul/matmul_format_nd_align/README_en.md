# Matmul ND_ALIGN Format Output Direct Invocation Example
## Overview
A Matmul example that enables N-direction alignment output when the N direction of the input matrix is not aligned. By setting the output matrix C to ND_ALIGN format, the Matmul API outputs the C matrix with 32-byte alignment padding in the N direction.
