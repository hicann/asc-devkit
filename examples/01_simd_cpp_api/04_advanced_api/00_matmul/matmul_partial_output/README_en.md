# Matmul Partial Output Direct Invocation Example
## Overview
An example that implements the Partial Output feature using the Matmul high-level API. The Partial Output feature applies to scenarios where the matrix multiplication result does not need accumulation and only requires outputting the computation result baseM*baseN from baseM*baseK and baseK*baseN.
