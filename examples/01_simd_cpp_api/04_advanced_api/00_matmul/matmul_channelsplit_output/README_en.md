# Matmul Output Channel Split Direct Invocation Example
## Overview
A Matmul example with the output Channel split feature. When the format of the Matmul computation result C matrix is NZ, the C matrix uses fractal storage. If the C matrix data type is float, the default fractal size is 16 * 16. The Channel split feature can split each 16 * 16 fractal of the C matrix into 16 * 8 fractals, enabling the C matrix to be stored in 16 * 8 fractals.
