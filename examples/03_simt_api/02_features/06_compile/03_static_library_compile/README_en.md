...
Use the `-lib` option to compile the `.asc` mixed file into a static library (.a). The specific compilation command is as follows:

```shell
bisheng -lib kernel.asc -o libkernel.a --npu-arch=dav-3510 --enable-simt
```
...
For more bisheng compilation options and usage, refer to [AI-Core Operator Compilation Basic Usage](../../../../../docs/zh/guide/programming_guide/compilation_and_execution/operator_compilation/ai_core_operator_compilation.md).
...
- Run the Example
  ...
  - Command Line Method
    ...
    ```bash
    bisheng -lib gather_kernel.asc -o libgather_kernel.a --npu-arch=dav-3510 --enable-simt     # Compile .asc to static library
    bisheng main.cpp -o demo -L./ -L${ASCEND_HOME_PATH}/lib64 -I${ASCEND_HOME_PATH}/include \
      -lgather_kernel -lascendcl -lruntime -lstdc++                                            # Link to generate executable file
