...
使用 `-lib` 选项将 `.asc` 混合文件编译为静态库（.a）。具体编译命令如下：

```shell
bisheng -lib kernel.asc -o libkernel.a --npu-arch=dav-3510 --enable-simt
```

其中，`--npu-arch` 为编译时指定的 AI 处理器架构，本样例仅支持 `dav-3510`，各产品型号对应的架构版本号请通过[对应关系表](../../../../../docs/zh/guide/programming_guide/language_extension/simd_builtin_keywords.md#npu-arch)进行查询。`--enable-simt` 用于指定SIMT方式编译。
...
- 样例执行
  ...
  - 命令行方式
    ...
    ```bash
    bisheng -lib gather_kernel.asc -o libgather_kernel.a --npu-arch=dav-3510 --enable-simt     # 编译 .asc 为静态库
    bisheng main.cpp -o demo -L./ -L${ASCEND_HOME_PATH}/lib64 -I${ASCEND_HOME_PATH}/include \
      -lgather_kernel -lascendcl -lruntime -lstdc++                                            # 链接生成可执行文件
    ./demo                                                                                      # 执行样例
    ```
