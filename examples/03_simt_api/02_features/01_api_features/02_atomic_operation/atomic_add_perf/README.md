...
本样例以[asc_atomic_add()](../../../../../../docs/zh/api/SIMT-API/atomic_operations/asc_atomic_add.md)接口为例，通过五组对照实验，展示不同内存空间（Global Memory / Unified Buffer）、不同原子累加组织方式（直接原子加 / 线程块内先累加再写回GM）、是否使用返回值、同一地址上的竞争强度，以及数据类型（int32 / int64）等不同使用场景下的接口性能差异，并给出相应的优化指导。
...
|:---:|---:|---:|:---:|
| GM int32（场景5、6） | 421.59 | 159.88 | 2.64× |
| UB int32（场景7、8） | 3.17 | 2.76 | 1.15× |
...
> [!NOTE]说明
> 本组场景统一使用返回值：由Case 3可知，不使用返回值时int32会触发指令优化，其耗时主要由该优化决定，会掩盖竞争强度和目标地址在128B范围内的分布对性能的影响，因此int32的case需要使用返回值版本；为保持一致，int64也使用返回值。这样可使int32与int64的耗时差异仅反映竞争强度与目标地址分布带来的性能差异。
...
<img src="figures/case4_1.png" alt="Case4 int32与int64竞争强度对比" style="width: 70%; height: auto;">

**分析**：

性能数据呈现出四个现象：

1. **竞争越强，耗时越高**。以int32为例，每个地址上的竞争线程数从1（无竞争）增加到12，再增加到384，耗时从15.54μs上升至111.30μs，再上升至2233.62μs，表明耗时随竞争强度显著增加。
2. **竞争增强到一定程度后，耗时不再上升，而是趋于饱和**。int32在`target_count = 32`（384线程/地址）与`target_count = 1`（12288线程/地址）时耗时接近（2233.62μs与2136.16μs）。竞争线程数增加32倍，耗时基本保持不变。
3. **相同竞争强度下，int64的耗时短于int32**。`target_count = 32`时，int64耗时1120.19μs，约为int32耗时（2233.62μs）的一半。该结果与“数据类型位宽越小、性能越优”的预期不一致。
4. **竞争最强（`target_count = 1`）时，int32与int64耗时基本一致**。场景12（int32）的耗时为2136.16μs，场景16（int64）的耗时为2135.92μs，两者基本相等，现象3所示的性能差异不再体现。

如Case 1所述，同一地址上的原子操作只能串行执行。`target_count`越小，每个地址上的竞争线程数越多，排队等待越久，因此耗时越高，这解释了现象1。但同一地址上的竞争无法解释耗时趋于饱和以及int32与int64之间的性能差异。要解释现象2～4，还需进一步了解原子操作的原理。

**原理：GM原子操作以Sector为粒度。**

原子加访问GM地址时，会经过L2缓存层级，由L2侧的原子处理单元完成“读—算—写”。L2 Cache以512B Cache Line为缓存管理单位，每条Cache Line包含4个连续的128B Sector；GM原子操作以128B Sector为处理粒度。原子操作的目标地址集中在同一个Sector内时，处理效率较低；目标地址分布在更多Sector内时，处理效率较高。

这一原理解释了现象2。以int32为例，一个128B Sector可容纳32个int32地址（32 × 4B = 128B）。`target_count = 32`时，32个地址恰好全部位于同一个Sector内；`target_count = 1`时，所有线程集中到唯一一个地址，同样位于一个Sector内。由于GM原子操作以128B Sector为处理粒度，两种情形的目标地址均位于同一个Sector内，因此处理效率接近。相应地，从`target_count = 32`到`target_count = 1`，耗时未继续增加，而是趋于饱和（2233.62μs与2136.16μs基本持平）。

在这一原理的基础上进一步看数据类型的影响。由于GM原子操作以128B Sector为处理粒度，数据类型位宽会影响相同数量的连续地址所覆盖的Sector数量。地址数量相同时，int64覆盖的Sector数量更多，目标地址分布更分散，处理效率更高。

一个128B Sector可容纳的地址数量取决于单个地址的位宽：

| 数据类型 | 单个地址大小 | 每个128B Sector容纳地址数 |
|:---:|:---:|:---:|
| int32 | 4B | 128B / 4B = 32 |
| int64 | 8B | 128B / 8B = 16 |
...
- **现象3（`target_count = 32`时int64耗时更短）**：由于GM原子操作以128B Sector为处理粒度，32个int32地址位于一个Sector内，而相同数量的int64地址分布在2个Sector内。int64覆盖的Sector数量更多，处理效率更高，因此耗时更短。
- **现象4（`target_count = 1`时两者耗时基本一致）**：由于GM原子操作以128B Sector为处理粒度，所有线程访问同一个地址时，无论使用int32还是int64，目标地址均位于一个Sector内，因此处理效率接近，两者耗时基本相等（2136.16μs与2135.92μs）。

上述结果表明，此处的性能差异与数据类型位宽本身无关，仅取决于原子操作地址在128B Sector上的分布密度。


**结论**：原子加性能对同一地址上的竞争强度和目标地址在128B Sector内的分布高度敏感。竞争越强、目标地址越集中，处理效率越低、耗时越高。应尽量分散原子更新的目标地址。

---

### Case 5：数据类型的取舍

**样例目标**：在GM上存在同地址竞争的场景下，对比int32与int64原子加的性能，说明数据类型的选择取决于是否使用返回值，不能仅依据“数据类型位宽越小、性能越优”的经验进行判断。
...
|:---:|---:|---:|:---:|
| 不使用返回值（场景17、18） | 1.89 | 58.46 | int32耗时较短，约为int64的1/31 |
| 使用返回值（场景19、20） | 110.95 | 54.28 | int32耗时较长，约为int64的2倍 |

**分析**：

int32与int64在两种情形下的性能关系相反：不使用返回值时，int32与int64的耗时分别为1.89μs和58.46μs；使用返回值时，int32与int64的耗时分别为110.95μs和54.28μs。两种情形分别由不同机制主导：

- **不使用返回值时**，int32原子加执行优化后的指令。在约12个线程竞争同一地址的场景下，int32与int64的耗时分别为1.89μs和58.46μs，表明int32的处理效率显著高于int64。
- **使用返回值时**，int32无法触发上述优化。由于GM原子操作以128B Sector为处理粒度，一个Sector可容纳32个int32地址，但仅容纳16个int64地址。地址数量相同时，int32覆盖的Sector数量少于int64，目标地址分布更集中，因此处理效率低于int64，其耗时约为int64的2倍。128B处理粒度的详细说明见Case 4。

上述结果表明，int32与int64的性能关系并非仅由数据类型位宽决定，还取决于是否使用返回值。

**结论**：应根据是否使用返回值选择数据类型。纯计数等不使用返回值的场景可优先选择int32，编译器可为该类型生成优化后的指令；在使用返回值且目标地址紧密排布的场景下，int32的目标地址在128B Sector内更集中，耗时高于int64。此时可通过padding或结构体对齐分散目标地址，以提升处理效率。
