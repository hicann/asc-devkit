...
TEST_F(tensor_api_cube_copy_3510, copy_l1_to_fb_routes_to_cube_arch_copy)
{
    ...
    auto l1_tensor =
        make_tensor_at<location::l1>(src, make_frame_layout<nd_ext_layout_ptn, layout_trait_default<uint64_t>>(m, n));
    auto fixbuf_tensor = make_tensor_at<location::fixbuf>(
        dst, make_frame_layout<nd_ext_layout_ptn, layout_trait_default<uint64_t>>(m, n));
...
TEST_F(tensor_api_cube_copy_3510, copy_l1_to_fb_nd_layout_routes_to_cube_arch_copy)
{
    ...
    auto l1_tensor =
        make_tensor_at<location::l1>(src, make_frame_layout<nd_layout_ptn, layout_trait_default<uint64_t>>(m, n));
    auto fixbuf_tensor =
        make_tensor_at<location::fixbuf>(dst, make_frame_layout<nd_layout_ptn, layout_trait_default<uint64_t>>(m, n));
...
__aicore__ inline void copy_cbuf_to_fbuf_stub(
    __fbuf__ void* dst, __cbuf__ void* src, uint16_t block_count, uint16_t block_len, uint16_t src_stride,
    uint16_t dst_stride)
{
...
#define DATA_COPY_TEST_L12FB_ND2ND(data_type, src_size1, src_size2, dst_size1, dst_size2)                                \
    TEST_F(                                                                                                              \
        ...
        test_tensor_api_data_copy_l1_to_fb_nd_to_nd_##data_type##_##src_size1##x##src_size2##_##dst_size1##x##dst_size2) \
    {                                                                                                                    \
        using namespace asc::te;                                                                                         \
