...
void run_copy_call_paths(const dst_tensor_type& dst, const src_tensor_type& src)
{
    ...
    if constexpr (Std::is_same_v<trait_type, l0c_to_l1_trait_default>) {
        copy(dst, src);
    }
}
...
TEST_F(tensor_api_cube_copy_3510, copy_l0c_to_l1_nz_to_nz_float)
{
    ...
    copy(atom.with(l0c_to_l1_params{}), l1_tensor, l0c_tensor, zero_coord, make_coord(0, 0), make_shape(16, 16));
...
TEST_F(tensor_api_cube_copy_3510, copy_l0c_to_l1_batch_tensor_quant_coord_shape_compiles)
{
    ...
    auto dst_tensor = make_tensor_at<location::l1>(
        dst, make_frame_layout<nz_layout_ptn, layout_trait_default<int8_t, _16>>(batch, m, n));
