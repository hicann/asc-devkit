...
TEST_F(tensor_api_cube_copy_3510, copy_l1_to_bt_routes_to_cube_arch_copy)
{
    ...
    auto l1_tensor =
        make_tensor_at<location::l1>(src, make_frame_layout<nd_ext_layout_ptn, layout_trait_default<float>>(m, n));
    auto biastable_tensor =
        make_tensor_at<location::bias>(dst, make_frame_layout<nd_ext_layout_ptn, layout_trait_default<float>>(m, n));
...
TEST_F(tensor_api_cube_copy_3510, copy_l1_to_bt_nd_layout_routes_to_cube_arch_copy)
{
    ...
    auto l1_tensor =
        make_tensor_at<location::l1>(src, make_frame_layout<nd_layout_ptn, layout_trait_default<float>>(m, n));
    auto biastable_tensor =
        make_tensor_at<location::bias>(dst, make_frame_layout<nd_layout_ptn, layout_trait_default<float>>(m, n));
...
TEST_F(tensor_api_cube_copy_3510, copy_l1_to_bt_offsets)
{
    ...
    copy(
        make_copy(copy_l1_to_biastable{}, l1_to_biastable_trait_default{}), dst_tensor, src_tensor, make_coord(0, 16),
        make_coord(0, 8), make_shape(1, 16));
    GlobalMockObject::verify();
}

enum class cube_layout { RowMajor, NZ, ColumnMajor, ZN, ND };
...
constexpr int test_ceil_division(int value, int divisor) { return (value + divisor - 1) / divisor; }
...
__aicore__ inline void copy_cbuf_to_bt_stub(
    uint64_t dst, __cbuf__ data_type* src, uint16_t conv_control, uint16_t block_count, uint16_t block_len,
...
__aicore__ inline void copy_cbuf_to_bt_two_type_stub(
    uint64_t dst, __cbuf__ src_data_type* src, uint16_t conv_control, uint16_t block_count, uint16_t block_len,
    uint16_t src_stride, uint16_t dst_stride)
{
...
#define CREATE_TENSOR(                                                                                               \
    data_type, src_size1, src_size2, dst_size1, dst_size2, src_prefix, src_location, src_layout_pattern, dst_prefix, \
    dst_location, dst_layout_pattern)                                                                                \
    using namespace asc::te;                                                                                         \
    ...
    auto src_layout = make_frame_layout<MAKE_LAYOUT_TYPE(src_layout_pattern), layout_trait_default<data_type>>(      \
        src_size1, src_size2);                                                                                       \
    auto src_tensor = make_tensor(src_iterator, src_layout);                                                         \
                                                                                                                     \
    auto dst_iterator = make_mem_ptr<location::bias>(dst_data);                                                      \
    auto dst_layout = make_frame_layout<MAKE_LAYOUT_TYPE(dst_layout_pattern), layout_trait_default<data_type>>(      \
        dst_size1, dst_size2);                                                                                       \
    auto dst_tensor = make_tensor(dst_iterator, dst_layout);

// create tensor
#define CREATE_TENSOR_TWO_TYPE(                                                                                     \
    src_data_type, src_size1, src_size2, dst_data_type, dst_size1, dst_size2, src_prefix, src_location,             \
    ...
    __##dst_prefix##__ dst_data_type dst_data[dst_size1 * dst_size2 * sizeof(dst_data_type)];                       \
                                                                                                                    \
    auto src_iterator = make_mem_ptr<location::l1>(src_data);                                                       \
    auto src_layout = make_frame_layout<MAKE_LAYOUT_TYPE(src_layout_pattern), layout_trait_default<src_data_type>>( \
        src_size1, src_size2);                                                                                      \
    auto src_tensor = make_tensor(src_iterator, src_layout);                                                        \
                                                                                                                    \
    auto dst_iterator = make_mem_ptr<location::bias>(dst_data);                                                     \
    auto dst_layout = make_frame_layout<MAKE_LAYOUT_TYPE(dst_layout_pattern), layout_trait_default<dst_data_type>>( \
        dst_size1, dst_size2);                                                                                      \
    auto dst_tensor = make_tensor(dst_iterator, dst_layout);
...
#define DATA_COPY_TEST_L12BIAS_ND2ND(data_type, src_size1, src_size2, dst_size1, dst_size2)                                \
    TEST_F(                                                                                                                \
        ...
        test_tensor_api_data_copy_l1_to_bias_nd_to_nd_##data_type##_##src_size1##x##src_size2##_##dst_size1##x##dst_size2) \
    {                                                                                                                      \
        using namespace asc::te;                                                                                           \
        MOCKER_CPP(                                                                                                        \
            copy_cbuf_to_bt, void(uint64_t, __cbuf__ data_type*, uint16_t, uint16_t, uint16_t, uint16_t, uint16_t))        \
            .times(1)                                                                                                      \
...
#define DATA_COPY_TEST_L12BIAS_TWO_TYPE_ND2ND(                                                                                                            \
    src_data_type, src_size1, src_size2, dst_data_type, dst_size1, dst_size2)                                                                             \
    TEST_F(                                                                                                                                               \
        tensor_api_cube_copy_3510,                                                                                                                        \
        test_tensor_api_data_copy_l1_to_bias_two_type_nd_to_nd_##src_data_type##_##src_size1##x##src_size2##_##dst_data_type##_##dst_size1##x##dst_size2) \
    {                                                                                                                                                     \
        using namespace asc::te;                                                                                                                          \
        MOCKER_CPP(                                                                                                                                       \
            copy_cbuf_to_bt,                                                                                                                              \
            void(uint64_t, __cbuf__ src_data_type*, uint16_t, uint16_t, uint16_t, uint16_t, uint16_t))                                                    \
            .times(1)                                                                                                                                     \
            .will(invoke(&copy_cbuf_to_bt_two_type_stub<                                                                                                  \
                         src_data_type, dst_data_type, src_size1, src_size2, dst_size1, dst_size2>));                                                     \
        CREATE_TENSOR_TWO_TYPE(                                                                                                                           \
            src_data_type, src_size1, src_size2, dst_data_type, dst_size1, dst_size2, cbuf, L1, ND, biasbuf, BIAS, ND)                                    \
        copy(copy_atom<copy_traits<copy_l1_to_biastable, l1_to_biastable_trait_default>>{}, dst_tensor, src_tensor);                                      \
        GlobalMockObject::verify();                                                                                                                       \
    }
...
#define DATA_COPY_TEST_L12BIAS_BATCH_ND2ND_COMPACT(data_type, batch_value, m_value, n_value)                         \
    TEST_F(                                                                                                          \
        tensor_api_cube_copy_3510,                                                                                   \
        test_tensor_api_data_copy_l1_to_bias_batch_nd_to_nd_##data_type##_##batch_value##x##m_value##x##n_value)     \
    {                                                                                                                \
        using namespace asc::te;                                                                                     \
        MOCKER_CPP(                                                                                                  \
            copy_cbuf_to_bt, void(uint64_t, __cbuf__ data_type*, uint16_t, uint16_t, uint16_t, uint16_t, uint16_t))  \
...
__aicore__ inline void copy_cbuf_to_bt_batch_compact_half_to_float_stub(
    uint64_t dst, __cbuf__ half* src, uint16_t conv_control, uint16_t block_count, uint16_t block_len,
    uint16_t src_stride, uint16_t dst_stride)
{
    EXPECT_EQ(conv_control, 1);
    EXPECT_EQ(block_count, batch_value);
...
#define DATA_COPY_TEST_L12BIAS_BATCH_ND2ND_HALF2FLOAT(batch_value, m_value, n_value)                                  \
    TEST_F(                                                                                                           \
        tensor_api_cube_copy_3510,                                                                                    \
        test_tensor_api_data_copy_l1_to_bias_batch_nd_to_nd_half_to_float_##batch_value##x##m_value##x##n_value)      \
    {                                                                                                                 \
        using namespace asc::te;                                                                                      \
