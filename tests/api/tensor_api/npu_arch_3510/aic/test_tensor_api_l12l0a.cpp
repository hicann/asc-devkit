...
template<typename T, int M_STEP, int K_STEP, int SRC_STRIDE, int DST_STRIDE, uint32_t SRC_BATCH_STRIDE,
         uint32_t DST_BATCH_STRIDE, uint32_t DST_SPLIT_STRIDE, uint32_t SPLIT_NUM>
void load_cbuf_to_ca_batch_b4_split_stub(__ca__ T* dst, __cbuf__ T* src,
                                         uint16_t mStartPosition, uint16_t kStartPosition,
                                         uint8_t mStep, uint8_t kStep,
                                         int16_t srcStride, uint16_t dstStride,
                                         bool transposed) {
...
TEST_F(Tensor_Api_Cube_Copy_3510, CopyL12L0ABatchZn2NzB8B4)
{
    ...
    MOCKER_CPP(load_cbuf_to_ca_s4,
        ...
        .will(invoke(&load_cbuf_to_ca_batch_b4_split_stub<fp4x2_e1m2_t, 4, 1, 8, 1, srcBatchStride,
            dstBatchStride, dstSplitStride, splitNum>));
