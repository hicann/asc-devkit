...
private:
    ...
    __aicore__ inline static void load_data(const T& dst, const U& src)
    {
        ...
        static_assert(Std::is_same_v<dst_pos, location::l0scalea>, "When Copy tensor from L1 to L0ScaleA, dst tensor must on L0ScaleA");
        static_assert(Std::is_same_v<src_pos, location::l1>, "When Copy tensor from L1 to L0ScaleA, src tensor must on L1");
        ...
        const T& dst, const U& src, const DstCoord& coord_dst, const SrcCoord& coord_src, const ShapeType& copy_shape)
    {
        ...
        using impl_type = typename copy_l1_to_l0scalea_routing<CURRENT_ARCH_VERSION, dst_pattern, src_pattern>::type;
