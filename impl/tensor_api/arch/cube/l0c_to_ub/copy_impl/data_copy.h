...
public:
        ...
        const T& dst, const U& src, const DstCoord& coord_dst, const SrcCoord& coord_src, const ShapeType& copy_shape, const fixpipe_params& params)
    {
        ...
        copy_l0c_to_ub_instr::data_copy_with_offset<quant_pre, T, U>(dst, src, dst_offset, src_offset, copy_params.n_size,
            copy_params.m_size, copy_params.src_stride, copy_params.dst_stride, trait.dual_dst_ctl,
            trait.enable_relu, params.unit_flag, params.sub_block_id, trait.enable_channel_split, is_nd_format, is_dn_format);
    }
...
public:
        ...
        const DstCoord& coord_dst, const SrcCoord& coord_src, const ShapeType& copy_shape, const fixpipe_params& params)
    {
        ...
        for (uint16_t i = 0; i < n_iter_num; ++i) {
            ...
            copy_l0c_to_ub_instr::data_copy_with_offset<quant_pre>(dst, src(src_coord), dst_offset, src_offset, cal_n_size,
                copy_params.m_size, copy_params.src_stride, copy_params.dst_stride, trait.dual_dst_ctl,
                trait.enable_relu, params.unit_flag, params.sub_block_id, trait.enable_channel_split,
                is_l0c_out_nd_format_v<T>, is_l0c_out_dn_format_v<T>);
        }
    }
...
public:
    ...
    template <const copy_l0c_to_ub_trait& trait, typename T, typename U, typename DstCoord, typename SrcCoord, typename ShapeType>
        ...
        const fixpipe_params& params)
    {
        ...
        check_data_type::check_l0c_to_ub_data_type<quant_pre, T, U>();
        set_register_impl<T, U>(dst, src);
        data_copy_l0c_to_ub_no_vector_quant::data_copy_with_offset<trait, quant_pre>(dst, src, coord_dst, coord_src, copy_shape, params);
    }
    ...
    template <const copy_l0c_to_ub_trait& trait, typename T, typename U, typename V, typename DstCoord, typename SrcCoord, typename ShapeType>
