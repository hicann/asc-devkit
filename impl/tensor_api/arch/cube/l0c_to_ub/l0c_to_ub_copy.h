...
#warning \
    "impl/tensor_api/arch/cube/l0c_to_ub/l0c_to_ub_copy.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "#include "tensor_api/tensor.h"" and use public functions or variables defined in interface headers files."
#define ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS
#define UNDEF_ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS_ASCENDC
#endif

/*!
 * \file l0c_to_ub_copy.h
 * \brief
 */
...
public:
    ...
    __aicore__ inline static void data_copy_impl(
        const DstTensor& dst, const SrcTensor& src, const l0c_to_ub_params& params = default_l0c_to_ub_params)
    {
        using dst_pos = get_mem_location<DstTensor>;
        using src_pos = get_mem_location<SrcTensor>;
        static_assert(
            Std::is_same_v<dst_pos, location::ub>, "For copy_l0c_to_ub, the destination tensor must be located in UB.");
        static_assert(
            Std::is_same_v<src_pos, location::l0c>, "For copy_l0c_to_ub, the source tensor must be located in L0C.");
    ...
    __aicore__ inline static void data_copy_impl(
        ...
        const CopyShape& copy_shape, const l0c_to_ub_params& params = default_l0c_to_ub_params)
    {
        using dst_pos = get_mem_location<DstTensor>;
        using src_pos = get_mem_location<SrcTensor>;
        static_assert(
        ...
        const l0c_to_ub_params& params = default_l0c_to_ub_params)
    {
        ...
        auto resolved_dst_coord = resolve_copy_coord(dst.layout(), copy_shape, dst_coord);
        auto resolved_src_coord = resolve_copy_coord(src.layout(), copy_shape, src_coord);
        copy_l0c_to_ub_impl::template run<trait>(
