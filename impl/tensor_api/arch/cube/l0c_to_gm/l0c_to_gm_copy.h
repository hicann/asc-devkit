...
#warning \
    "impl/tensor_api/arch/cube/l0c_to_gm/l0c_to_gm_copy.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "#include "tensor_api/tensor.h"" and use public functions or variables defined in interface headers files."
#define ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS
#define UNDEF_ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS_ASCENDC
#endif

/*!
 * \file l0c_to_gm_copy.h
 * \brief
 */
...
public:
    ...
    __aicore__ inline static void data_copy_impl(
        const DstTensor& dst, const SrcTensor& src, const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        using dst_pos = get_mem_location<DstTensor>;
        using src_pos = get_mem_location<SrcTensor>;
        static_assert(
            Std::is_same_v<dst_pos, location::gm>, "For copy_l0c_to_gm, the destination tensor must be located in GM.");
        static_assert(
            Std::is_same_v<src_pos, location::l0c>, "For copy_l0c_to_gm, the source tensor must be located in L0C.");
        ...
        using copy_l0c_to_gm_impl =
            typename copy_l0c_to_gm_routing<current_arch_version, dst_layout_ptn, src_layout_ptn>::type;
    ...
    __aicore__ inline static void data_copy_impl(
        ...
        const CopyShape& copy_shape, const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        using dst_pos = get_mem_location<DstTensor>;
        using src_pos = get_mem_location<SrcTensor>;
        static_assert(
        ...
        using copy_l0c_to_gm_impl =
            typename copy_l0c_to_gm_routing<current_arch_version, dst_pattern, src_pattern>::type;
        ...
        copy_l0c_to_gm_impl::template run<trait>(dst, src, resolved_dst_coord, resolved_src_coord, copy_shape, params);
    }
    ...
    __aicore__ inline static typename Std::enable_if<Std::is_same_v<Quant, uint64_t>, void>::type data_copy_impl(
        ...
        const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        using dst_pos = get_mem_location<DstTensor>;
        using src_pos = get_mem_location<SrcTensor>;
        static_assert(
        ...
        const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        ...
        auto resolved_dst_coord = resolve_copy_coord(dst.layout(), copy_shape, dst_coord);
        auto resolved_src_coord = resolve_copy_coord(src.layout(), copy_shape, src_coord);
        copy_l0c_to_gm_impl::template run<trait>(
...
__aicore__ inline void copy_l0c_to_gm::copy(const Args&... args)
{
    copy_l0c_to_gm_base::data_copy_impl<trait>(args...);
}
