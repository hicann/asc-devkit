...
public:
                                                 ...
                                                 const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        ...
        static_assert(Std::is_same_v<dst_pos, location::gm>,
                      "For copy_l0c_to_gm, the destination tensor must be located in GM.");
        static_assert(Std::is_same_v<src_pos, location::l0c>,
                      "For copy_l0c_to_gm, the source tensor must be located in L0C.");
                   ...
                   const l0c_to_gm_params& params = default_l0c_to_gm_params)
    {
        ...
        auto resolved_dst_coord = resolve_copy_coord(dst.layout(), copy_shape, dst_coord);
        auto resolved_src_coord = resolve_copy_coord(src.layout(), copy_shape, src_coord);
        copy_l0c_to_gm_impl::template run<trait>(dst, src, quant, resolved_dst_coord, resolved_src_coord, copy_shape,
                                                 params);
    }
