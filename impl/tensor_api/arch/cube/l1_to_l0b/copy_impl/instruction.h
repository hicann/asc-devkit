...
public:
    ...
    __aicore__ inline static void load_data(__cb__ DataType* dst, __cbuf__ DataType* src, uint16_t m_start_position,
                                            uint16_t k_start_position, uint8_t m_step, uint8_t k_step,
                                            int16_t src_stride, uint16_t dst_stride)
    {
        TENSOR_API_DEBUG_CHECK(debug_check_l0_transpose, transpose, sizeof(DataType), k_step, "copy_l1_to_l0b");
        if constexpr (transpose) {
            asc_copy_l12l0b_transpose(dst, src, m_start_position, k_start_position, m_step, k_step, src_stride,
                                      dst_stride);
        } else {
