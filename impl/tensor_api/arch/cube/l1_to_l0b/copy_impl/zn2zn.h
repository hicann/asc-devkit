...
public:
                                      ...
                                      const SrcCoord& src_coord, const CopyShape& copy_shape)
    {
        ...
        auto m_step = get<1>(copy_shape) / fractal_fixed;
        auto k_step = get<0>(copy_shape) / c0_element<dst_type>;
        ...
        load_l1_to_l0b_instr::load_data_with_offset<false>(
...
private:
    ...
    __aicore__ inline static void load_data_impl(const DstTensor& dst, const SrcTensor& src)
    {
        ...
        auto dst_stride = get_element<attr_info::stride, attr_info::row, 1>(dst_layout) / stride_unit;
        load_l1_to_l0b_instr::load_data<false>(dst.data().get(), src.data().get(), m_start_position, k_start_position,
    ...
    __aicore__ inline static void batch_load_data_impl(const DstTensor& dst, const SrcTensor& src)
    {
        ...
        auto k_step =
            get<0>(dst_layout.shape()) * get_element<attr_info::shape, attr_info::row, 1>(dst_no_batch_layout);
