...
public:
    ...
    __aicore__ inline static void run(const DstTensor& dst, const SrcTensor& src)
    {
        if constexpr (sizeof(typename DstTensor::element_type) == 1) {
            load_data_l1_to_l0b_nz2zn_b8b4::template run<trait, DstTensor, SrcTensor>(dst, src);
        } else {
    ...
    {
        if constexpr (sizeof(typename DstTensor::element_type) == 1) {
            load_data_l1_to_l0b_nz2zn_b8b4::template run<trait, DstTensor, SrcTensor>(dst, src, dst_coord, src_coord,
                                                                                     copy_shape);
        } else {
            check_template<trait, DstTensor, SrcTensor>();
...
private:
                                                    ...
                                                    const DstLayout& dst_layout, const SrcLayout& src_layout)
    {
        ...
        load_l1_to_l0b_instr::load_data<true>(dst.data().get(), src.data().get(), m_start_position, k_start_position,
