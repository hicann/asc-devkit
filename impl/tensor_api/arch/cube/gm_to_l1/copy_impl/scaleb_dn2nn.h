...
public:
    ...
    __aicore__ inline static void run(const DstTensor& dst, const SrcTensor& src)
    {
        ...
        run_gm_to_l1_batched<trait, copy_gm_to_l1_scaleb_dn2nn, DstTensor, SrcTensor>(dst, src);
    }
                                      ...
                                      const SrcCoord& src_coord, const CopyShape& copy_shape)
    {
        ...
        uint16_t dst_stride =
            get_matrix_element<attr_info::stride, attr_info::column, 1>(dst.layout()) * sizeof(type) / c0_size<>;
                                            ...
                                            uint32_t dst_nz_matrix_stride)
    {
        ...
        copy_gm_to_l1_multi_dn2nz_instr::data_copy(reinterpret_cast<__cbuf__ half*>(dst.data().get()),
                                                   reinterpret_cast<__gm__ half*>(src.data().get()), dn_num,
