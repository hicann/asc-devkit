...
#warning \
    "impl/tensor_api/arch/cube/gm_to_l1/copy_impl/scalea_dn2zz.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "#include "tensor_api/tensor.h"" and use public functions or variables defined in interface headers files."
...
class copy_gm_to_l1_scalea_dn2zz {
public:
    ...
    __aicore__ inline static void run(const DstTensor& dst, const SrcTensor& src)
    {
        if constexpr (
            ...
            TENSOR_API_DEBUG_CHECK(
                debug_check_batch_count, get<0>(src.layout().shape()), "copy_gm_to_l1 scalea_dn2zz path");
        }
        run_gm_to_l1_batched<trait, copy_gm_to_l1_scalea_dn2zz, DstTensor, SrcTensor>(dst, src);
    }
        ...
        const CopyShape& copy_shape)
    {
        ...
        uint16_t n_value = get_shape_columns(src_shape);
        ...
        copy_gm_to_l1_multi_nd2nz_instr::data_copy_with_offset(
            ...
            src_col_stride * sizeof(type), static_cast<asc_load_l2_cache_mode>(src.engine().get_cache_mode()), n_value,
        ...
        uint16_t nd_num, uint64_t src_nd_matrix_stride, uint32_t dst_nz_matrix_stride)
    {
        ...
        if constexpr (is_satisfied_ptn_format_v<SrcTensor, dn_layout_ptn>) {
            ...
            src_col_stride = get_element<attr_info::stride, attr_info::column>(src_layout);
        } else {
            ...
            src_col_stride = get_element<attr_info::stride, attr_info::column, 1>(src_layout);
        }
        ...
        uint16_t dst_nz_c0_stride = dst_b_row_stride * sizeof(type) / c0_size<>;
        ...
        copy_gm_to_l1_multi_nd2nz_instr::data_copy(
            ...
            nd_num, loop2_dst_stride, loop3_dst_stride, loop4_dst_stride, loop1_src_stride, cache_mode, n_value,
