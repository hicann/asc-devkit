...
__aicore__ inline void copy_gm_to_l1::data_copy_impl(const DstTensor& dst, const SrcTensor& src)
{
    ...
    static_assert(Std::is_same_v<dst_pos, location::l1>,
                  "For copy_gm_to_l1, the destination tensor must be located in L1.");
...
template <const gm_to_l1_trait& trait, typename DstTensor, typename SrcTensor, typename DstCoord,
    ...
    const DstCoord& dst_coord, const SrcCoord& src_coord, const CopyShape& copy_shape)
{
    ...
    using copy_gm_to_l1_impl =
        typename copy_gm_to_l1_routing<current_arch_version, dst_pattern, src_pattern>::type;
