...
#warning \
    "impl/tensor_api/arch/cube/l1_to_bt/l1_to_bt_copy.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "#include "tensor_api/tensor.h"" and use public functions or variables defined in interface headers files."
#define ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS
#define UNDEF_ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS_ASCENDC
#endif

/*!
 * \file l1_to_bt_copy.h
 * \brief
 */
...
__aicore__ inline void copy_l1_to_biastable::data_copy_impl(const DstTensor& dst, const SrcTensor& src)
{
    ...
    static_assert(
        Std::is_same_v<dst_pos, location::bias>,
        "For copy_l1_to_biastable, the destination tensor must be located in BIAS.");
    static_assert(
        Std::is_same_v<src_pos, location::l1>, "For copy_l1_to_biastable, the source tensor must be located in L1.");
...
template <
    const l1_to_biastable_trait& trait, typename DstTensor, typename SrcTensor, typename DstCoord, typename SrcCoord,
    typename CopyShape>
__aicore__ inline void copy_l1_to_biastable::data_copy_impl(
