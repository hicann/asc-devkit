...
#warning \
    "impl/tensor_api/arch/cube/l1_to_l0scaleb/l1_to_l0scaleb_copy.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "#include "tensor_api/tensor.h"" and use public functions or variables defined in interface headers files."
#define ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS
#define UNDEF_ASCENDC_TENSOR_API_INCLUDE_COMPILER_INTERNAL_HEADERS_ASCENDC
#endif

/*!
 * \file l1_to_l0scaleb_copy.h
 * \brief
 */
...
__aicore__ inline void copy_l1_to_l0scaleb::copy(const Args&... args)
{
    load_data<trait, Args...>(args...);
}

template <const l1_to_l0scaleb_trait& trait, typename DstTensor, typename SrcTensor>
__aicore__ inline void copy_l1_to_l0scaleb::load_data(const DstTensor& dst, const SrcTensor& src)
{
    using dst_pos = get_mem_location<DstTensor>;
    using src_pos = get_mem_location<SrcTensor>;
    static_assert(
        Std::is_same_v<dst_pos, location::l0scaleb>,
        "For copy_l1_to_l0scaleb, the destination tensor must be located in L0ScaleB.");
    static_assert(
        Std::is_same_v<src_pos, location::l1>, "For copy_l1_to_l0scaleb, the source tensor must be located in L1.");
    ...
    TENSOR_API_DEBUG_CHECK(debug_check_layout, dst.layout(), "dst", "copy_l1_to_l0scaleb");
    TENSOR_API_DEBUG_CHECK(debug_check_layout, src.layout(), "src", "copy_l1_to_l0scaleb");
    TENSOR_API_DEBUG_CHECK(debug_check_copy_size, src, dst, "copy_l1_to_l0scaleb");
    using copy_l1_to_l0scaleb_impl =
        typename copy_l1_to_l0scaleb_routing<current_arch_version, dst_pattern, src_pattern>::type;
    copy_l1_to_l0scaleb_impl::template run<trait, DstTensor, SrcTensor>(dst, src);
}

template <
    const l1_to_l0scaleb_trait& trait, typename DstTensor, typename SrcTensor, typename DstCoord, typename SrcCoord,
    typename CopyShape>
__aicore__ inline void copy_l1_to_l0scaleb::load_data(
    ...
    const CopyShape& copy_shape)
{
    ...
    copy_l1_to_l0scaleb_impl::template run<trait, DstTensor, SrcTensor>(
