...
enum DeviceIdType { DEVICE_ID_TYPE_PHY_ID = 0, DEVICE_ID_TYPE_SDID };
...
using QpConfig = struct QpConfigDef {
    ...
    QpConfigDef(
        IpAddress& selfIp, IpAddress& peerIp, u32 maxWr, u32 maxSendSge, u32 maxRecvSge, s32 sqEvent, s32 rqEvent)
        : selfIp(selfIp),
          peerIp(peerIp),
          maxWr(maxWr),
...
using QpInfo = struct QpInfoDef {
    ...
    QpInfoDef()
        : rdmaHandle(nullptr),
          ...
          sendChannel(nullptr),
          recvChannel(nullptr),
          trafficClass(HCCL_COMM_TRAFFIC_CLASS_CONFIG_NOT_SET),
          serviceLevel(HCCL_COMM_SERVICE_LEVEL_CONFIG_NOT_SET)
    {}
    QpInfoDef(
        QpConfig attr, RdmaHandle rdmaHandle, QpHandle qpHandle, struct ibv_qp* qp, void* context,
...
using CqInfo = struct CqInfoDef {
    ...
    {}
    CqInfoDef(
        struct ibv_cq* sq, struct ibv_cq* rq, void* context, u32 depth, s32 sqEvent, s32 rqEvent,
        void* srqContext = nullptr, struct ibv_comp_channel* sendChannel = nullptr,
...
RequestHandle RaUbLocalMemRegAsync(
    RdmaHandle handle, const HrtRaUbLocMemRegParam& in, vector<char_t>& out, void*& lmemHandle);
RequestHandle RaUbLocalMemUnregAsync(RdmaHandle rdmaHandle, LocMemHandle lmemHandle);

RequestHandle RaUbCreateJettyAsync(
...
using RaUbGetTpInfoParam = struct RaUbGetTpInfoParamDef {
    ...
    {
        return StringFormat(
            "RaUbGetTpInfoParam[locAddr=%s, rmtAddr=%s, tpProtocol=%s]", locAddr.Describe().c_str(),
            rmtAddr.Describe().c_str(), tpProtocol.Describe().c_str());
    }
};

RequestHandle RaUbGetTpInfoAsync(
    const RdmaHandle rdmaHandle, const RaUbGetTpInfoParam& param, vector<char_t>& out, uint32_t& num);

RequestHandle RaUbImportJettyAsync(
    const RdmaHandle rdmaHandle, const HrtRaUbJettyImportedInParam& in, vector<char_t>& out, void*& remQpHandle);
RequestHandle RaUbTpImportJettyAsync(
    const RdmaHandle rdmaHandle, const HrtRaUbJettyImportedInParam& in, vector<char_t>& out, void*& remQpHandle);
...
void HrtRaWaitEventHandle(
    int event_handle, std::vector<SocketEventInfo>& event_infos, int timeout, unsigned int maxevents, u32& events_num);
void HrtRaGetSecRandom(u32* value, u32& devPhyId);

HcclResult HrtRaCreateQpWithCq(
    RdmaHandle rdmaHandle, s32 sqEvent, s32 rqEvent, void* sendChannel, void* recvChannel, QpInfo& info,
    bool isHdcMode);
...
HcclResult HrtRaCtxQpDestoryBatch(
    const RdmaHandle handle, const std::unordered_set<JettyHandle>& jettyHandles,
...
HcclResult HrtRaSetTpAttrAsync(
