...
class CheckFuncClassSinCos : public DataTypeCheckFuncBasicClass,
...
public:
        ...
        const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t count)
    {
        ...
        CalCountCheckFuncBasicClass::CalCountVerifyingParameters(
            ARG_AND_STRING(count), VA_ARGS_TO_MAKE_TUPLE(dst0, dst1, src));
        ...
        MultipleTensorCheckFuncBasicClass::TensorReuseVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(dst0, src));

        MultipleTensorCheckFuncBasicClass::TensorReuseVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(dst1, src));
    };
};
} // namespace HighLevelApiCheck
} // namespace AscendC
