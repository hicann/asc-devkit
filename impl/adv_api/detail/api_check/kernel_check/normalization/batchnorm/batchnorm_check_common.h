...
/*!
 * \file batchnorm_check_common.h
 * \brief
 */
#if !defined(__ASCENDC_INCLUDE_INTERNAL_HEADERS__)
#pragma message( \
    "impl/adv_api/detail/api_check/kernel_check/normalization/batchnorm/batchnorm_check_common.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"adv_api/normalization/batchnorm.h\"\" and use public functions or variables defined in interface headers files.")
#define __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#define __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_BATCHNORM_CHECK_COMMON_H__
#endif
...
private:
        ...
        const LocalTensor<uint8_t>& sharedTmpBuffer, const T epsilon, BatchNormTiling& tiling)
    {
        ASCENDC_ASSERT(((gamm.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[BatchNorm] The gamm length is %lu, should be 32B aligned.", gamm.GetSize() * sizeof(T));
        });
        ASCENDC_ASSERT(((beta.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[BatchNorm] The beta length is %lu, should be 32B aligned.", beta.GetSize() * sizeof(T));
        });
    }
};
...
#if defined(__UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_BATCHNORM_CHECK_COMMON_H__)
#undef __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#undef __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_BATCHNORM_CHECK_COMMON_H__
#endif
