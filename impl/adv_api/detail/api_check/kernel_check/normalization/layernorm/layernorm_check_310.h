...
public:
    ...
    {
        VerifyingParameters<T, isReuseSource>(output, outputMean, outputVariance, inputX, gamma, beta, epsilon, tiling);
    }
...
private:
        ...
        LayerNormTiling& tiling)
    {
        ASCENDC_ASSERT(((gamma.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[LayerNorm] The gamma length is %u, should be 32B aligned.",
                gamma.GetSize() * sizeof(T));
        });
        ASCENDC_ASSERT(((beta.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[LayerNorm] The beta length is %u, should be 32B aligned.", beta.GetSize() * sizeof(T));
        });
    }
};
...
public:
        ...
        const LocalTensor<uint8_t>& sharedTmpBuffer, const T epsilon, LayerNormTiling& tiling)
    {
        ...
        CheckLayerNormParamsClass::CheckLayerNormParams<T, isReuseSource>(
            output, outputMean, outputVariance, inputX, gamma, beta, epsilon, tiling);
    };
};
...
public:
    ...
    __aicore__ inline void CheckLayerNormRstdParams(
        ...
        const LocalTensor<T>& inputX, const LocalTensor<U>& inputGamma, const LocalTensor<U>& inputBeta,
        const float epsilon, const LocalTensor<uint8_t>& sharedTmpBuffer, const LayerNormPara& para,
        const LayerNormSeparateTiling& tiling)
    {
        VerifyingParameters<U, T, isReuseSource, config>(
            output, outputMean, output1, inputX, inputGamma, inputBeta, epsilon, sharedTmpBuffer, para, tiling);
...
public:
        ...
        const LayerNormSeparateTiling& tiling)
    {
        ...
        MultipleTensorCheckFuncBasicClass::TensorReuseVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(output, inputX));
