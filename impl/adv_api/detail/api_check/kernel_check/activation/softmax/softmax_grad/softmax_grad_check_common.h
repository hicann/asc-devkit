...
/*!
 * \file softmax_grad_check_common.h
 * \brief
 */
#if !defined(__ASCENDC_INCLUDE_INTERNAL_HEADERS__)
#pragma message( \
    "impl/adv_api/detail/api_check/kernel_check/activation/softmax/softmax_grad/softmax_grad_check_common.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"adv_api/activation/softmaxgrad.h\"\" and use public functions or variables defined in interface headers files.")
#define __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#define __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_GRAD_CHECK_COMMON_H__
#endif

#ifndef IMPL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_GRAD_SOFTMAX_GRAD_CHECK_COMMON_H_
#define IMPL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_GRAD_SOFTMAX_GRAD_CHECK_COMMON_H_
...
constexpr uint32_t CHECK_SOFTMAXGRADFRONT_SRCM_SIZE = 8;

class CheckSoftmaxGradParamsClass {
public:
    template <typename T, bool isReuseSource = false, bool isDataFormatNZ = false>
    __aicore__ inline void CheckSoftmaxGradParams(
...
private:
        ...
        const SoftMaxShapeInfo& softmaxShapeInfo)
    {
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[SoftmaxGrad] The softmaxShapeInfo.srcK is %u, should be 32B aligned.",
        ...
        ASCENDC_ASSERT(
            (softmaxShapeInfo.srcK * softmaxShapeInfo.srcM <= srcTensor.GetSize() || HighLevelAPIParametersPrint), {
                KERNEL_LOG(
                    KERNEL_ERROR,
                    "[SoftmaxGrad] The softmaxShapeInfo.srcK is %u, softmaxShapeInfo.srcM is %u, "
        ...
        ASCENDC_ASSERT(
            ((dstTensor.GetSize() == srcTensor.GetSize() && srcTensor.GetSize() == gradTensor.GetSize()) ||
             HighLevelAPIParametersPrint),
            {
                KERNEL_LOG(
                    KERNEL_ERROR,
                    "[SoftmaxGrad] The dstTensor size is %u, srcTensor size %u, gradTensor size is %u, they should be "
                    "equal.",
                    dstTensor.GetSize(), srcTensor.GetSize(), gradTensor.GetSize());
            });
    }
...
class CheckFuncClassSoftmaxGrad : public DataTypeCheckFuncBasicClass,
                                  public CheckSoftmaxGradParamsClass,
...
#endif // IMPL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_GRAD_SOFTMAX_GRAD_CHECK_COMMON_H_

#if defined(__UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_GRAD_CHECK_COMMON_H__)
#undef __ASCENDC_INCLUDE_INTERNAL_HEADERS__
#undef __UNDEF_ASCENDC_INCLUDE_INTERNAL_HEADERS_SOFTMAX_GRAD_CHECK_COMMON_H__
#endif
