...
#pragma message( \
    "impl/basic_api/dav_l311/kernel_operator_dump_tensor_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_tpipe.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void DumpTensorLocal2GMImpl(const LocalTensor<T>& src, uint32_t desc, uint32_t dumpSize)
{
    ...
    if (ptr->dumpOffset < (offset + sizeof(DumpMessageHead))) {
        ASCENDC_ASSERT((false), {
            KERNEL_LOG(
                KERNEL_ERROR, "dumpOffset is %d, which must less than remained space %d", ptr->dumpOffset,
    ...
    if (srcHWPos == Hardware::UB) {
        ...
        if constexpr (!(IsSameType<T, float>::value)) {
            copy_matrix_cc_to_gm(
                (__gm__ T*)(ptr->dumpAddr), (__cc__ T*)(src.GetPhyAddr()), 0, n, m, m * BLOCK_CUBE, m, 0, 0,
                static_cast<uint64_t>(QuantMode_t::NoQuant), static_cast<uint8_t>(false), false, false,
                static_cast<uint64_t>(QuantMode_post::NoConv), 0, false, false, 0, false, false, true, false, false,
                false);
        } else {
...
__aicore__ inline bool CheckDumpValid(uint32_t offset)
{
    ...
    if (ptr->dumpOffset < (offset + sizeof(DumpMessageHead) + ONE_DUMP_BACKUP_SIZE)) {
        KERNEL_LOG(
            KERNEL_ERROR, "dumpOffset is %d, which must less than remained space %ld", ptr->dumpOffset,
...
template <typename T>
__aicore__ inline void DumpGMTailImpl(
    LocalTensor<T>& tmp, uint32_t alignSize, uint64_t tmpAddr, uint64_t gmAddr, uint32_t offset)
{
...
__aicore__ inline void DumpTensorGM2GMImpl(const GlobalTensor<T>& src, uint32_t desc, uint32_t dumpSize)
{
    ...
    for (int i = 0; i < offset / ONE_DUMP_BACKUP_SIZE; i++) {
        DataCopyGM2UBImpl((__ubuf__ T*)tmp.GetPhyAddr(), (__gm__ T*)(tmpAddr + ONE_DUMP_BACKUP_SIZE * i), backupParams);
        PipeBarrier<PIPE_ALL>();
