...
#pragma message( \
    "impl/basic_api/dav_m310/kernel_operator_sys_var_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_operator_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported GetProgramCounter on current device"); });
    return 0;
}

__aicore__ inline int64_t GetSystemCycleImpl()
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported GetSystemCycle on current device"); });
    return 0;
}
...
__aicore__ inline void SetPcieRDCtrlImpl(bool isSetPcie, uint8_t maxBurstLen)
{
    (void)(isSetPcie);
    (void)(maxBurstLen);
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported SetPcieRDCtrl on current device"); });
}
...
__aicore__ inline void SetPcieWRCtrlImpl(bool isSetPcie, uint8_t maxBurstLen)
{
    (void)(isSetPcie);
    (void)(maxBurstLen);
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported SetPcieWRCtrl on current device"); });
}

__aicore__ inline void TrapImpl()
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported Trap on current device"); });
}
} // namespace AscendC
