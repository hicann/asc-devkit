...
#pragma message( \
    "impl/basic_api/dav_v516/kernel_operator_vec_duplicate_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void VecDupLevel0VFImpl(
    ...
    const uint8_t repeatTime, const uint16_t dstBlockStride, const uint8_t dstRepeatStride)
{
    ...
    if (Internal::IsCounterMode()) {
        ...
        VF_CALL<VecDupLevel0VFImpl<isSetMask, isMaskBitMode, false, T>>(
            dst, scalarValue, maskArray, maskCount, repeatTime, dstBlockStride, dstRepeatStride, maskBuf);
        if constexpr (!isSetMask) {
        ...
        VF_CALL<VecDupLevel0VFImpl<isSetMask, isMaskBitMode, true, T>>(
            dst, scalarValue, maskArray, maskCount, repeatTime, dstBlockStride, dstRepeatStride, maskBuf);
...
__aicore__ inline void InterleaveImplNormal(
...
template <typename T, bool hasUnalign = true>
__aicore__ inline void InterleaveImplB64(
    __ubuf__ T* dst0Local, __ubuf__ T* dst1Local, __ubuf__ T* src0Local, __ubuf__ T* src1Local, const int32_t calCount)
{
    ...
    if constexpr (!hasUnalign) {
        ...
        for (uint16_t i = 0; i < repeatTime; ++i) {
            ...
            Reg::LoadUnAlign(src0RegTmp1, ureg, src0Local + halfCount + i * sregLower + VECTOR_REG_WIDTH / sizeof(T));
            Reg::LoadUnAlignPre(ureg, src1Local + halfCount + i * sregLower);
            ...
            Reg::LoadUnAlign(src1RegTmp1, ureg, src1Local + halfCount + i * sregLower + VECTOR_REG_WIDTH / sizeof(T));
...
__aicore__ inline void DeInterleaveImplNormal(
    __ubuf__ T* dst0Local, __ubuf__ T* dst1Local, __ubuf__ T* src0Local, __ubuf__ T* src1Local, const int32_t calCount)
{
    Reg::RegTensor<T> src0Reg, src1Reg, dst0Reg, dst1Reg;
    uint32_t halfCount = static_cast<uint32_t>(calCount) / 2;
...
template <typename T, bool hasUnalign = true, bool hasSrc1 = true>
__aicore__ inline void DeInterleaveImplB64(
    __ubuf__ T* dst0Local, __ubuf__ T* dst1Local, __ubuf__ T* src0Local, __ubuf__ T* src1Local, const int32_t calCount)
{
    Reg::RegTensor<T, Reg::RegTraitNumTwo> src0Reg, src1Reg, dst0Reg, dst1Reg;
    uint32_t halfCount = static_cast<uint32_t>(calCount) / 2;
