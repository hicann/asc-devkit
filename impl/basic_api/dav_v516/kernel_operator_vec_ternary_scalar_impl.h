...
#pragma message( \
    "impl/basic_api/dav_v516/kernel_operator_vec_ternary_scalar_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file may be removed in the future. Please use \"#include \"basic_api/kernel_vec_intf.h\"\" and use public functions or variables defined in interface headers files.")
...
__aicore__ inline void VecAxpyLevel2VFImpl(__ubuf__ T* dst, __ubuf__ U* src, U scalarValue, const uint32_t calCount)
{
    RegU srcReg;
    RegT dstReg;
    ...
    __ubuf__ T* dst, __ubuf__ U* src, U scalarValue, const uint32_t calCount)
{
    if constexpr (SupportBytes<T, 8>()) {
        VF_CALL<VecAxpyLevel2VFImpl<
            func, T, U, Reg::RegTensor<T, Reg::RegTraitNumTwo>, Reg::RegTensor<U, Reg::RegTraitNumTwo>>>(
            dst, src, scalarValue, calCount);
    } else {
        VF_CALL<VecAxpyLevel2VFImpl<func, T, U, Reg::RegTensor<T>, Reg::RegTensor<U>>>(dst, src, scalarValue, calCount);
    }
}
...
__aicore__ inline void VecAxpyVFImpl(
    ...
    const uint8_t repeatTime, const UnaryRepeatParams& repeatParams)
{
    ...
    if (Internal::IsCounterMode()) {
        ...
        VF_CALL<VecAxpyVFImpl<func, isSetMask, isMaskBitMode, false, T, U>>(
        ...
        VF_CALL<VecAxpyVFImpl<func, isSetMask, isMaskBitMode, true, T, U>>(
            dst, src, scalarValue, maskArray, maskCount, repeatTime, repeatParams, maskBuf);
    }
}
} // namespace Internal
...
__aicore__ inline void Axpy(RegT& dstReg, RegU& srcReg, U scalarValue, Reg::MaskReg& mask)
{
    ...
    const UnaryRepeatParams& repeatParams)
{
    static_assert(
        SupportType<
            Tuple<T, U>, Tuple<half, half>, Tuple<float, float>, Tuple<bfloat16_t, bfloat16_t>, Tuple<float, half>>(),
...
__aicore__ inline void AxpyImpl(__ubuf__ T* dst, __ubuf__ U* src, const U& scalarValue, const int32_t& calCount)
{
    static_assert(
        SupportType<
            Tuple<T, U>, Tuple<half, half>, Tuple<float, float>, Tuple<bfloat16_t, bfloat16_t>, Tuple<float, half>,
            Tuple<uint64_t, uint64_t>, Tuple<int64_t, int64_t>>(),
        "current data type is not supported on current device!");
    if constexpr (SupportBytes<T, 8>()) {
        constexpr auto func =
