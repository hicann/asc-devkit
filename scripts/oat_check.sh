...
# Check oat-py version AND structural integrity (key modules must be importable).
# Returns: "ok" | "old" | "broken" | "missing"
_check_oat_version() {
    "$_PYTHON" -c "
...
# 2. Structural sanity: verify the key modules added in 1.0.x exist.
#    An old/partial installation may pass the version check but be unusable.
#    find_spec raises ModuleNotFoundError when the parent package is missing,
#    so wrap each check in try/except.
for mod in ('oat.walker.directory_walker', 'oat.analysis.pipeline', 'oat.config.loader'):
    try:
...
try:
    ...
    # Metadata unavailable (e.g. installed without wheel metadata).
    # Structural check already passed, treat as acceptable.
    print('ok')
...
if [ "$_OAT_OK" != "ok" ]; then
    case "$_OAT_OK" in
        old)    echo "[OAT] oat-py is outdated. Upgrading to oat-py>=${_OAT_MIN} ..." ;;
        broken) echo "[OAT] oat-py installation is broken. Reinstalling oat-py>=${_OAT_MIN} ..." ;;
        *)      echo "[OAT] oat-py not found. Installing oat-py>=${_OAT_MIN} ..." ;;
    esac
    _LOCK_FILE="/tmp/oat_pip_install.lock"
    # flock serialises concurrent pip installs; re-check inside so waiting
    # processes skip the install when the first one already finished.
    if command -v flock >/dev/null 2>&1; then
        (
            flock -w 120 9
            _RECHECK=$(_check_oat_version)
            if [ "$_RECHECK" != "ok" ]; then
                # Use --force-reinstall for broken installations to ensure a clean state.
                if [ "$_RECHECK" = "broken" ] || [ "$_OAT_OK" = "broken" ]; then
                    "$_PYTHON" -m pip install --quiet --force-reinstall "oat-py>=${_OAT_MIN}"
                else
...
if command -v flock >/dev/null 2>&1; then
    ...
    # Re-check done-marker after acquiring the lock � covers both paths:
    # (a) waited for lock: previous instance may have completed normally
    # (b) acquired immediately: previous holder may have already created the marker
    #     before releasing the lock (race window between pre-flock check and flock acquire)
    if [ -n "$_DONE_MARKER" ] && [ -f "$_DONE_MARKER" ]; then
        echo "[OAT] Scan already completed by another instance. Skipping."
        exit 0
    fi
    if [ "$_WAITED_FOR_LOCK" = "1" ]; then
