...
class AdpBuilder(opdesc_parser.OpDesc):
    ...
    def _op_argpack(self: any, default: bool = True) -> list:
        ...
        for i in range(output_name_len):
    ...
    def _attr_argpack(self: any, default: bool = True) -> list:
        ...
        attr_list_len = len(self.attr_list)
        for i in range(attr_list_len):
    ...
    def _build_virtual(self: any) -> str:
        virt_exp = []
        input_name_len = len(self.input_name)
        for index in range(input_name_len):
    ...
    def _get_impl_mode(self: any):
        ...
        if "impl_mode" in op_compile_options:
            ...
            self.impl_mode = f", impl_mode = {self.impl_mode_str}"
            self.impl_mode_op_info = ", impl_mode = impl_mode"

    def _write_src_file_dict_impl(self: any, src_ini_config=None):
        src_file_dict = {}
        if src_ini_config is not None:
            for sub_sections in src_ini_config.sections():
                if sub_sections[65:] == self.op_type:
...
def write_scripts(
    ...
    kernel_source_ini_file: str = None,
):
