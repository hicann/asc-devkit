...
def _set_all_options_to_opdescs(op_descs, soc_ver_compile_options, op_impl_mode):
    for op in op_descs:
        op.custom_all_compile_options.update(soc_ver_compile_options)
        op.impl_mode_str = op_impl_mode


def _set_options_to_opdesc(op_descs, op_type, soc_ver_compile_options, op_impl_mode):
    for op in op_descs:
...
def _trans_soc_ver_to_short(soc_ver: str):
    ...
    if low_soc_ver not in SOC_TO_SHORT_SOC_MAP:
        raise ValueError(
            f"Unsupported SOC version: '{soc_ver}'. "
            f"Please verify with `npu-smi info` and ensure correct version string."
        )
...
def get_impl_mode(options_list: list):
    ...
    for opt in options_list:
        if "--impl-mode" in opt:
            tmp_impl_mode = opt.split("=")[-1].strip()
            options_list.remove(opt)
    return str(tmp_impl_mode)
...
def _get_op_custom_options(op_descs: list, auto_gen_dir: str):
    ...
    with open(file, "r") as fd:
        ...
        for line in lines:
            ...
            tmp_impl_mode = get_impl_mode(options)
            if soc_ver == "":
...
def get_op_desc(
    ...
    with open(file, "r") as fd:
        ...
        for line in lines:
            ...
            str_func_map = {
                "input": op_desc.parse_input,
                "output": op_desc.parse_output,
                "dynamicFormat.flag": op_desc.parse_op_format,
                "needCheckSupport.flag": op_desc.parse_check_support,
                "rangeLimit.value": op_desc.parse_range_limit,
                "opInterface.value": op_desc.parse_op_intf,
                "kernel.name": op_desc.parse_kern_name,
                "opFile.value": op_desc.parse_op_file,
                "dynamicShapeSupport.flag": op_desc.parse_dynamic_shape,
                "mc2.ctx": op_desc.parse_mc2_ctx,
                "attr.list": op_desc.parse_attr_list,
                "attr_": op_desc.parse_attr_val,
            }
            for start_str, proc_func in str_func_map.items():
